...
export const inspectionSortOptions = [
  ...
  'latest_upload',
  'oldest_upload',
  'newest_first_detected',
  'oldest_first_detected',
  'latest_observation',
  'oldest_observation',
  'newest_inspection',
  'oldest_inspection',
  ...
  const filtered = rows.filter((row) => {
    if (query) {
      const haystack = [
        ...
        ...row.notes,
        ...row.damageCaseIds,
        ...row.firstReporterNames,
        ...row.latestUploaderNames,
      ]
        .join(' ')
        .replaceAll('_', ' ')
        .toLocaleLowerCase()
    ...
    if (
      ...
      !row.severities.includes(filters.severity)
    )
      return false
    ...
    if (
      filters.period !== 'all' &&
      getInspectionPeriod(row.uploadAt, timeZone).period !== filters.period
    )
      return false
    ...
    if (filters.repairStatus !== 'all' && !row.repairStatuses.includes(filters.repairStatus))
      return false
    if (filters.firstReporter !== 'all' && !row.firstReporterIds.includes(filters.firstReporter))
      return false
    if (filters.latestUploader !== 'all' && !row.latestUploaderIds.includes(filters.latestUploader))
      return false
    if (filters.level3 && !row.hasLevel3) return false
    if (filters.damageState === 'new_damage' && !row.observationTypes.includes('new_damage'))
      return false
    if (
      filters.damageState === 'existing_damage' &&
      !row.observationTypes.includes('existing_damage_observed')
    )
      return false
    if (
      filters.damageState === 'recurring_damage' &&
      !row.observationTypes.includes('recurrent_damage')
    )
      return false
    if (
      filters.damageState === 'duplicate_observations' &&
      !row.observationTypes.includes('possible_duplicate')
    )
      return false
    if (filters.today && getInspectionLocalDateKey(row.uploadAt, timeZone) !== todayKey)
      return false
    return true
  })
...
function compareInspections(a: InspectionSearchRow, b: InspectionSearchRow, sort: InspectionSort) {
  ...
  switch (sort) {
    case 'newest_damage':
      return newest(a.latestDamageAt, b.latestDamageAt) || newest(a.createdAt, b.createdAt)
    case 'oldest_damage':
      return oldestDamage(a.firstDamageAt, b.firstDamageAt) || oldest(a.createdAt, b.createdAt)
    case 'latest_upload':
      return newest(a.uploadAt, b.uploadAt)
    case 'oldest_upload':
      return oldest(a.uploadAt, b.uploadAt)
    case 'newest_first_detected':
      return newest(a.firstDamageAt, b.firstDamageAt) || newest(a.createdAt, b.createdAt)
    case 'oldest_first_detected':
      return oldestDamage(a.firstDamageAt, b.firstDamageAt) || oldest(a.createdAt, b.createdAt)
    case 'latest_observation':
      return (
        newest(a.latestObservationAt, b.latestObservationAt) || newest(a.createdAt, b.createdAt)
      )
    case 'oldest_observation':
      return (
        oldestDamage(a.latestObservationAt, b.latestObservationAt) ||
        oldest(a.createdAt, b.createdAt)
      )
    case 'oldest_inspection':
      return oldest(a.createdAt, b.createdAt)
    case 'highest_severity':
      return maxSeverity(b) - maxSeverity(a) || newest(a.createdAt, b.createdAt)
    case 'lowest_severity':
      return maxSeverity(a) - maxSeverity(b) || newest(a.createdAt, b.createdAt)
    case 'most_images':
      return b.imageCount - a.imageCount || newest(a.createdAt, b.createdAt)
    case 'fewest_images':
      return a.imageCount - b.imageCount || newest(a.createdAt, b.createdAt)
    case 'most_observations':
      return (
        b.observationCount - a.observationCount ||
        newest(a.latestObservationAt, b.latestObservationAt)
      )
    case 'most_active_damage':
      return b.activeDamageCount - a.activeDamageCount || b.damageCount - a.damageCount
    case 'recently_updated':
      return newest(a.updatedAt, b.updatedAt)
    ...
    case 'repair_scheduled':
      return (
        Number(includesValue(b.repairStatuses, 'scheduled')) -
          Number(includesValue(a.repairStatuses, 'scheduled')) || newest(a.createdAt, b.createdAt)
      )
    case 'in_repair':
      return (
        Number(includesValue(b.repairStatuses, 'in_repair')) -
          Number(includesValue(a.repairStatuses, 'in_repair')) || newest(a.createdAt, b.createdAt)
      )
    case 'repaired':
      return (
        Number(b.repairStatuses.includes('repaired')) -
          Number(a.repairStatuses.includes('repaired')) || newest(a.createdAt, b.createdAt)
      )
    case 'driver_name':
      return a.driverName.localeCompare(b.driverName, undefined, { numeric: true })
    case 'van_number':
      return a.vanNumber.localeCompare(b.vanNumber, undefined, { numeric: true })
    case 'inspection_number':
      return a.inspectionNumber.localeCompare(b.inspectionNumber, undefined, { numeric: true })
...
export function uniqueOptions(
  rows: InspectionSearchRow[],
  field: 'damageTypes' | 'regions' | 'severities' | 'repairStatuses'
) {
  return [...new Set(rows.flatMap((row) => row[field]).filter(Boolean))].sort((a, b) =>
    a.localeCompare(b)
  )
}
