...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (errorParam) {
    ...
    return NextResponse.redirect(`${DASHBOARD_REDIRECT}?error=${encodeURIComponent(errorParam)}`)
  }
  ...
  if (!cookieVerify.valid || !callbackVerify.valid) {
    console.error(
      '[SquareOAuth] State validation failed:',
      cookieVerify.error ?? callbackVerify.error
    )
  ...
  const { error: upsertError } = await supabase.from('payment_accounts').upsert(accountPayload, {
  ...
  await supabase.from('payment_providers').upsert(
    {
      tenant_id: tenantId,
      provider_key: 'square',
      is_enabled: true,
