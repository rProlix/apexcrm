...
import { AlertTriangle, CheckCircle2, Loader2, MessageSquare, RefreshCw, Search, ShieldCheck, Trash2, Unplug } from 'lucide-react'
...
export function SlackSettingsClient({
  ...
  async function testChannelConfiguration() {
    ...
    const result = (await response.json()) as { healthy?: boolean; issues?: string[]; error?: string }
    setMessage(
      result.healthy
  ...
  const availableMaintenanceChannels = channels.filter((channel) =>
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-white/10 bg-graphite-800 p-6">
        ...
        {health && (
          <div className={`mt-4 rounded-xl border px-4 py-3 ${health.healthy ? 'border-emerald-400/20 bg-emerald-400/[0.06]' : 'border-amber-300/20 bg-amber-300/[0.06]'}`}>
            <div className="flex items-center gap-2 text-sm">
              {health.healthy ? <ShieldCheck className="h-4 w-4 text-emerald-300" /> : <AlertTriangle className="h-4 w-4 text-amber-200" />}
              <span className={health.healthy ? 'text-emerald-200' : 'text-amber-100'}>Configuration {health.healthy ? 'healthy' : 'needs attention'}</span>
            </div>
            {health.issues.length > 0 && <ul className="mt-2 space-y-1 text-xs text-amber-100/70">{health.issues.map((issue) => <li key={issue}>• {issue}</li>)}</ul>}
          </div>
        )}
      </section>
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-xs text-white/35">
                Last processed inspection upload: {health?.lastInspectionUploadAt ? `${new Date(health.lastInspectionUploadAt).toLocaleString()} · ${health.lastInspectionStatus ?? 'processed'}` : 'None'}
              </p>
            </div>
            <button type="button" onClick={refreshChannels} disabled={Boolean(busy)} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-white/60 hover:bg-white/5 disabled:opacity-40">
              <RefreshCw className={`mr-1.5 inline h-3.5 w-3.5 ${busy === 'refresh' ? 'animate-spin' : ''}`} />Refresh
            </button>
          </div>
          <div className="mt-5 max-h-80 space-y-2 overflow-y-auto pr-2">
            ...
            {channels.map((channel) => (
              ...
              >
                ...
                <input
                  type="checkbox"
                  disabled={!channel.isMember || channel.isArchived || !channel.isAccessible || maintenanceChannelId === channel.id}
          ...
          <div className="mt-7 border-t border-white/10 pt-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-white/45">
                  Messages posted in this channel create Fleet maintenance records, notes, and attachments.
                </p>
                <p className="mt-2 text-xs text-white/35">
                  Last processed maintenance message: {health?.lastMaintenanceMessageAt ? `${new Date(health.lastMaintenanceMessageAt).toLocaleString()} · ${health.lastMaintenanceStatus ?? 'processed'}` : 'None'}
                </p>
              </div>
            ...
            <p className="mt-3 rounded-lg border border-amber-300/15 bg-amber-300/[0.06] px-3 py-2 text-xs text-amber-100/70">
              The maintenance channel must be different from the channels used to upload van inspection images.
            </p>
            <p className="mt-2 text-xs text-white/45">
              Maintenance-channel messages create maintenance records, not Van Damage AI inspections.
            </p>
            {selectedMaintenance && (
              <div className={`mt-4 rounded-xl border px-3 py-3 ${selectedMaintenance.isAccessible && !selectedMaintenance.isArchived && selectedMaintenance.isMember ? 'border-emerald-400/15 bg-emerald-400/[0.05]' : 'border-amber-300/20 bg-amber-300/[0.06]'}`}>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm text-white/80">Selected: #{selectedMaintenance.name}</p>
                    <p className="mt-1 text-xs text-white/35">
                      {selectedMaintenance.isPrivate ? 'Private' : 'Public'} · {selectedMaintenance.isArchived ? 'Archived' : selectedMaintenance.isAccessible ? 'Accessible' : 'Inaccessible'} · {selectedMaintenance.isMember ? 'Bot joined' : 'Bot not joined'}
                    </p>
                  </div>
                  <button type="button" onClick={() => setMaintenanceEnabled(false)} className="rounded-lg border border-white/10 px-2 py-1 text-xs text-white/50">Disable</button>
                </div>
              </div>
            )}
            <label className="focus-within:focus-ring mt-4 flex min-h-11 items-center rounded-xl border border-white/10 bg-graphite-900 px-3">
              ...
              <input value={channelSearch} onChange={(event) => setChannelSearch(event.target.value)} placeholder="Search available Slack channels" className="w-full bg-transparent text-sm text-white outline-none placeholder:text-white/25" />
            </label>
            <div className="mt-2 max-h-56 space-y-2 overflow-y-auto pr-2">
              {availableMaintenanceChannels.length === 0 && <p className="rounded-lg border border-dashed border-white/10 px-3 py-4 text-center text-xs text-white/30">No available joined channels match this search.</p>}
              {availableMaintenanceChannels.map((channel) => (
                ...
                >
                  ...
                  <span className="text-xs text-white/30">{channel.isPrivate ? 'private' : 'public'} · bot joined</span>
                </button>
              ))}
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              <button onClick={saveChannels} disabled={Boolean(busy)} className="rounded-lg bg-white px-4 py-2 text-sm font-medium text-graphite-950 disabled:opacity-50">
                {busy === 'channels' && <Loader2 className="mr-1.5 inline h-4 w-4 animate-spin" />}Save routing
              </button>
              <button onClick={testChannelConfiguration} disabled={Boolean(busy)} className="rounded-lg border border-white/10 px-4 py-2 text-sm text-white/65 hover:bg-white/5 disabled:opacity-50">
                {busy === 'channel-test' && <Loader2 className="mr-1.5 inline h-4 w-4 animate-spin" />}Test routing
              </button>
            </div>
          </div>
        </section>
