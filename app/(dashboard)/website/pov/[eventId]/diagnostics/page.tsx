...
export default async function PovDiagnosticsPage({ params }: Props) {
  ...
  const rows: Array<[string, React.ReactNode]> = [
    ...
    [
      'Allow photos / videos / audio',
      `${diag.upload_settings.allow_photos} / ${diag.upload_settings.allow_videos} / ${diag.upload_settings.allow_audio}`,
    ],
    [
      'Video max / Audio max (s)',
      `${diag.upload_settings.video_max_seconds} / ${diag.upload_settings.audio_max_seconds}`,
    ],
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <Link
          href={`/website/pov/${event.id}`}
          className="inline-flex items-center gap-1.5 text-xs text-white/40 hover:text-white/70 mb-3"
        >
      ...
      <div
        className={`rounded-2xl border px-5 py-4 flex items-center gap-3 ${diag.storage.exists ? 'bg-emerald-500/8 border-emerald-500/20' : 'bg-red-500/8 border-red-500/20'}`}
      >
        {diag.storage.exists ? (
      ...
      <div className="rounded-2xl border border-surface-border overflow-hidden">
        {rows.map(([k, v], i) => (
          <div
            key={k}
            className={`flex items-center justify-between gap-4 px-5 py-3 ${i !== 0 ? 'border-t border-surface-border' : ''}`}
          >
