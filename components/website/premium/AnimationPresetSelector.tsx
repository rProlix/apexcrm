  ...
  return (
    <div className="space-y-4">
      ...
      <SelectField
        ...
        disabled={disabled || config.preset === 'none'}
      />
      <SelectField
        label="Section Style"
        value={config.stylePreset}
        onChange={(v) => set('stylePreset', v)}
        options={STYLE_OPTIONS}
        disabled={disabled}
      />
      <SelectField
        label="Image Treatment"
        value={config.imageTreatment}
        onChange={(v) => set('imageTreatment', v)}
        options={IMAGE_TREATMENT_OPTIONS}
        disabled={disabled}
      />
      <SelectField
        label="Button Treatment"
        value={config.buttonTreatment}
        onChange={(v) => set('buttonTreatment', v)}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                set('durationMs', Math.min(3000, Math.max(100, Number(e.target.value))))
              }
              disabled={disabled}
      ...
      <label className="flex items-center gap-3 cursor-pointer group">
        <div className="relative flex-shrink-0">
          ...
          />
          <div
            className={`w-10 h-6 rounded-full transition-colors duration-200 ${config.mobileEnabled ? 'bg-amber-500' : 'bg-white/10'}`}
          />
          <div
            className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 ${config.mobileEnabled ? 'translate-x-4' : ''}`}
          />
        </div>
