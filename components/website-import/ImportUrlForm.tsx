...
export function ImportUrlForm({ tenantId, onJobCreated, disabled }: Props) {
  ...
  function updateUrl(id: string, url: string) {
    setEntries((prev) =>
      prev.map((e) =>
        e.id === id ? { ...e, url, source_type: detectSourceType(url), error: undefined } : e
      )
    )
  }
              ...
              >
                <div
                  className={cn(
                    ...
                    entry.error
                      ? 'border-red-400/40'
                      : 'border-white/10 focus-within:border-amber-400/40'
                  )}
                >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateType(entry.id, e.target.value as UrlEntry['source_type'])
                    }
