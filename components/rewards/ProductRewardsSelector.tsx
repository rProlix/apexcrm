  ...
  function saveProduct(productId: string) {
    ...
    startTransition(async () => {
      try {
        ...
        setErrors((prev) => ({
          ...prev,
          [productId]: err instanceof Error ? err.message : 'Save failed',
        }))
      }
    })
  }
    ...
    return (
      <div className="premium-panel premium-border rounded-2xl p-10 text-center">
        ...
        <p className="text-xs text-white/30 mt-1">
          Add products to your store to configure rewards.
        </p>
      </div>
    )
  }
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <label className="ml-auto flex items-center gap-1.5 cursor-pointer flex-shrink-0">
                      <div
                        onClick={() =>
                          updateConfig(product.id, 'rewards_enabled', !cfg.rewards_enabled)
                        }
                        className={`relative h-4 w-7 rounded-full transition-colors cursor-pointer ${cfg.rewards_enabled ? 'bg-amber-400' : 'bg-white/10'}`}
                      >
                        <div
                          className={`absolute top-0.5 h-3 w-3 rounded-full bg-white shadow transition-transform ${cfg.rewards_enabled ? 'translate-x-3' : 'translate-x-0.5'}`}
                        />
                      </div>
                      <span className="text-xs text-white/40">
                        {cfg.rewards_enabled ? 'Earns points' : 'No rewards'}
                      </span>
                    </label>
                  </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateConfig(product.id, 'rewards_multiplier', Number(e.target.value))
                          }
                          className="store-input w-full rounded-lg px-3 py-1.5 text-xs"
                        />
                      </div>
