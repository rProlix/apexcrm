...
export async function GET() {
  ...
  try {
    ...
    const { data: sections } = (await (db as any)
      ...
      data: Array<{ id: string; style_config?: Record<string, unknown> | null }> | null
      error: unknown
    }
    ...
    sectionsWithDesign = (sections ?? []).filter((s) => {
      ...
      return (
        sc && typeof sc === 'object' && 'design' in sc && sc.design && typeof sc.design === 'object'
      )
    }).length
  } catch {
    /* ignore */
  ...
  return NextResponse.json({
    ...
    sectionsTotal,
    sectionsWithDesign,
    sectionsWithDesignPct:
      sectionsTotal > 0 ? Math.round((sectionsWithDesign / sectionsTotal) * 100) : 0,
