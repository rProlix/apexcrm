...
export function HeroSection({ content, componentAnimations: ca, sectionDesign }: Props) {
  ...
  const headlineColor =
    sectionDesign?.textColor ?? (backgroundImage ? '#fff' : 'var(--ds-text, var(--color-text))')
  const subheadColor =
    sectionDesign?.subtextColor ??
    (backgroundImage ? 'rgba(255,255,255,0.85)' : 'var(--ds-muted, var(--color-muted))')
  ...
  return (
    ...
    >
      ...
      >
        <AnimatedElement
          as="h1"
          animConfig={ca?.heading ?? ca?.headline ?? ca?.text}
          style={{
        ...
        {subheadline && (
          <AnimatedElement
            as="p"
            animConfig={ca?.subheading ?? ca?.paragraph ?? ca?.text}
            index={1}
            style={{
