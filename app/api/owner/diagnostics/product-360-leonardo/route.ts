...
const FRAME_COLUMNS = ['image_url', 'frame_index'] as const
...
async function getMissingColumns(tableName: string, required: readonly string[]) {
  ...
  try {
    ...
    const present = new Set(
      ((data as Array<{ column_name?: string }> | null) ?? [])
...
export async function GET() {
  ...
  const env = Object.fromEntries(
    REQUIRED_ENV.map((name) => [name, process.env[name] ? 'present' : 'MISSING'])
  )
  const missingEnv = REQUIRED_ENV.filter((name) => !process.env[name])
  const textVariablesFormat =
    process.env.LEONARDO_360_TEXT_VARIABLES_FORMAT === 'json' ? 'json' : 'text'
  ...
  const examplePayload = {
    ...
    input: {
      nodeInputs: [
        {
          nodeId:
            process.env.LEONARDO_360_REFERENCE_IMAGE_NODE_ID ||
            'MISSING_LEONARDO_360_REFERENCE_IMAGE_NODE_ID',
        ...
        },
        {
          ...
          value:
            'Preserve camera distance, lens, crop, scale, and composition from the reference image.',
          settingName: 'camera',
        },
        {
          nodeId:
            process.env.LEONARDO_360_LIGHTING_NODE_ID || 'MISSING_LEONARDO_360_LIGHTING_NODE_ID',
          value: 'Preserve lighting, shadows, highlights, and atmosphere from the reference image.',
          settingName: 'lighting',
        },
        {
          nodeId:
            process.env.LEONARDO_360_BACKGROUND_NODE_ID ||
            'MISSING_LEONARDO_360_BACKGROUND_NODE_ID',
          value:
            'Preserve background, table surface, wall/backdrop, props, and arrangement from the reference image.',
  ...
  const fixes = [
    ...
    ...(!framesBucket
      ? ['Create Supabase Storage bucket product-360-frames or run migration 074.']
      : []),
    ...(!referencesBucket
      ? ['Create Supabase Storage bucket product-360-references or run migration 074.']
      : []),
    ...(packageColumns.missing.length
      ? [
          `product_360_packages missing columns: ${packageColumns.missing.join(', ')}. Run migration 074.`,
        ]
      : []),
    ...(frameColumns.missing.length
      ? [
          `product_360_frames missing columns: ${frameColumns.missing.join(', ')}. Run migration 074.`,
        ]
      : []),
    ...(!leonardoReachable
      ? ['Leonardo API probe failed. Check LEONARDO_API_KEY and account access.']
      : []),
  ]
  ...
  return NextResponse.json(
    {
      ...
      env: {
        ...env,
        LEONARDO_360_OUTPUT_IMAGE_URL_PATH: process.env.LEONARDO_360_OUTPUT_IMAGE_URL_PATH
          ? 'present'
          : 'optional-not-set',
