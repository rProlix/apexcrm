...
export async function POST(req: NextRequest) {
  ...
  if (bodyPlan && Object.keys(bodyPlan).length > 0) {
    ...
    const { data: runRow } = (await (db as any)
      ...
      .single()) as {
      data: { restyle_plan: Record<string, unknown>; status: string; tenant_id: string } | null
      error: unknown
    }

    if (!runRow) return NextResponse.json({ error: 'Restyle run not found.' }, { status: 404 })
  ...
  if (!rawPlan || Object.keys(rawPlan).length === 0)
    return NextResponse.json(
      { error: 'No restyle plan provided. Generate a preview first.' },
      { status: 400 }
    )
  ...
  if (normalizeError || !plan)
    return NextResponse.json(
      { error: normalizeError ?? 'Failed to process restyle plan.' },
      { status: 400 }
    )
  ...
  for (const dbSection of dbSections) {
    ...
    const existingStyleConfig = (
      typeof dbSection.style_config === 'object' && dbSection.style_config !== null
