...
async function restoreCanvaSettingsFromSnapshot(
  db: DB,
  tenantId: string,
  snapshot: Snapshot
): Promise<void> {
...
export async function applyCanvaImportWithRun(params: {
  ...
  try {
    beforeDraft = (await getCurrentWebsiteSnapshot(tenantId)).data as Snapshot
  } catch {
    /* ignore */
  }
  ...
  try {
    afterDraft = (await getCurrentWebsiteSnapshot(tenantId)).data as Snapshot
  } catch {
    /* ignore */
  }
  if (runId) {
...
export async function undoCanvaImport(params: {
  ...
  if (!beforeDraft)
    return { ok: false, error: 'No pre-import snapshot is available for this import.' }

  const applied = await applySnapshotToWebsiteTables(
    tenantId,
    beforeDraft as never,
    params.userId ?? ''
  )
  if (!applied.data)
    return { ok: false, error: applied.error ?? 'Could not restore the pre-import draft.' }
...
export async function restoreLastPublished(params: {
  ...
  if (!applied.data)
    return { ok: false, error: applied.error ?? 'Could not restore the published version.' }
  ...
  try {
    await db.from('website_canva_import_runs').insert({
      ...
      run_type: 'restore',
      status: 'completed',
      before_published_snapshot: snap,
      after_draft_snapshot: snap,
  ...
  try {
    ...
    if (!data) {
      return {
        ...
        hasPreImportSnapshot: false,
        hasBeforePublishedSnapshot: false,
        undoAvailable: false,
      }
    }
    return {
