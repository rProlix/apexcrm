...
// Returns time slots for a given date.
// Customers receive only available slots; admins receive all (with availability flag).
//
// Query params:
...
export async function GET(req: NextRequest) {
  ...
  // Reject requests for dates more than 1 year in the future
  ...
  const allSlots = await generateTimeSlots({ tenant_id, date })

  // Customers only see available slots — prevents information leakage about bookings
  const slots = isAdmin
    ? allSlots
    : allSlots.filter((s) => s.available)

  return NextResponse.json({ slots, date, total: allSlots.length, available: slots.filter((s) => s.available).length })
}

// ─── POST /api/appointments/availability ──────────────────────────────────────
// Bulk upsert availability rules (kept for backward compatibility with
// the AvailabilityEditor's legacy save path).
// Prefer /api/appointments/availability-rules for granular CRUD.
export async function POST(req: NextRequest) {
  ...
  const upserts = rules.map((rule: Record<string, unknown>) => {
    ...
    return {
      ...
      slot_duration_minutes: intervalMin,  // keep in sync for backward compat
