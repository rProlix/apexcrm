...
assert(
  typeof t1.subject.vessel === 'string' && t1.subject.vessel.length > 0,
  `vessel is non-empty: "${t1.subject.vessel}"`
)
...
assert(
  t6.subject.vessel.toLowerCase().includes('bowl'),
  `food gets bowl vessel: "${t6.subject.vessel}"`
)

// ── Test 7: drink product gets cup/glass vessel ───────────────────────────────
console.log('\nTest 7: drink product gets cup/glass vessel')
const t7 = normalizeSceneBlueprint(null, drinkSubject, {
  ...baseConfig,
  categoryPreset: 'beverage',
})
assert(
  t7.subject.vessel.toLowerCase().includes('cup') ||
    t7.subject.vessel.toLowerCase().includes('glass'),
...
assert(
  t8.subject.vessel === 'handmade ceramic ramen bowl',
  `custom vessel preserved: "${t8.subject.vessel}"`
)
...
assert(
  typeof t10.consistencyRules === 'object' && Array.isArray(t10.consistencyRules),
  'consistencyRules is array'
)
assert(
  typeof t10.lighting.style === 'string' && t10.lighting.style.length > 0,
  `lighting.style: "${t10.lighting.style}"`
)
