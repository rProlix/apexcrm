...
export type TextAnimation = 'fadeUpWords' | 'blurReveal' | 'scaleWords' | 'luxurySplit' | 'none'
...
export type CameraPathPreset = 'static' | 'orbit' | 'dollyIn' | 'craneUp' | 'arc'
...
export function normalizeScrollHeroContent(raw: unknown): Premium3DScrollHeroContent {
  ...
  const vsRaw =
    c.videoScrub && typeof c.videoScrub === 'object'
  ...
  const reducedMotionFallback = str(
    c.reducedMotionFallback,
    d.reducedMotionFallback!
  ) as ReducedMotionFallback
  ...
  const backgroundGradientStrength = clamp01(
    num(c.backgroundGradientStrength, d.backgroundGradientStrength!)
  )
  const heroHeight = (
    ['100vh', '120vh', '150vh', 'custom'].includes(str(c.heroHeight))
      ? str(c.heroHeight)
      : d.heroHeight!
  ) as HeroHeight
  const customHeroHeight = c.customHeroHeight ? str(c.customHeroHeight) : null
  const textRevealTiming = (
    ['early', 'middle', 'late'].includes(str(c.textRevealTiming))
      ? str(c.textRevealTiming)
      : d.textRevealTiming!
  ) as TextRevealTiming
  const contentAlignment = (
    ['left', 'center', 'right'].includes(str(c.contentAlignment))
      ? str(c.contentAlignment)
      : d.contentAlignment!
  ) as ContentAlignment
  ...
  const videoScrub: VideoScrubSettings = {
    ...
    reducedMotionFallback: str(
      vsRaw.reducedMotionFallback,
    ...
    backgroundGradientStrength: clamp01(
      num(vsRaw.backgroundGradientStrength, backgroundGradientStrength)
    ),
    heroHeight: (['100vh', '120vh', '150vh', 'custom'].includes(str(vsRaw.heroHeight))
      ? str(vsRaw.heroHeight)
      : heroHeight) as HeroHeight,
  ...
  return {
    ...
    activeImageSequenceAssetId: c.activeImageSequenceAssetId
      ? str(c.activeImageSequenceAssetId)
      : null,
    ...
    scrollTimeline:
      c.scrollTimeline && typeof c.scrollTimeline === 'object'
