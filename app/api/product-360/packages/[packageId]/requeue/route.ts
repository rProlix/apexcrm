...
export async function POST(req: NextRequest, ctx: Ctx) {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          type: 'forbidden',
          title: 'Forbidden',
          message: 'Only owners and admins can requeue packages.',
        },
      },
      { status: 403 }
    )
    ...
    return NextResponse.json(
      {
        ...
        error: {
          type: 'conflict',
          title: 'Archived',
          message: 'Unarchive this package before requeuing.',
        },
      },
      { status: 409 }
    )
  ...
  console.info(
    `[p360:requeue] pkg=${packageId} reset to draft, ${framesReset ?? 0} stuck frames cleared`
  )
