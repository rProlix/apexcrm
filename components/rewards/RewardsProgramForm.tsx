        ...
        >
          <div className="px-5 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-xs px-2 py-0.5 rounded-lg border font-medium ${STATUS_COLORS[p.status]}`}
              >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                <div>
                  ...
                  <p className="text-white font-semibold">
                    {p.earning_rules.points_per_dollar ?? 10}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p
                    className={p.settings.punch_cards_enabled ? 'text-emerald-400' : 'text-red-400'}
                  >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {p.earning_rules.bonus_points_products?.map((b) => {
                      ...
                      return (
                        <span
                          key={b.product_id}
                          className="text-xs px-2 py-1 rounded-lg bg-amber-400/10 border border-amber-400/20 text-amber-400"
                        >
        ...
        >
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="store-input w-full rounded-xl px-3 py-2 text-sm resize-none"
          ...
          <div className="flex flex-wrap gap-4">
            {(
              [
                ['pointsEnabled', pointsEnabled, setPointsEnabled, 'Points'],
                ['punchCardsEnabled', punchCardsEnabled, setPunchCardsEnabled, 'Punch Cards'],
                ['shopEnabled', shopEnabled, setShopEnabled, 'Shop'],
              ] as const
            ).map(([, val, set, lbl]) => (
              <label key={lbl} className="flex items-center gap-2 cursor-pointer">
                ...
                >
                  <div
                    className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${val ? 'translate-x-4' : 'translate-x-0.5'}`}
                  />
                </div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="submit"
              disabled={isPending}
              className="bg-gold-gradient text-graphite-900 font-semibold text-sm px-5 py-2 rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50"
            >
