...
export function computeOrbitAngles(
  frameCount: number,
...
export function serializeLockedSceneToPrompt(scene: Product360LockedScene): string {
  ...
  if (scene.foodDetails) {
    ...
    if (f.toppings.length > 0) {
      ...
      for (const t of f.toppings) {
        lines.push(
          `    • ${t.name}: ${t.count}, ${t.placement}, ${t.color}${t.size ? `, ${t.size}` : ''}`
        )
      }
    }
  ...
  lines.push(
    `  ${scene.vessel.type} — ${scene.vessel.material}, ${scene.vessel.color}, ${scene.vessel.size}`
  )
  ...
  lines.push(
    `  Table surface: ${scene.environment.tableSurfaceType}, ${scene.environment.tableSurfaceColor}, ${scene.environment.tableTexture}`
  )
  lines.push(
    `  Background/wall: ${scene.environment.wallOrBackgroundType}, ${scene.environment.wallOrBackgroundColor}`
  )
  lines.push(`  Atmosphere: ${scene.environment.atmosphere}`)
  if (scene.environment.props.length > 0) {
    lines.push(
      `  Props: ${scene.environment.props.map((p) => `${p.name} at ${p.position}`).join('; ')}`
    )
  } else {
  ...
  lines.push(
    `  ✕ Do not change the table surface (must stay: ${scene.environment.tableSurfaceType})`
  )
  lines.push(
    `  ✕ Do not change the background (must stay: ${scene.environment.wallOrBackgroundColor})`
  )
