...
export function convertCanvaHtml(
  html: string,
  ...
  const og = extractMeta(safeHtml, 'og:image')
  if (og) images.push(og)
  for (const m of safeHtml.matchAll(/background-image\s*:\s*url\(["']?([^"')]+)["']?\)/gi))
    images.push(m[1])
  ...
  if (headings.length > 1 || bodyParas.length) {
    const html =
      headings
        .slice(1, 3)
        .map((h) => `<h2>${escapeHtml(h)}</h2>`)
        .join('') + bodyParas.map((p) => `<p>${escapeHtml(p)}</p>`).join('')
    sections.push({
      section_type: 'about',
  ...
  if (!safeHtml.trim()) {
    warnings.push(
      'No HTML content was provided to convert. Add a Canva export (HTML) or use Preserve Canva Mode.'
    )
  }
  if (!absImages.length) warnings.push('No images could be extracted from the Canva source.')
  if (animations.length) {
    warnings.push(
      `${animations.length} animation(s) were approximated using NexoraNow presets. Use Preserve Canva Mode for exact fidelity.`
    )
  } else {
    warnings.push(
      'No animations were detected; sections use NexoraNow default entrance animations.'
    )
  }
  if (opts?.sourceUrl)
    warnings.push(
      'Live Canva URLs render fully only inside Canva. Converted Editable Mode rebuilds content for editing and may differ visually.'
    )
