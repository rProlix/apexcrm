          ...
          <div className="flex gap-2 flex-wrap">
            {['', 'open', 'sent_to_kitchen', 'ready', 'completed', 'cancelled'].map((s) => (
              <button
                ...
                onClick={() => {
                  setStatus(s)
                  if (s !== statusFilter) fetchOrders(s)
                }}
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[order.status] ?? 'text-zinc-400 bg-zinc-800'}`}
                    >
                      {order.status.replace(/_/g, ' ')}
                    </span>
                    <span
                      className={`text-xs font-medium ${PAYMENT_COLORS[order.payment_status] ?? 'text-zinc-500'}`}
                    >
