...
import type { InventoryPrediction, InventoryTrendSummary, InventoryItem } from './types'
...
export async function calculateSalesVelocity(
  ...
  const consumed30 = (movements as MovRow[]).reduce(
    (sum: number, m) => sum + Math.abs(m.quantity_delta),
    0
  )
...
export async function buildItemPrediction(
  ...
  item: Pick<
    InventoryItem,
    'id' | 'name' | 'current_quantity' | 'unit' | 'target_quantity' | 'reorder_point'
  >,
  predictionDays: number = 14
...
export async function buildTrendSummary(
  ...
  const [products7d, products30d, consumed7d, consumed30d, { data: activeItems }] =
    await Promise.all([
  ...
  const predictions = await Promise.all(
    itemsNeedingPrediction
      .slice(0, 20)
      .map((item) => buildItemPrediction(tenantId, item, predictionDays))
  )
