...
export async function POST(req: NextRequest) {
  ...
  if (scope === 'section' && plan.site_section_id) {
    ...
    const secAnim =
      sectionAnimations.find(
        (a) => !a.targetKey || a.targetKey === 'global' || a.sectionId === sectionId
      ) ??
      sectionAnimations[0] ??
      null
    if (secAnim) {
    ...
    const compAnims: Record<string, unknown> =
      (config.componentAnimations as Record<string, unknown>) ?? {}
  ...
  if (scope === 'page' && plan.site_page_id) {
    ...
    const { data: pageSections } = (await supabase
      ...
      data: Array<{ id: string; section_type: string; animation_config: unknown }> | null
    }
    ...
    for (const sec of pageSections ?? []) {
      ...
      const config: Record<string, unknown> =
        rawConf && typeof rawConf === 'object' && !Array.isArray(rawConf)
      ...
      const secAnim =
        sectionAnimations.find(
          (a) => a.sectionId === secId || a.targetKey === secType || a.targetKey === 'global'
        ) ?? sectionAnimations[0]
      ...
      const upgrade = upgrades.find(
        (u) =>
          (u.sectionId && u.sectionId === secId) || (u.sectionType && u.sectionType === secType)
      )
      if (upgrade) {
        config.style = {
          stylePreset: upgrade.stylePreset,
    ...
    const { error: pageErr } = await supabase
      .from('site_pages')
      .update({
        animation_config: pageAnimConfig as never,
        style_config: pageAnimConfig as never,
        updated_at: now,
      } as never)
      .eq('id', pageId)
  ...
  if (scope === 'global') {
    const { data: allSections } = (await supabase
      ...
      .select('id, section_type, animation_config' as 'id')
      .eq('tenant_id', tenantId)
      .eq('is_visible', true)) as unknown as {
      data: Array<{ id: string; section_type: string; animation_config: unknown }> | null
    }
