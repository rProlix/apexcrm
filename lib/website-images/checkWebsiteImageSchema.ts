...
const REQUIRED_COLS: Record<string, string[]> = {
  ...
  website_image_jobs: ['id', 'tenant_id', 'plan_id', 'status', 'model', 'created_at'],
...
export async function checkWebsiteImageSchema(): Promise<WebsiteImageSchemaHealth> {
  ...
  try {
    ...
    if (!rpcErr && rpcData && typeof rpcData === 'object') {
      ...
      const anyMissingCols =
        plansMissingCols.length + jobsMissingCols.length + ssiMissingCols.length > 0
    ...
    if (rpcErr) {
      errors.push(
        `RPC check_website_image_schema not available: ${rpcErr.message}. Run migration 058.`
      )
    }
  } catch (e) {
  ...
  const [plansMissingCols, jobsMissingCols, ssiMissingCols] = await Promise.all([
    plansOk
      ? getMissingColumns('website_image_plans')
      : Promise.resolve(REQUIRED_COLS.website_image_plans),
    jobsOk
      ? getMissingColumns('website_image_jobs')
      : Promise.resolve(REQUIRED_COLS.website_image_jobs),
    ssiOk
      ? getMissingColumns('website_section_images')
      : Promise.resolve(REQUIRED_COLS.website_section_images),
  ])
