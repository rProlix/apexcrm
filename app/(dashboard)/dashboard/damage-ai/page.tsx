...
import { RecentInspectionsList } from '@/components/van-damage/RecentInspectionsList'
...
export default async function DamageAIPage({
  ...
  const [
    ...
    tables
      ...
      .select(
        'id, van_id, driver_profile_id, slack_user_id, driver_snapshot, title, status, review_status, image_count, damage_count, ai_summary, ai_confidence, metadata, slack_upload_at, created_at, updated_at, reviewed_at'
      )
    ...
    tables
      ...
      .select(
        'inspection_id, damage_type, normalized_damage_type, vehicle_area, canonical_region, severity, description, repair_recommendation, observation_type, damage_case_id, created_at'
      )
      .eq('tenant_id', scope.tenantId)
      .eq('business_id', scope.businessId)
      .limit(10000),
    tables
      ...
      .select('inspection_id, damage_case_id, observation_type, observed_at')
      .eq('tenant_id', scope.tenantId)
      .eq('business_id', scope.businessId)
      .limit(10000),
    tables
      ...
      .select(
        'id, latest_observed_inspection_id, first_detected_at, first_source_timestamp, last_observed_at, current_severity, lifecycle_status, needs_review, first_reporter_snapshot, latest_uploader_snapshot, metadata'
      )
      .eq('tenant_id', scope.tenantId)
      .eq('business_id', scope.businessId)
      .limit(5000),
  ...
  for (const raw of itemResult.data ?? []) {
    ...
    if (inspectionId)
      itemsByInspection.set(inspectionId, [...(itemsByInspection.get(inspectionId) ?? []), item])
  }
  ...
  for (const raw of observationResult.data ?? []) {
    ...
    if (inspectionId)
      observationsByInspection.set(inspectionId, [
        ...(observationsByInspection.get(inspectionId) ?? []),
        observation,
      ])
  ...
  for (const raw of imageResult.data ?? []) {
    ...
    if (
      inspectionId &&
      !latestImageByInspection.has(inspectionId) &&
      ['uploaded', 'analyzed'].includes(text(image.status))
    )
      latestImageByInspection.set(inspectionId, text(image.id))
  }
  ...
  const allRows: InspectionSearchRow[] = (inspectionResult.data ?? []).map((raw) => {
    ...
    const driver = asRecord(
      inspection.driver_snapshot ?? metadata.driver ?? metadata.driverSnapshot
    )
    ...
    const linkedCases = linkedCaseIds
      .map((caseId) => cases.get(caseId))
      .filter((entry): entry is Record<string, unknown> => Boolean(entry))
    const repairStatuses = stringList(
      linkedCases.map((damageCase) => {
        const caseMetadata = asRecord(damageCase.metadata)
        return text(
          caseMetadata.repairStatus,
          text(caseMetadata.repair_status, text(damageCase.lifecycle_status))
        )
      })
    )
    const latestDamageAt =
      linkedCases
        .map((damageCase) =>
          text(damageCase.first_source_timestamp, text(damageCase.first_detected_at))
        )
        .filter(Boolean)
        .sort()
        .at(-1) ??
      items
        .map((item) => text(item.created_at))
        .sort()
        .at(-1) ??
      null
    const firstDamageAt =
    ...
    const latestObservationAt =
      linkedCases
        .map((damageCase) => text(damageCase.last_observed_at))
        .filter(Boolean)
        .sort()
        .at(-1) ??
      observations
        .map((observation) => text(observation.observed_at))
        .filter(Boolean)
        .sort()
        .at(-1) ??
      null
    const snapshotId = (snapshot: Record<string, unknown>) =>
      text(snapshot.driverProfileId, text(snapshot.slackUserId, text(snapshot.slack_user_id)))
    ...
    const inspectionNumber = text(
      metadata.inspectionNumber,
      text(metadata.inspection_number, `INS-${id.slice(0, 8).toUpperCase()}`)
    )
    const driverId = text(inspection.driver_profile_id, text(inspection.slack_user_id)) || null
    const severities = stringList([
    ...
    return {
      ...
      aiConfidence:
        typeof inspection.ai_confidence === 'number'
          ? inspection.ai_confidence
          : Number(inspection.ai_confidence) || null,
      ...
      firstDamageAt,
      latestObservationAt,
      observationCount: linkedCaseIds.reduce(
        (total, caseId) => total + (observationCountByCase.get(caseId) ?? 0),
        0
      ),
      newDamageCount: Math.max(
        observations.filter((observation) => text(observation.observation_type) === 'new_damage')
          .length,
        items.filter((item) => text(item.observation_type) === 'new_damage').length
      ),
      existingDamageCount: Math.max(
        observations.filter(
          (observation) => text(observation.observation_type) === 'existing_damage_observed'
        ).length,
        items.filter((item) => text(item.observation_type) === 'existing_damage_observed').length
      ),
      ...
      inspectionNumber,
      damageTypes: stringList(
        items.map((item) => text(item.normalized_damage_type, text(item.damage_type)))
      ),
      regions: stringList(
        items.map((item) => text(item.canonical_region, text(item.vehicle_area)))
      ),
      severities,
      observationTypes: stringList([
        ...items.map((item) => text(item.observation_type)),
        ...observations.map((observation) => text(observation.observation_type)),
      ]),
      repairStatuses,
      notes: stringList([
        ...items.map((item) => text(item.description)),
        ...items.map((item) => text(item.repair_recommendation)),
        text(metadata.notes),
      ]),
  ...
  const filters = {
    ...
    period:
      value(query, 'period') === 'SOD' || value(query, 'period') === 'EOD'
        ? (value(query, 'period') as 'SOD' | 'EOD')
        : ('all' as const),
    damageState: ([
      'new_damage',
      'existing_damage',
    ...
    ].includes(value(query, 'damageState'))
      ? value(query, 'damageState')
      : 'all') as DamageStateFilter,
    review: (['needs_review', 'ai_reviewed', 'human_reviewed'].includes(value(query, 'review'))
      ? value(query, 'review')
      : 'all') as InspectionReviewFilter,
    images: (['has_images', 'no_images'].includes(value(query, 'images'))
      ? value(query, 'images')
      : 'all') as InspectionImageFilter,
  ...
  const driverOptions = [
    ...new Map(
      allRows
        ...
        .map((row) => [row.driverId!, { value: row.driverId!, label: row.driverName }])
    ).values(),
  ].sort((a, b) => a.label.localeCompare(b.label))
  const vanOptions = [
    ...new Map(
      allRows
        .filter((row) => row.vanId)
        .map((row) => [
          row.vanId!,
          { value: row.vanId!, label: row.vanNumber ? `Van ${row.vanNumber}` : row.vanName },
        ])
    ).values(),
  ].sort((a, b) => a.label.localeCompare(b.label, undefined, { numeric: true }))
  const firstReporterOptions = [
    ...new Map(
      allRows.flatMap((row) =>
        row.firstReporterIds.map(
          (id, index) => [id, { value: id, label: row.firstReporterNames[index] ?? id }] as const
        )
      )
    ).values(),
  ].sort((a, b) => a.label.localeCompare(b.label))
  const latestUploaderOptions = [
    ...new Map(
      allRows.flatMap((row) =>
        row.latestUploaderIds.map(
          (id, index) => [id, { value: id, label: row.latestUploaderNames[index] ?? id }] as const
        )
      )
    ).values(),
  ].sort((a, b) => a.label.localeCompare(b.label))
  ...
  Object.entries(query).forEach(([key, raw]) => {
    ...
    if (entry) returnParams.set(key, entry)
  })
  if (!returnParams.has('businessId')) returnParams.set('businessId', scope.businessId)
  ...
  return (
    <div className="space-y-7">
      ...
      <InspectionSearchControls
        ...
        repairStatuses={[
          ...new Set([
            ...
            'in_repair',
            'repaired',
            ...uniqueOptions(allRows, 'repairStatuses'),
          ]),
        ]}
      ...
      <RecentInspectionsList
        ...
        error={Boolean(inspectionResult.error)}
        pagination={
          pageCount > 1 ? (
            <nav
              aria-label="Inspection result pages"
              className="flex items-center justify-between border-t border-white/8 px-5 py-4"
            >
              <PaginationLink query={query} page={page - 1} disabled={page <= 1}>
                Previous
              </PaginationLink>
              <span className="text-xs text-white/35">
                {(page - 1) * pageSize + 1}–{Math.min(page * pageSize, results.length)} of{' '}
                {results.length}
              </span>
              <PaginationLink query={query} page={page + 1} disabled={page >= pageCount}>
                Next
              </PaginationLink>
            </nav>
          ) : null
...
function PaginationLink({
  ...
  if (disabled)
    return (
      <span
        aria-disabled="true"
        className="rounded-lg border border-white/5 px-3 py-2 text-xs text-white/20"
      >
        {children}
      </span>
    )
  ...
  return (
    <Link
      href={`/dashboard/damage-ai?${params.toString()}`}
