...
export default async function CustomerRewardDetailPage({
  params,
}: {
  ...
  const item = itemRes.data as unknown as unknown as RewardShopItem & {
    products?: { name: string; description: string | null; price: number; currency: string } | null
  }
      ...
      <div className="premium-panel premium-border rounded-2xl overflow-hidden border-gold-500/20">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-lg font-bold tabular-nums ${canAfford ? 'text-emerald-400' : 'text-red-400'}`}
            >
              {balance.points_balance.toLocaleString()} pts
            </span>
          </div>

          {canAfford && !outOfStock && <div className="h-px bg-white/6" />}
          ...
          <div className="text-xs text-white/30 space-y-1">
            <div className="flex justify-between">
              ...
              <span className="text-white/50 capitalize">
                {item.redemption_type.replace('_', ' ')}
              </span>
            </div>
              ...
              <span className={outOfStock ? 'text-red-400' : 'text-white/50'}>
                {outOfStock
                  ...
                  : item.inventory_count === 0
                    ? 'Unlimited'
                    : `${item.inventory_count} remaining`}
              </span>
            </div>
