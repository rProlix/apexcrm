...
function bgVariantToValue(variant: BackgroundVariant, ds: WebsiteDesignSystem): string {
  switch (variant) {
    ...
    case 'surface_alt':
      return ds.palette.surfaceAlt
    case 'primary_wash':
      return ds.palette.primary
    case 'gradient_soft':
      return ds.gradients.sectionSoft
    case 'accent_wash':
      return ds.gradients.accentWash
    default:
      return ds.palette.background
  }
}
...
function chooseDivider(ctx: DividerContext, default_: DividerStyle): DividerStyle {
  ...
  return sections.map((section, i) => {
    ...
    if (role === 'hero') {
      ...
      const design: SectionDesign = {
        ...
        dividerBottom:
          dividerStyle !== 'none'
            ? chooseDivider({ from: 'gradient_soft', to: 'base', flowStyle }, dividerStyle)
            : 'none',
        cardStyle: 'none',
        spacing: 'luxury',
        layoutVariant: 'hero',
    ...
    if (role === 'cta') {
      ...
      const design: SectionDesign = {
        ...
        dividerTop:
          dividerStyle !== 'none'
            ? chooseDivider(
                { from: assigned[i - 1] ?? 'base', to: 'primary_wash', flowStyle },
                dividerStyle
              )
            : 'none',
        dividerBottom: 'none',
    ...
    const design: SectionDesign = {
      ...
      overlay: { enabled: false, type: 'gradient', value: '', opacity: 0 },
      dividerTop:
        dividerStyle !== 'none' && prevVariant !== bgVariant
...
function getCardStyle(role: string, designSystem: WebsiteDesignSystem): SectionDesign['cardStyle'] {
