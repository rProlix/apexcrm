...
import { SignupModuleConfigurator } from '@/components/onboarding/SignupModuleConfigurator'
...
const INITIAL: WizardState = {
  ...
  needsEmailReminders: false,
  needsAiBuilder: false,
  needsAiImages: false,
  needs360Products: false,
  ...
  recommendedModules: [],
  selectedModules: [],
  selectedPackageKey: 'custom-foundation',
  selectedPackageName: 'Business Foundation',
  selectedPackageDescription: 'A flexible starting point you can tailor module by module.',
...
function getSelectedNeedKeys(state: WizardState): string[] {
  return [
    state.needsAppointments && 'appointments',
    state.needsPayments && 'payments',
    state.needsWebsite && 'website',
    state.needsStore && 'store',
    state.needsCustomers && 'customers',
    state.needsRewards && 'rewards',
    state.needs360Products && '360',
    ...
    try {
      const res = await fetch('/api/onboarding/business/recommend', {
        ...
        body: JSON.stringify({
          ...
          needsCustomers: state.needsCustomers,
      ...
      if (data.success) {
        const preset =
          data.subscription?.package ?? getSignupPackageForBusinessType(state.businessCategory)
        const selectedModules =
          data.subscription?.selected_modules ??
          getRecommendedSignupModules({
            businessType: state.businessCategory,
            needs: getSelectedNeedKeys(state),
            sellsProducts: state.sellsProducts,
            offersServices: state.sellsServices,
          })
        update({
          recommendationReason:
            data.subscription?.recommendation_reason ?? data.recommended.recommendation_reason,
          recommendedModules: selectedModules,
          selectedModules,
          selectedPackageKey: preset.key,
          selectedPackageName: preset.name,
          selectedPackageDescription: preset.description,
        })
        setStep(6)
      } else {
      ...
      <div className="min-h-dvh bg-graphite-950 flex items-center justify-center px-6 py-12">
        <div className="glass-surface premium-border noise-overlay p-8 shadow-panel-lg text-center max-w-md w-full">
          ...
          <p className="text-sm text-white/50 mb-6 leading-relaxed">
            ...
            <span className="text-white/80 font-medium">{state.email}</span>. Click the link to
    ...
    <div className="min-h-dvh bg-graphite-950 flex flex-col">
      ...
      <div className="flex-1 px-6 pb-6 overflow-y-auto">
        <div className="max-w-2xl mx-auto">
          ...
          {step === 7 && (
            <Step7Confirm state={state} update={update} subdomainPreview={subdomainPreview} />
          )}
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-3 gap-2">
          {BUSINESS_CATEGORIES.map((cat) => (
            <button
              ...
              type="button"
              onClick={() => {
                const preset = getSignupPackageForBusinessType(cat.key)
                const modules = getRecommendedSignupModules({
                  businessType: cat.key,
                })
                update({
        ...
        {state.businessCategory && (
          <div className="mt-3 rounded-xl border border-gold-500/20 bg-gold-500/[0.06] px-3.5 py-3">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold text-white">{state.selectedPackageName}</p>
                <p className="mt-1 text-[11px] leading-relaxed text-white/40">
                  {state.selectedModules.length} recommended modules, fully customizable before
                  signup.
                </p>
              </div>
              <p className="shrink-0 text-xs font-semibold text-gold-300">
                {formatSignupPrice(
                  calculateSignupQuote(state.selectedModules, state.billingInterval)
                    .monthlyPriceCents
                )}
                /mo
              </p>
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center overflow-hidden rounded-xl border border-graphite-600 bg-graphite-800 focus-within:border-gold-500/50 transition-colors">
          <span className="shrink-0 px-3 text-xs text-white/30 border-r border-graphite-600">
            crm.app/
          </span>
          <input
            id="subdomain"
            ...
            onChange={(e) =>
              update({ desiredSubdomain: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') })
            }
        ...
        {subdomainPreview && (
          <p className="text-xs text-white/30">
            Your workspace: <span className="font-mono text-white/50">{subdomainPreview}</span>
          </p>
        )}
      </div>
...
const NEEDS_OPTIONS: Array<{
  ...
  },
  {
    ...
    icon: '💳',
    description: 'Stripe or Square integration',
  },
  ...
  },
  {
    ...
    icon: '⭐',
    description: 'Points & loyalty programs',
  },
  {
    key: 'needsStaff',
    label: 'Staff Accounts',
    icon: '👔',
    description: 'Employees & professionals',
  },
  {
    key: 'needsCustomerPortal',
    label: 'Customer Portal',
    icon: '🪪',
    description: 'Self-service for clients',
  },
  {
    key: 'needsEmailReminders',
    label: 'Email & SMS',
    icon: '📧',
  ...
  },
  {
    ...
    icon: '✨',
    description: 'Generate pages with AI',
    isPremium: true,
  },
  {
    key: 'needsAiImages',
  ...
  {
    key: 'needs360Products',
    label: '360 Product Studio',
    icon: '🔄',
    description: 'Interactive product spins',
    isPremium: true,
  },
  {
    key: 'needsAnalytics',
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-white/40 mt-1">
          Select all that apply. We&apos;ll refine your module package.
        </p>
      </div>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {NEEDS_OPTIONS.map((opt) => {
          ...
          return (
            ...
            >
              {opt.isPremium && (
                <span className="absolute top-2 right-2 text-[10px] text-purple-400 font-medium">
                  Pro
                </span>
              )}
              <span className="text-lg">{opt.icon}</span>
              <span
                className={cn('text-xs font-semibold', selected ? 'text-white' : 'text-white/70')}
              >
                {opt.label}
              </span>
              ...
              {selected && (
                <div className="absolute top-2 left-2 h-4 w-4 rounded-full bg-gold-500 flex items-center justify-center">
                  <svg
                    className="h-2.5 w-2.5 text-graphite-900"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={3}
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-white/40 mt-1">
          This helps us tailor your workspace recommendation.
        </p>
      </div>
...
function Step6PlanSelection({
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold text-white">Build your subscription</h2>
        <p className="mt-1 text-sm text-white/40">
          Start with our recommendation, then add or remove anything you do not need.
        </p>
      ...
      <SignupModuleConfigurator
        packageName={state.selectedPackageName}
        packageDescription={state.selectedPackageDescription}
        selectedModules={state.selectedModules}
        recommendedModules={state.recommendedModules}
        billingInterval={state.billingInterval}
        onSelectionChange={(selectedModules) => update({ selectedModules })}
        onBillingIntervalChange={(billingInterval) => update({ billingInterval })}
      />
    </div>
  )
}
...
function Step7Confirm({
  ...
  const quote = calculateSignupQuote(state.selectedModules, state.billingInterval)
  const enabledCount = state.selectedModules.length
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        ...
        <SummaryRow
          label="Subscription"
          value={`${state.selectedPackageName} · ${formatSignupPrice(quote.monthlyPriceCents)}/mo`}
          icon="💼"
        />
      ...
      <div className="rounded-xl bg-graphite-800/40 border border-graphite-700 p-4">
        ...
        {state.selectedModules.length > 0 ? (
          ...
          <p className="text-xs text-white/35">
            No add-on modules selected. Your core workspace remains available.
          </p>
        )}
      </div>
