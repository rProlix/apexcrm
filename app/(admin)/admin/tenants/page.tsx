...
export default async function AdminTenantsPage() {
  ...
  const [
    ...
    supabase
      .from('tenants')
      .select(
        'id, name, slug, subdomain, custom_domain, status, created_at, subscriptions(status, current_period_end, plan_id)'
      )
      .order('created_at', { ascending: false }),
    supabase.from('tenant_modules').select('tenant_id, module_key, enabled'),
    supabase.from('users').select('tenant_id').not('tenant_id', 'is', null),
          ...
          return (
            <Card key={t.id} className="!p-0 overflow-hidden">
              ...
              <div className="flex flex-wrap items-center gap-6 px-5 py-3 border-b border-white/4 bg-white/[0.01]">
                ...
                {sub && (
                  <span
                    className={`text-xs px-2 py-0.5 rounded-lg border ml-auto ${
                      sub.status === 'active'
