...
export class VanDamageSqsConsumer {
  ...
  constructor(
    ...
    private processor: (body: string, metadata?: JobRuntimeMetadata) => Promise<ProcessResult> = (
      body,
      metadata
    ) => processMessageBody(body, undefined, metadata)
  ) {
  ...
  async run() {
    logger.info('Van Damage worker started', {
    ...
    while (!this.stopping) {
      try {
        ...
        for (let index = 0; index < messages.length; index += this.config.concurrency) {
          await Promise.all(
            messages
              .slice(index, index + this.config.concurrency)
              .map((message) => this.handleMessage(message))
          )
        }
      } catch (error) {
        logger.error('SQS receive loop failed', {
  ...
  private async handleMessage(message: Message) {
    ...
    const heartbeat = setInterval(() => {
      void this.client
        ...
        .catch((error) =>
          logger.warn('Visibility heartbeat failed', {
            messageId: message.MessageId,
            error: error instanceof Error ? error.message : String(error),
          })
        )
