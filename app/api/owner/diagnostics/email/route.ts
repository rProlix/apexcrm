...
export async function GET() {
  ...
  for (const tableName of ['email_logs']) {
    try {
      const { data: failed } = await supabase
        ...
        .select(
          'id, to_email, subject, status, provider, category, error_message, from_email, created_at'
        )
        .eq('status', 'failed')
      ...
      const { data: sent } = await supabase
        ...
        .from(tableName as any)
        .select(
          'id, to_email, subject, status, provider, category, message_id, from_email, created_at'
        )
        .eq('status', 'sent')
        .order('created_at', { ascending: false })
        .limit(20)
  ...
  if (!status.resendConfigured && !status.sesConfigured) {
    suggestions.push(
      'Add RESEND_API_KEY to your Vercel environment variables (Settings → Environment Variables).'
    )
  }
