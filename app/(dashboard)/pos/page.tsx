...
export default async function POSPage() {
  ...
  const [
    ...
    supabase
      ...
      .select(
        `id,name,description,selection_type,min_required,max_allowed,is_required,applies_to_all_products,sort_order,pos_modifiers(id,name,modifier_type,price_delta_cents,is_default,sort_order,status)`
      )
      .eq('tenant_id', tenantId)
      .eq('status', 'active')
      .order('sort_order'),
  ...
  const enrichedProducts = (products ?? []).map((p: Record<string, unknown>) => {
    ...
    const allGroups = [
      ...
      ...globalGroups.filter(
        (g: { id: string }) =>
          !productGroups.some((pg: Record<string, unknown> | undefined) => pg && pg.id === g.id)
      ),
    ]
