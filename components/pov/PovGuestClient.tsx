...
interface PublicEvent {
  ...
  upload_success_message: string | null
  unlocked: boolean
}
interface GuestInfo {
  id: string
...
const PALETTES: Record<
  string,
  { bg: string; panel: string; accent: string; text: string; sub: string }
> = {
  disposable: {
    bg: '#0c0c0d',
...
export function PovGuestClient({
  eventSlug,
  initialView,
}: {
  eventSlug: string
  initialView: 'auto' | 'capture' | 'gallery'
}) {
  ...
  const refresh = useCallback(async () => {
    const [evRes, meRes] = await Promise.all([
      fetch(`/api/pov/events/${eventSlug}`)
        .then((r) => r.json())
        .catch(() => null),
      fetch(`/api/pov/events/${eventSlug}/me`)
        .then((r) => r.json())
        .catch(() => null),
    ])
  ...
  if (!ready) {
    return (
      <Shell pal={pal}>
        <p style={{ color: pal.sub }}>Loading…</p>
      </Shell>
    )
  }
  if (!event) {
    return (
      <Shell pal={pal}>
        <p style={{ color: pal.sub }}>This event was not found.</p>
      </Shell>
    )
  }
    ...
    <Shell pal={pal}>
      <div style={{ width: '100%', maxWidth: 460, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 800, color: pal.text, margin: '8px 0 4px' }}>
            {event.name}
          </h1>
          {event.event_date && <p style={{ color: pal.sub, fontSize: 14 }}>{event.event_date}</p>}
        </div>

        {guest && <Tabs pal={pal} view={view} setView={setView} />}
        ...
        {view === 'capture' && guest && <CaptureView event={event} pal={pal} />}
        {view === 'gallery' && <GalleryView eventSlug={eventSlug} pal={pal} />}
        ...
        {guest && (
          <button
            onClick={async () => {
              await fetch(`/api/pov/events/${eventSlug}/logout`, { method: 'POST' })
              void refresh()
            }}
            style={{
              display: 'block',
    ...
    try {
      ...
      if (mediaType === 'video' || mediaType === 'audio') {
        ...
        if (duration != null && duration > max + 2.5) {
          throw new Error(
            `Please keep it to ${max} seconds or less (yours is ${Math.round(duration)}s).`
          )
        }
      }
      ...
      const res = await fetch(`/api/pov/events/${event.slug}/media/upload`, {
        method: 'POST',
        body: fd,
      })
      const json = await res.json()
  ...
  return (
    <Panel pal={pal}>
      ...
      <input
        ...
        capture="environment"
        hidden
        onChange={() => onPick(photoRef, 'photo')}
      />
      <input
        ref={videoRef}
        type="file"
        ...
        capture="environment"
        hidden
        onChange={() => onPick(videoRef, 'video')}
      />
      <input
        ref={audioFileRef}
        type="file"
        accept="audio/*"
        hidden
        onChange={() => onPick(audioFileRef, 'audio')}
      />

      <label style={{ display: 'block', fontSize: 12, color: pal.sub, marginBottom: 6 }}>
        Caption (optional)
      </label>
      <input
        value={caption}
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        ...
        {event.allow_videos && (
          <BigBtn
            ...
            disabled={busy}
            onClick={() =>
              mediaRecorderSupported ? setShowVideoRecorder(true) : videoRef.current?.click()
            }
            emoji="🎬"
        ...
        {event.allow_audio &&
          (recording ? (
            ...
            <BigBtn
              ...
              disabled={busy}
              onClick={() =>
                mediaRecorderSupported ? startRecording() : audioFileRef.current?.click()
              }
              emoji="🎙️"
              title={`Record a ${event.audio_max_seconds}s audio message`}
            />
          ))}
      ...
      {busy && (
        <p style={{ textAlign: 'center', color: pal.sub, fontSize: 13, marginTop: 14 }}>
          Uploading…
        </p>
      )}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 10, width: '100%', maxWidth: 460 }}>
        {phase === 'live' && (
          <>
            <button onClick={startRecording} disabled={!!err} style={recBtn('#ef4444')}>
              ● Record
            </button>
            <button onClick={cancel} style={recBtn('rgba(255,255,255,0.12)')}>
              Cancel
            </button>
          </>
        )}
        {phase === 'recording' && (
          <button onClick={stopRecording} style={recBtn('#ef4444')}>
            ⏹ Stop
          </button>
        )}
        {phase === 'recorded' && (
          <>
            <button onClick={confirmUpload} style={recBtn(pal.accent, '#111')}>
              Upload clip
            </button>
            <button onClick={retake} style={recBtn('rgba(255,255,255,0.12)')}>
              Retake
            </button>
            <button onClick={cancel} style={recBtn('rgba(255,255,255,0.12)')}>
              Cancel
...
function GalleryView({ eventSlug, pal }: { eventSlug: string; pal: ReturnType<typeof palette> }) {
  ...
  useEffect(() => {
    fetch(`/api/pov/events/${eventSlug}/media`)
      .then((r) => r.json())
      .then((j) => {
  ...
  return (
    <div>
        ...
        {(['all', 'photo', 'video', 'audio'] as const).map((f) => (
          <button
            ...
            style={{
              ...
              borderRadius: 999,
              border: '1px solid rgba(255,255,255,0.12)',
              background: filter === f ? pal.accent : 'transparent',
              color: filter === f ? '#111' : pal.sub,
              fontSize: 12,
              fontWeight: 600,
              cursor: 'pointer',
      ...
      {items.length === 0 ? (
        <p style={{ textAlign: 'center', color: pal.sub, fontSize: 13, padding: '24px 0' }}>
            ...
            >
              ...
              {(m.caption || m.guest_name) && (
                <div style={{ padding: '6px 8px' }}>
                  ...
                  {m.guest_name && (
                    <p style={{ fontSize: 10, color: pal.sub, margin: 0 }}>{m.guest_name}</p>
                  )}
                </div>
              )}
            </div>
...
function Countdown({ ms, pal }: { ms: number; pal: ReturnType<typeof palette> }) {
  ...
  const cell = (v: number, label: string) => (
    <div style={{ textAlign: 'center' }}>
      <div style={{ fontSize: 26, fontWeight: 800, color: pal.text }}>
        {String(v).padStart(2, '0')}
      </div>
      <div style={{ fontSize: 10, color: pal.sub, textTransform: 'uppercase', letterSpacing: 1 }}>
        {label}
      </div>
    </div>
  )
