    ...
    <>
      ...
      >
        ...
        >
          ...
          {step === 'preview' && restylePlan && (
            <PreviewPanel plan={restylePlan} onApply={handleApply} onBack={() => setStep('form')} />
          )}
          {step === 'applying' && <LoadingPanel message={applyingMsg} isApplying />}
          {step === 'done' && applyResult && (
            <DonePanel
              result={applyResult}
              onClose={() => setRestyleDrawer(false)}
              onNewRestyle={() => setStep('form')}
            />
          )}
          {step === 'error' && (
            <ErrorPanel
...
function FormPanel({
  ...
  return (
    <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      >
        <strong style={{ color: '#c7d2fe' }}>AI Restyle</strong> transforms your website's visual
        design while keeping all your existing content, sections, reviews, products, and business
        information.
      </div>
      ...
      <Section label="Redesign Intensity">
          ...
          {(['subtle', 'balanced', 'cinematic'] as const).map((v) => (
            <button
              ...
              style={{
                ...
                padding: '0.5rem',
                borderRadius: 8,
                border: `1px solid ${intensity === v ? '#6366f1' : '#3f3f46'}`,
        ...
        <p style={{ fontSize: '0.6875rem', color: '#52525b', margin: '0.375rem 0 0' }}>
          ...
          {intensity === 'balanced' &&
            'Clear improvement. Updated palette, backgrounds, and typography.'}
          {intensity === 'cinematic' &&
            'Dramatic full redesign. Premium sections, bold palette, cinematic feel.'}
        </p>
      </Section>
      ...
      <Section label="Options">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
          <Toggle
            label="Preserve existing images"
            value={preserveImages}
          ...
          />
          <Toggle
            label="Apply section animations"
            value={applyAnimations}
            onChange={setApplyAnimations}
          />
      ...
      <div style={{ fontSize: '0.6875rem', color: '#52525b', lineHeight: 1.5 }}>
        Content preservation is always ON. AI Restyle will not delete sections, reviews, products,
        FAQs, or any business data. A version checkpoint is saved before applying so you can undo
        anytime.
      </div>
...
function PreviewPanel({
  ...
  return (
    <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      >
        <div style={{ fontWeight: 700, color: '#4ade80', marginBottom: '0.375rem' }}>
          Restyle Preview Ready
        </div>
        {plan.summary}
      </div>
            ...
            {[
              ...
              >
                <div
                  ...
                  title={color}
                />
                <span style={{ fontSize: '0.625rem', color: '#52525b', textAlign: 'center' }}>
                  {label}
                </span>
              </div>
            ))}
          ...
          <div style={{ fontSize: '0.75rem', color: '#a1a1aa', lineHeight: 1.6 }}>
            <div>
              ...
              <span style={{ color: '#f4f4f5', fontWeight: 600 }}>
                {plan.designSystem.typography.headingFontStack?.split(',')[0] ?? 'System'}
              </span>
            </div>
            <div>
              Body:{' '}
              <span style={{ color: '#f4f4f5' }}>
                {plan.designSystem.typography.bodyFontStack?.split(',')[0] ?? 'System'}
              </span>
            </div>
            <div>
              Category:{' '}
              <span style={{ color: '#f4f4f5', textTransform: 'capitalize' }}>
                {plan.designSystem.typography.headingFontCategory}
              </span>
            </div>
          </div>
        </Section>
        ...
        <Section label="Fixes Applied">
          <div style={{ fontSize: '0.75rem', color: '#a1a1aa' }}>
            {plan.contrastFixes.length > 0 && (
              <div>
                ✓ {plan.contrastFixes.length} contrast issue
                {plan.contrastFixes.length !== 1 ? 's' : ''} corrected
              </div>
            )}
            {plan.mobileFixes.length > 0 && (
              <div>
                ✓ {plan.mobileFixes.length} mobile layout fix
                {plan.mobileFixes.length !== 1 ? 'es' : ''} applied
              </div>
            )}
          </div>
        </Section>
      ...
      {/* Info */}
      <div style={{ fontSize: '0.6875rem', color: '#52525b', lineHeight: 1.5 }}>
        A version checkpoint will be saved before applying. You can always restore the previous
        design from Version History.
      </div>
  ...
  return (
    <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      >
        ...
        <div
          style={{ fontSize: '1rem', fontWeight: 700, color: '#4ade80', marginBottom: '0.25rem' }}
        >
          Restyle Applied!
        </div>
