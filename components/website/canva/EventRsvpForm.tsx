...
export function EventRsvpForm({ eventSlug, title, theme, cameraHref, galleryHref }: Props) {
  ...
  if (done) {
    return (
      ...
      >
        ...
        >
          {cameraHref && (
            <a href={cameraHref} style={btnStyle(colors)}>
              Open Event Camera
            </a>
          )}
          {galleryHref && (
            <a href={galleryHref} style={btnStyle(colors)}>
              View Gallery
            </a>
          )}
          <a href={`/events/${eventSlug}`} style={btnStyle(colors, true)}>
            Back to event
          </a>
        </div>
      </div>
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: 'clamp(1.5rem,4vw,2rem)', fontWeight: 700, textAlign: 'center' }}>
        {title ?? 'RSVP'}
      </h1>
      <p style={{ textAlign: 'center', opacity: 0.75, marginTop: '0.5rem' }}>
      ...
      <form onSubmit={submit} style={{ marginTop: '1.5rem', display: 'grid', gap: '1rem' }}>
        ...
        <label style={labelStyle}>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginTop: 6 }}>
            <button
              ...
              style={pill(attending === true, colors)}
            >
              Yes
            </button>
            <button
              ...
              style={pill(attending === false, colors)}
            >
              No
            </button>
          </div>
        ...
        <Field
          ...
          onChange={(v) => setGuestCount(Math.max(1, parseInt(v, 10) || 1))}
          type="number"
        />
        <label style={labelStyle}>
          Message
...
function btnStyle(colors: Record<string, string>, outline = false): React.CSSProperties {
  return {
    ...
    background: outline
      ? 'transparent'
      : `linear-gradient(135deg,${colors.primary ?? '#7c3aed'},${colors.accent ?? '#db2777'})`,
...
function pill(active: boolean, colors: Record<string, string>): React.CSSProperties {
  return {
    ...
    cursor: 'pointer',
    fontWeight: 600,
    background: active ? (colors.primary ?? '#7c3aed') : 'rgba(255,255,255,0.1)',
    color: '#fff',
  }
}
