...
export function SlackSettingsClient({
  ...
  useEffect(() => {
    ...
    fetch(`/api/integrations/slack/channels?businessId=${encodeURIComponent(businessId)}`, {
      ...
      .then(async (response) => {
        ...
        setChannels(list)
        setSelected(
          new Set(list.filter((channel) => channel.selected).map((channel) => channel.id))
        )
  ...
  async function saveChannels() {
    ...
    const response = await fetch('/api/integrations/slack/channels', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ businessId, channelIds: [...selected], maintenanceChannelId }),
    })
    const result = (await response.json()) as { error?: string }
    setMessage(
      response.ok
        ? `${selected.size} inspection channel${selected.size === 1 ? '' : 's'} and ${maintenanceChannelId ? '1 maintenance channel' : 'no maintenance channel'} enabled.`
    ...
    <div className="space-y-6">
      ...
      {integration.connected && (
        <section className="rounded-2xl border border-white/10 bg-graphite-800 p-6">
          ...
          <p className="mt-1 text-sm text-white/45">
            Only image messages in selected channels create inspections. No channels are selected
            automatically.
          </p>
          <div className="mt-5 max-h-80 space-y-2 overflow-y-auto pr-2">
            ...
            {channels.map((channel) => (
              <label
                key={channel.id}
                className="flex cursor-pointer items-center justify-between rounded-lg border border-white/8 px-3 py-2 hover:bg-white/[0.03]"
              >
                ...
                <input
                  type="checkbox"
                  disabled={!channel.isMember || maintenanceChannelId === channel.id}
          ...
          <div className="mt-7 border-t border-white/10 pt-6">
            <h2 className="font-semibold text-white">Fleet maintenance channel</h2>
            <p className="mt-1 text-sm text-white/45">
            ...
            <p className="mt-3 rounded-lg border border-amber-300/15 bg-amber-300/[0.06] px-3 py-2 text-xs text-amber-100/70">
              A channel cannot serve both purposes. Maintenance messages never create damage
              inspections or Gemini jobs.
            </p>
            <select
              value={maintenanceChannelId ?? ''}
              onChange={(event) => {
                const value = event.target.value || null
                setMaintenanceChannelId(value)
                if (value)
                  setSelected((current) => {
                    const next = new Set(current)
                    next.delete(value)
              ...
              className="mt-4 w-full rounded-lg border border-white/10 bg-graphite-900 px-3 py-2 text-sm text-white"
            >
              <option value="">No maintenance channel</option>
              {channels
