...
export async function applyPOSInventoryMovements(input: POSInventoryInput): Promise<{
  ...
  for (const orderItem of orderItems) {
    ...
    for (const recipe of recipes ?? []) {
      ...
      if (updateErr) {
        errors.push(`Recipe deduction failed: ${updateErr.message}`)
        continue
      }
    ...
    for (const link of links ?? []) {
      ...
      const alreadyHandled = (recipes ?? []).some(
        (r: { inventory_item_id: string }) => r.inventory_item_id === link.inventory_item_id
      )
  ...
  const { data: modifiers } = await supabase
    ...
    .in(
      'order_item_id',
      orderItems.map((i: { id: string }) => i.id)
    )
  ...
  if (applied > 0) {
    supabase.rpc('recalculate_inventory_alerts', { p_tenant_id: tenantId }).catch(() => {
      /* silent */
    })
  }
