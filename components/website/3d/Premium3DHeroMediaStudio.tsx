...
export function Premium3DHeroMediaStudio({ sectionId }: Props) {
  ...
  const [groups, setGroups] = useState<Website3DAssetGroups>({
    videos: [],
    imageSequences: [],
    posters: [],
    fallbacks: [],
  ...
  const refresh = useCallback(() => {
    if (!tenantId) return
    void getWebsite3DAssetGroups(tenantId, {})
      .then(setGroups)
      .catch(() => {})
  }, [tenantId])
  ...
  const patch = useCallback(
    (changes: Partial<Premium3DScrollHeroContent>) => {
      if (!section) return
  ...
  const patchScrub = useCallback(
    (changes: Partial<VideoScrubSettings>) => {
    ...
    async (files: FileList) => {
      ...
      try {
        ...
        for (let i = 0; i < sorted.length; i++) {
          setUploading({
            kind: 'image_sequence_frame',
            pct: Math.round(((i + 1) / sorted.length) * 90),
          })
  ...
  const activate = useCallback(
    async (asset: Website3DAsset, mode: Tab) => {
      setBusy(true)
      setNotice(null)
      try {
        const seqId =
          asset.sequence_id ?? (asset.metadata?.sequenceId as string | undefined) ?? null
        ...
        if (res?.contentPatch) {
          ...
          setNotice({
            level: 'ok',
            msg: 'Set as active hero media. Publish the website to show it publicly.',
          })
          refresh()
        } else {
  ...
  const rename = useCallback(
    async (asset: Website3DAsset) => {
      const name = window.prompt('Rename asset', asset.name)
      if (!name || name === asset.name) return
      setBusy(true)
      try {
        await updateWebsite3DAsset(asset.id, { name })
        refresh()
      } finally {
        setBusy(false)
      }
  ...
  const remove = useCallback(
    async (asset: Website3DAsset) => {
      ...
      if (
        isActive &&
        !window.confirm(
          'This asset is active on the hero. Remove it anyway? You should pick another asset and re-publish.'
        )
      )
        return
      setBusy(true)
      try {
        await archiveWebsite3DAsset(asset.id)
        refresh()
      } finally {
        setBusy(false)
      }
  ...
  const capturePoster = useCallback(
    async (videoEl: HTMLVideoElement | null) => {
      ...
      try {
        ...
        await doUpload(file, 'poster', (url, asset) =>
          patch({ posterUrl: url, posterAssetId: asset?.id ?? null })
        )
      } catch {
        setNotice({
          level: 'error',
          msg: 'Could not auto-generate a poster (CORS/codec). Upload a poster image instead.',
        })
      }
    },
  ...
  const activeKind = useCallback(
    (a: Website3DAsset): Tab | null => {
      const seqId = a.sequence_id ?? (a.metadata?.sequenceId as string | undefined) ?? a.id
      if (a.id === content.activeVideoAssetId) return 'video'
      if (
        seqId === content.activeImageSequenceAssetId ||
        a.id === content.activeImageSequenceAssetId
      )
        return 'image_sequence'
      if (a.id === content.posterAssetId || (a.public_url && a.public_url === content.posterUrl))
        return 'poster'
      if (
        a.id === content.fallbackAssetId ||
        (a.public_url && a.public_url === content.fallbackImageUrl)
      )
        return 'fallback'
  ...
  const warnings = useMemo(() => {
    ...
    if (scrub.mode === 'video') {
      ...
      if (v?.mime_type && v.mime_type !== 'video/mp4')
        list.push({
          level: 'warn',
          msg: 'Active video is not MP4 — H.264 MP4 is recommended for browser support.',
        })
      if (v?.file_size_bytes && v.file_size_bytes > VIDEO_MAX_MB * 1024 * 1024)
        list.push({
          level: 'warn',
          msg: `Active video is large (${fmtBytes(v.file_size_bytes)}). Keep it short and optimized.`,
        })
      if (v?.duration_seconds && v.duration_seconds > 30)
        list.push({
          level: 'warn',
          msg: 'Active video is long (>30s). Shorter clips scrub better.',
        })
    } else {
      ...
      if (n < 8)
        list.push({
          level: 'error',
          msg: 'Upload at least 8 image sequence frames before enabling image sequence scrub.',
        })
      else if (n < 40)
        list.push({
          level: 'warn',
          msg: `${n} frames — 40–150 frames is ideal for smooth scrubbing.`,
        })
      else if (n > 300)
        list.push({
          level: 'warn',
          msg: `${n} frames may use a lot of bandwidth. 40–150 is ideal.`,
        })
    }
    if (!content.posterUrl)
      list.push({
        level: 'warn',
        msg: 'Poster image recommended (loading / mobile / reduced motion).',
      })
    if (!content.fallbackImageUrl) list.push({ level: 'warn', msg: 'Fallback image recommended.' })
    if (!isPublished)
      list.push({
        level: 'info',
        msg: 'This media is saved in draft. Publish the website to show it publicly.',
      })
    return list
    ...
    <div>
        ...
        <Dropzone
          ...
          validate={(f) =>
            f.type === 'video/mp4' || f.name.toLowerCase().endsWith('.mp4')
              ? null
              : 'Must be an MP4 video.'
          }
          onFiles={(files) =>
            doUpload(files[0], 'video', (url, asset) => {
              patch({
                renderMode: 'video_scrub',
                useImageSequence: false,
                videoUrl: url,
                activeVideoAssetId: asset?.id ?? null,
      ...
      {tab === 'poster' && (
        <Dropzone
          ...
          onFiles={(files) =>
            doUpload(files[0], 'poster', (url, asset) =>
              patch({ posterUrl: url, posterAssetId: asset?.id ?? null })
            )
          }
        />
      ...
      {tab === 'fallback' && (
        <Dropzone
          ...
          onFiles={(files) =>
            doUpload(files[0], 'fallback', (url, asset) =>
      ...
      {notice && <Banner level={notice.level === 'ok' ? 'info' : 'error'}>{notice.msg}</Banner>}
      ...
      <LibrarySection
        ...
        assets={activeVideo ? [activeVideo] : []}
        activeKind={activeKind}
        busy={busy}
        actionLabel="Use as 3D Hero"
        onUse={(a) => activate(a, 'video')}
        onRename={rename}
        onRemove={remove}
        onCopy={copyUrl}
      />
      <LibrarySection
        ...
        assets={groups.videos.filter((a) => activeKind(a) !== 'video')}
        activeKind={activeKind}
        busy={busy}
        actionLabel="Use this video"
        onUse={(a) => activate(a, 'video')}
        onRename={rename}
        onRemove={remove}
        onCopy={copyUrl}
      />
      <LibrarySection
        title="Active image sequence"
        assets={activeSeq ? [activeSeq] : []}
        activeKind={activeKind}
        busy={busy}
        actionLabel="Use as 3D Hero"
        onUse={(a) => activate(a, 'image_sequence')}
        onRename={rename}
        onRemove={remove}
        onCopy={copyUrl}
      />
      <LibrarySection
        title="Available image sequences"
        assets={groups.imageSequences.filter((a) => activeKind(a) !== 'image_sequence')}
        activeKind={activeKind}
        busy={busy}
        actionLabel="Use this image sequence"
        onUse={(a) => activate(a, 'image_sequence')}
        onRename={rename}
        onRemove={remove}
        onCopy={copyUrl}
      ...
      <LibrarySection
        title="Poster images"
        assets={groups.posters}
        activeKind={activeKind}
        busy={busy}
        actionLabel="Use as Poster"
        onUse={(a) => activate(a, 'poster')}
        onRename={rename}
        onRemove={remove}
        onCopy={copyUrl}
      />
      <LibrarySection
        ...
        assets={groups.fallbacks}
        activeKind={activeKind}
        busy={busy}
        actionLabel="Use as Fallback"
        onUse={(a) => activate(a, 'fallback')}
        onRename={rename}
        onRemove={remove}
        onCopy={copyUrl}
      />
      {groups.videos.length +
        groups.imageSequences.length +
        groups.posters.length +
        groups.fallbacks.length ===
        0 && (
      ...
      >
        {SCRUB_PRESETS.map((p) => (
          <button
            ...
            disabled={busy}
            onClick={() => {
              patch(buildScrubPresetPatch(p.key, content))
              setTab(p.mode)
              setNotice({
                level: 'ok',
                msg: `Applied "${p.label}" preset (your media is preserved).`,
              })
            }}
            style={{
              textAlign: 'left',
      ...
      <div style={group}>
        <Toggle
          label="Pin hero while scrolling"
          value={scrub.pinOnScroll}
          onChange={(v) => {
            patchScrub({ pinOnScroll: v })
            patch({ pinOnScroll: v })
          }}
        />
      </div>
      <Range
        label="Scroll length (× viewport height)"
        min={1}
        max={6}
        step={0.5}
        value={content.scrollLength ?? 2.5}
        onChange={(v) => {
          patch({ scrollLength: v })
          patchScrub({ scrollLength: v })
        }}
      />
      <Range
        ...
        onChange={(v) => {
          patch({ scrubSmoothing: v })
          patchScrub({ scrubSmoothing: v })
        }}
      />
      <div style={group}>
        <label style={label}>Object fit</label>
        <Select
          value={scrub.objectFit}
          onChange={(v) => {
            patchScrub({ objectFit: v as 'cover' | 'contain' })
            patch({ videoObjectFit: v as 'cover' | 'contain' })
          }}
          options={[
      ...
      <Range
        ...
        max={1}
        step={0.05}
        value={content.overlayOpacity ?? 0.35}
        onChange={(v) => {
          patch({ overlayOpacity: v })
          patchScrub({ overlayOpacity: v })
        }}
      />
      <Range
        label="Background gradient strength"
        min={0}
        max={1}
        step={0.05}
        value={content.backgroundGradientStrength ?? 0.45}
        onChange={(v) => {
          patch({ backgroundGradientStrength: v })
          patchScrub({ backgroundGradientStrength: v })
        }}
      />
      <div style={group}>
        <label style={label}>Hero height</label>
        <Select
          value={content.heroHeight ?? '100vh'}
          onChange={(v) => {
            patch({ heroHeight: v as Premium3DScrollHeroContent['heroHeight'] })
            patchScrub({ heroHeight: v as VideoScrubSettings['heroHeight'] })
          }}
          options={[
            { value: '100vh', label: '100vh' },
            { value: '120vh', label: '120vh' },
            { value: '150vh', label: '150vh' },
      ...
      {content.heroHeight === 'custom' && (
        <div style={group}>
          ...
          <input
            style={inputBox}
            value={content.customHeroHeight ?? ''}
            onChange={(e) => {
              patch({ customHeroHeight: e.target.value })
              patchScrub({ customHeroHeight: e.target.value })
            }}
          />
        </div>
      )}
      <div style={group}>
        ...
        <Select
          ...
          onChange={(v) => {
            patch({ contentAlignment: v as Premium3DScrollHeroContent['contentAlignment'] })
            patchScrub({ contentAlignment: v as VideoScrubSettings['contentAlignment'] })
          }}
          options={[
      ...
      <div style={group}>
        ...
        <Select
          value={content.textRevealTiming ?? 'middle'}
          onChange={(v) => {
            patch({ textRevealTiming: v as Premium3DScrollHeroContent['textRevealTiming'] })
            patchScrub({ textRevealTiming: v as VideoScrubSettings['textRevealTiming'] })
          }}
          options={[
      ...
      {scrub.mode === 'video' && (
        <>
          ...
          <Range
            label="Start time (s, 0 = beginning)"
            min={0}
            max={60}
            step={0.5}
            value={scrub.startTime ?? 0}
            onChange={(v) => patchScrub({ startTime: v || null })}
          />
          <Range
            label="End time (s, 0 = full duration)"
            min={0}
            max={120}
            step={0.5}
            value={scrub.endTime ?? 0}
            onChange={(v) => patchScrub({ endTime: v || null })}
          />
        </>
      )}
      {scrub.mode === 'image_sequence' && (
        <>
          <Range
            label="FPS (image sequence metadata)"
            min={12}
            max={60}
            step={1}
            value={scrub.fps ?? 30}
            onChange={(v) => patchScrub({ fps: v })}
          />
          <div style={group}>
            <label style={label}>Frame order</label>
            <Select
              value={scrub.frameOrder ?? 'filename'}
          ...
          <div style={group}>
            ...
            <Select
              value={scrub.preloadStrategy ?? 'nearby'}
              onChange={(v) =>
                patchScrub({ preloadStrategy: v as VideoScrubSettings['preloadStrategy'] })
              }
              options={[
      ...
      <div style={group}>
        ...
        <Select
          value={scrub.mobileFallbackMode}
          onChange={(v) => {
            patchScrub({ mobileFallbackMode: v as VideoScrubSettings['mobileFallbackMode'] })
            patch({ mobileFallbackMode: v as Premium3DScrollHeroContent['mobileFallbackMode'] })
          }}
          options={[
      ...
      <div style={group}>
        ...
        <Select
          value={scrub.reducedMotionFallback}
          onChange={(v) => {
            patchScrub({ reducedMotionFallback: v as VideoScrubSettings['reducedMotionFallback'] })
            patch({
              reducedMotionFallback: v as Premium3DScrollHeroContent['reducedMotionFallback'],
            })
          }}
          options={[
      ...
      <div style={{ ...card, fontSize: '0.65rem', color: '#a1a1aa', lineHeight: 1.6 }}>
        <div style={{ color: '#c9a84c', marginBottom: '0.25rem' }}>
          Convert a video to an image sequence (FFmpeg):
        </div>
      ...
      {showDiag && (
        ...
        >
          ...
          {warnings
            ...
            .map((w, i) => (
              <div key={i} style={{ color: w.level === 'error' ? '#fca5a5' : '#fcd34d' }}>
                ! {w.msg}
              </div>
            ))}
        </div>
...
function Diag({ k, v }: { k: string; v: string }) {
  return (
    <div>
      <span style={{ color: '#52525b' }}>{k}:</span> <span style={{ color: '#d4d4d8' }}>{v}</span>
    </div>
  )
}
...
function Tabs({ tab, setTab }: { tab: Tab; setTab: (t: Tab) => void }) {
  const items: { id: Tab; label: string }[] = [
    ...
    { id: 'image_sequence', label: 'Image Sequence' },
    ...
    <div style={{ marginBottom: '0.75rem' }}>
      <div
        ...
        style={{
          ...
          border: `1.5px dashed ${over ? '#c9a84c' : '#3f3f46'}`,
          background: over ? '#c9a84c14' : '#101013',
          color: '#a1a1aa',
          fontSize: '0.75rem',
          textAlign: 'center',
        }}
...
function LibrarySection({
  ...
  activeKind: (a: Website3DAsset) => Tab | null
  busy: boolean
  actionLabel: string
  onUse: (a: Website3DAsset) => void
  onRename: (a: Website3DAsset) => void
  onRemove: (a: Website3DAsset) => void
  onCopy: (a: Website3DAsset) => void
}) {
  if (assets.length === 0) return null
  const badgeText: Record<Tab, string> = {
    video: 'Active Hero Video',
    image_sequence: 'Active Hero Image Sequence',
    poster: 'Active Poster',
    fallback: 'Active Fallback',
  }
  return (
    <div style={{ marginBottom: '0.875rem' }}>
      <div style={{ fontSize: '0.6875rem', color: '#71717a', marginBottom: '0.375rem' }}>
        {title}
      </div>
      {assets.map((a) => {
        ...
        const isImg =
          a.asset_type === 'poster' ||
          a.asset_type === 'fallback' ||
          a.asset_type === 'image_sequence'
        return (
          <div
            key={a.id}
            style={{
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div
                style={{ display: 'flex', gap: '0.25rem', marginTop: '0.35rem', flexWrap: 'wrap' }}
              >
                <MiniBtn primary disabled={busy} onClick={() => onUse(a)}>
                  {actionLabel}
                </MiniBtn>
                <MiniBtn disabled={busy} onClick={() => onRename(a)}>
                  Rename
                </MiniBtn>
                <MiniBtn disabled={busy} onClick={() => onCopy(a)}>
                  Copy URL
                </MiniBtn>
                {a.public_url && (
                  <a
                ...
                <MiniBtn disabled={busy} onClick={() => onRemove(a)}>
                  Remove
                </MiniBtn>
              </div>
            </div>
...
function MiniBtn({
  ...
  return (
    <button
      disabled={disabled}
      onClick={onClick}
      style={{ ...miniBtnStyle(!!primary), opacity: disabled ? 0.6 : 1 }}
    >
      {children}
    </button>
  )
...
function VideoPreviewTimeline({
  ...
  return (
    <div style={card}>
      ...
      >
        <span>{cur.toFixed(2)}s</span>
        <span>{dur.toFixed(2)}s</span>
      </div>
      <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap' }}>
        <MiniBtn primary onClick={testScrub}>
          Test Scroll Scrub
        </MiniBtn>
