    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="text-white/40 tabular-nums text-xs font-medium">{progress}%</span>
        </div>
      ...
      {sources && sources.length > 0 && (
        <div className="space-y-1.5">
          {sources.map((src) => (
            <div key={src.id} className="flex items-center gap-2.5 text-xs py-1">
              ...
              >
                {src.fetched_status === 'fetched'
                  ? 'Fetched'
                  : src.fetched_status === 'failed'
                    ? 'Failed'
                    : 'Pending'}
              </span>
            </div>
