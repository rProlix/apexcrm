...
function getLeonardoTextVariablesFormat(): 'json' | 'text' {
  return process.env.LEONARDO_360_TEXT_VARIABLES_FORMAT?.trim().toLowerCase() === 'json'
    ? 'json'
    : 'text'
}
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  if (!packageId || !UUID_RE.test(packageId)) {
    return pumpError({
      ...
      errorMessage: 'packageId must be a valid UUID',
  ...
  if (user.role !== 'owner' && user.role !== 'admin') {
    return pumpError({
      ...
      status: 403,
      errorCode: 'auth_error',
      errorMessage: 'Only owners and admins may pump generation',
      failedStage: 'auth',
    })
  }
  ...
  if (!tenantId) {
    return pumpError({
      ...
      errorMessage: 'Could not resolve tenant from request',
      failedStage: 'auth',
    })
  }
  ...
  if (currentStatus === 'archived') {
    return pumpError({
      packageId,
      status: 409,
      errorCode: 'package_archived',
      errorMessage: 'Package is archived and cannot be generated',
  ...
  const productDesc =
    (productRaw?.description as string | null) || (pkg.description as string) || ''
  ...
  const resolvedReferenceImageUrl: string | null =
    referenceImageUrl || (pkg.master_frame_url as string | null) || null
  ...
  if (providerName === 'leonardo' && !resolvedReferenceImageUrl) {
    ...
    return pumpError({
      packageId,
      status: 422,
      errorCode: 'missing_reference_image',
      errorMessage: msg,
      failedStage: 'reference-check',
    })
  }
  ...
  if (
    ...
    try {
      ...
      console.warn(
        `[P360] pump:scene-contract-build warn: ${scErr instanceof Error ? scErr.message : scErr}`
      )
    }
  }
  ...
  if (
    !hasStoredPrompt ||
    (hasNewLockedScene && !(pkg.scene_blueprint as Record<string, unknown>)?.lockedScene)
  ) {
  ...
  let identityBp = getIdentityBlueprint(
    pkg.locked_identity_blueprint as Record<string, unknown> | null
  )
  ...
  if (!identityBp) {
    ...
    try {
      ...
      await db
        ...
        .eq('id', packageId)
      console.info(
        `[P360] pump:identity-blueprint-built product="${identityBp.subject.productName}"`
      )
    } catch (bpErr) {
      console.warn(
        `[P360] pump:identity-blueprint-build warn: ${bpErr instanceof Error ? bpErr.message : bpErr}`
      )
    }
  }
  ...
  const { data: existingFrames, error: framesErr } = await db
    ...
    .select(
      'frame_index, image_url, provider_job_id, provider_execution_id, provider_status, status, needs_regeneration'
    )
  ...
  if (framesErr) {
    console.warn(
      `[P360] pump:warn stage=frames-load error="${framesErr.message}" (non-fatal, assuming 0 frames)`
    )
  }
  ...
  const completedSet = new Set(
    frames.filter((f) => !!f.image_url && !f.needs_regeneration).map((f) => f.frame_index)
  )
  ...
  if (pendingExecutionId && providerName === 'leonardo' && provider.pollExecution) {
    ...
    if (pollResult.status === 'pending') {
      ...
      const pendingMsg =
        ...
        `Frame ${nextFrameIndex} still generating via Leonardo. Pump again shortly.`
    ...
    if (pollResult.status === 'failed') {
      ...
      return pumpError({
        ...
        errorMessage: errMsg,
        errorDetails: pollResult.error?.details ?? null,
    ...
    if (pollResult.imageBuffer) {
      const up = await uploadFrame({
        tenantId: tenantIdPkg,
        productId,
        packageId,
        frameIndex: nextFrameIndex,
        buffer: pollResult.imageBuffer,
        contentType: mimePoll,
        ext: extPoll,
      })
      uploadedUrlPoll = up.imageUrl
      storagePathPoll = up.storagePath
      ...
      try {
        ...
        if (!contentType.toLowerCase().startsWith('image/')) {
          throw new Error(
            `Remote frame fetch did not return an image content-type. Got: ${contentType || 'unknown'}`
          )
        }
      } catch (fetchErr) {
        ...
        const pkgUpdF: Record<string, unknown> = {
          frames_done: newDoneF,
          progress_percent: progressF,
        ...
        if (isMasterFramePoll) {
          pkgUpdF.master_frame_url = pollResult.imageUrl
          pkgUpdF.master_frame_generated = true
        }
        ...
        return NextResponse.json({
          ...
          progressPercent: progressF,
          framesDone: newDoneF,
          totalFrames,
          remainingFrames: totalFrames - newDoneF,
          processedFrameIndex: nextFrameIndex,
          imageUrl: pollResult.imageUrl,
          message: `Frame ${newDoneF} of ${totalFrames} generated (Leonardo, URL-only).`,
        })
      }
      ...
      const up = await uploadFrame({
        ...
        ext: fetchContentType.includes('jpeg') ? 'jpg' : 'png',
      })
      uploadedUrlPoll = up.imageUrl
      storagePathPoll = up.storagePath
    } else {
      ...
      const diagMsg =
        `Leonardo execution ${pendingExecutionId} did not return an image after polling. ` +
    ...
    return NextResponse.json({
      ...
      packageStatus: 'generating',
      generationStage: 'generating',
      progressPercent: progressPoll,
      framesDone: newDonePoll,
      totalFrames,
      remainingFrames: totalFrames - newDonePoll,
      processedFrameIndex: nextFrameIndex,
      imageUrl: uploadedUrlPoll,
      message: `Frame ${newDonePoll} of ${totalFrames} generated (Leonardo).`,
    })
  }
  ...
  if (!isMasterFrame && storedMasterUrl) {
    try {
      ...
      console.warn(
        `[P360] pump:warn failed to fetch master reference: ${e instanceof Error ? e.message : e}`
      )
    }
  }
  ...
  for (let attempt = 0; attempt <= MAX_FRAME_RETRIES; attempt++) {
    ...
    if (attempt > 0) {
      console.info(
        `[P360] pump:retry attempt=${attempt + 1} frame=${nextFrameIndex} drift="${lastDriftDetails ?? ''}"`
      )
      await db
        .from('product_360_frames')
        .update({
    ...
    try {
      ...
      console.info(
        `[P360] pump:provider:start provider="${providerName}" frame=${nextFrameIndex} attempt=${attempt + 1}`
      )
      ...
      if (result.status === 'failed') {
        ...
        throw Object.assign(new Error(provErr?.message ?? 'Provider returned failed status'), {
      ...
      if (
        !isMasterFrame &&
        lockedSceneForValidation &&
        shouldValidateFrame(consistencyMode, isMasterFrame, attempt + 1)
      ) {
        const frameBase64 = result.imageBuffer?.toString('base64')
        if (frameBase64) {
          const frameMime = (result.mimeType ?? 'image/png') as 'image/png' | 'image/jpeg'
          const validation = await validateFrameAgainstLockedScene(
            frameBase64,
            frameMime,
            lockedSceneForValidation
          )
          ...
          if (validation) {
            ...
            await db
              ...
              .update({
                ...
                consistency_details: {
                  ...
                  driftDetails: validation.driftDetails,
                  attempt: attempt + 1,
                },
                updated_at: new Date().toISOString(),
              })
            ...
            if (
              validation.detectedVariantDrift &&
              !validation.passed &&
              attempt >= MAX_FRAME_RETRIES
            ) {
              ...
              await db
                ...
                .eq('frame_index', nextFrameIndex)
              console.warn(
                `[P360] pump:validation-failed-final frame=${nextFrameIndex} — accepting frame with warning`
              )
            }
          }
        }
      ...
      const isQuota =
        (lastFrameError as { isQuota?: boolean }).isQuota ||
  ...
  if (isMasterFrame && result.imageBuffer) {
    try {
      ...
      console.warn(
        `[P360] pump:blueprint-analysis-error: ${analysisErr instanceof Error ? analysisErr.message : analysisErr}`
      )
    }
  }
  ...
  try {
    ...
    if (result.imageBuffer) {
      ...
      try {
        ...
        if (!fetchRes.ok)
          throw new Error(
            `Remote frame fetch failed: HTTP ${fetchRes.status} for ${result.imageUrl}`
          )
        const contentType = fetchRes.headers.get('content-type') ?? ''
        if (!contentType.toLowerCase().startsWith('image/')) {
          throw new Error(
      ...
      if (fetchRes) {
        ...
        storageStatus = 'remote_url_fallback'
      }
    } else {
      ...
      const noImageMsg =
        providerName === 'leonardo'
          ? `Leonardo accepted the request but did not return an image URL or image buffer. ` +
            ...
            (rawDiag
              ? `Debug: shape=${rawDiag['responseShape'] ?? '?'}, keys=[${(rawDiag['topLevelKeys'] as string[] | undefined)?.join(', ') ?? '?'}]. `
              : '') +
      ...
      console.error(
        `[P360] pump:no-image provider=${providerName} frame=${nextFrameIndex}: ${noImageMsg}`
      )
    ...
    if (err instanceof ImagenApiError && err.statusCode === 429) {
      ...
      console.warn(
        `[P360] pump:quota packageId=${packageId} frame=${nextFrameIndex} retryAt=${retryAt}`
      )
