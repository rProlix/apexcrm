        ...
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span
                className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full border font-medium ${STATUS_STYLES[customer.status] ?? STATUS_STYLES.active}`}
              >
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          },
          {
            icon: CreditCard,
            label: 'Transactions',
            value: recentPayments.transactions.length,
            color: 'text-cyan-400',
            bg: 'bg-cyan-400/10',
              ...
              {recentOrders.slice(0, 5).map((order) => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <p
                      className={`text-xs capitalize ${order.status === 'completed' ? 'text-emerald-400' : order.status === 'pending' ? 'text-yellow-400' : 'text-white/30'}`}
                    >
                      {order.status}
                    </p>
                  </div>
