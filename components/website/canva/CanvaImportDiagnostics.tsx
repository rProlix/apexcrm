...
export function CanvaImportDiagnostics({
  settings,
  imports,
  runs,
  website,
  ...
  const rows: Array<[string, string]> = [
    ...
    [
      'Iframe URL',
      settings.canva_import_mode === 'preserve' ? (settings.canva_source_url ?? '—') : '—',
    ],
    ['Import status', active?.status ?? '—'],
    [
      'Animation preservation',
      settings.canva_animation_preservation ?? active?.animation_preservation ?? '—',
    ],
    ...
    [
      'Publish required',
      settings.canva_import_enabled && !settings.is_published ? 'Yes — publish to go live' : 'No',
    ],
  ...
  if (active?.is_custom_domain)
    staticNotes.push('Custom domain accepted. Embedding may fail if the domain blocks iframes.')
  if (settings.canva_import_enabled) {
    staticNotes.push('Use canva.site or Canva embed code for best compatibility.')
    if (settings.pov_enabled)
      staticNotes.push('POV Event Camera remains native NexoraNow even when Canva is embedded.')
  }
    ...
    <div className="rounded-2xl border border-surface-border bg-graphite-800/40 p-6 space-y-4">
      ...
      {imports.length > 1 && (
        <div className="text-2xs text-white/30">
          {imports.length} total import attempts recorded.
        </div>
      )}
    </div>
  )
