...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await callSlackApi<AuthTestResponse>(
      'auth.test',
      decryptIntegrationToken(integration)
    )
    await db
    ...
    const message =
      error instanceof Error ? error.message.slice(0, 500) : 'Slack connection test failed'
    await db
      .from('van_slack_integrations')
      .update({
        last_tested_at: new Date().toISOString(),
