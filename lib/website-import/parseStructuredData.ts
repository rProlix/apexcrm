...
import type { ParsedStructuredData, StructuredAddress, StructuredReview } from './types'
...
function findPrimaryBlock(blocks: unknown[]): Record<string, unknown> | null {
  ...
  return (
    (blocks.find(
      (b) => b && typeof b === 'object' && typeof (b as Record<string, unknown>).name === 'string'
...
function parseAggregateRating(raw: unknown): { ratingValue: number; reviewCount: number } | null {
  if (!raw || typeof raw !== 'object') return null
...
function parseFaq(blocks: unknown[]): Array<{ question: string; answer: string }> {
  ...
  for (const item of mainEntity) {
    ...
    const a = ans && typeof ans === 'object' ? str((ans as Record<string, unknown>).text) : null
...
export function parseStructuredData(html: string): ParsedStructuredData {
  ...
  const logoRaw = primary.logo ?? (primary.image as Record<string, unknown>)?.logo ?? null
