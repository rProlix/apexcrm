...
export function Premium3DScrollHeroEditor({ sectionId }: Props) {
  ...
  const patchPalette = useCallback(
    (key: keyof ScrollHeroPalette, value: string) => {
  ...
  useEffect(() => {
    if (!tenantId) return
    void getWebsite3DAssets(tenantId)
      .then(setLibrary)
      .catch(() => {})
  }, [tenantId])

  const handleUpload = useCallback(
    async (file: File, assetType: string, field: keyof Premium3DScrollHeroContent) => {
      if (!tenantId) return
  ...
  const handleImageUpload = useCallback(
    async (file: File, field: keyof Premium3DScrollHeroContent) => {
      if (!tenantId) return
      setUploading(field as string)
      try {
    ...
    <div>
      ...
      <div style={groupStyle}>
        ...
        <Select
          ...
          onChange={(v) =>
            patch({ renderMode: v === 'video_scrub' ? 'video_scrub' : 'three_model' })
          }
          options={[
      ...
      <div style={groupStyle}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <input
            ...
            onChange={(e) =>
              patch({
                ctaPrimary: { label: e.target.value, href: content.ctaPrimary?.href ?? '#' },
              })
            }
          />
          <input
            ...
            onChange={(e) =>
              patch({
                ctaPrimary: {
                  label: content.ctaPrimary?.label ?? 'Get Started',
                  href: e.target.value,
                },
              })
            }
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
            ...
            onChange={(e) =>
              patch({
                ctaSecondary: e.target.value
                  ? { label: e.target.value, href: content.ctaSecondary?.href ?? '#' }
                  : null,
              })
            }
          />
          <input
            ...
            onChange={(e) =>
              patch({
                ctaSecondary: {
                  label: content.ctaSecondary?.label ?? 'Learn More',
                  href: e.target.value,
                },
              })
            }
      ...
      {isThree ? (
        <>
          <FileUploadRow
            ...
            onFile={(f) =>
              handleUpload(f, f.name.toLowerCase().endsWith('.gltf') ? 'gltf' : 'glb', 'modelUrl')
            }
          ...
          <Select
            ...
            onChange={(url) => {
              ...
              if (asset.asset_type === 'glb' || asset.asset_type === 'gltf')
                patch({ modelUrl: url, renderMode: 'three_model' })
              else if (asset.asset_type === 'video')
                patch({ videoUrl: url, renderMode: 'video_scrub' })
            ...
            options={[
              ...
              ...library
                ...
                .map((a) => ({
                  value: a.public_url as string,
                  label: `${a.asset_type} · ${a.name}`,
                })),
            ]}
          />
        </div>
      ...
      {isThree && (
        <>
          ...
          <RangeRow
            ...
            max={4}
            step={0.1}
            value={content.modelScale ?? 1}
            onChange={(v) => patch({ modelScale: v })}
          />
          <RangeRow
            label="Rotation amount (final Y angle, turns)"
            min={0}
            max={3}
            step={0.25}
            ...
            onChange={(v) =>
              patch({
                targetRotation: {
                  x: content.targetRotation?.x ?? 0,
                  y: v * Math.PI * 2,
                  z: content.targetRotation?.z ?? 0,
                },
              })
            }
          />
          ...
          <div style={groupStyle}>
            ...
            <Select
              value={content.lightingPreset ?? 'studioSoftbox'}
              onChange={(v) =>
                patch({ lightingPreset: v as Premium3DScrollHeroContent['lightingPreset'] })
              }
              options={[
                'studioSoftbox',
                'premiumSpotlight',
          ...
          <RangeRow
            ...
            max={1}
            step={0.05}
            value={content.shadowIntensity ?? 0.6}
            onChange={(v) => patch({ shadowIntensity: v })}
          />
          <div style={groupStyle}>
            <label style={labelStyle}>Environment style</label>
            <Select
              value={content.environmentPreset ?? 'studio'}
              onChange={(v) =>
                patch({ environmentPreset: v as Premium3DScrollHeroContent['environmentPreset'] })
              }
              options={['none', 'studio', 'city', 'warehouse', 'sunset', 'dawn', 'night'].map(
                (o) => ({ value: o, label: o })
              )}
            />
          </div>
          <div style={groupStyle}>
            ...
            <Select
              value={content.stageRevealMode ?? 'none'}
              onChange={(v) =>
                patch({ stageRevealMode: v as Premium3DScrollHeroContent['stageRevealMode'] })
              }
          ...
          <div style={groupStyle}>
            <Toggle
              ...
              value={content.pinOnScroll !== false}
      ...
      <div style={groupStyle}>
        ...
        <Select
          value={content.textAnimation ?? 'fadeUpWords'}
          onChange={(v) =>
            patch({ textAnimation: v as Premium3DScrollHeroContent['textAnimation'] })
          }
          options={['fadeUpWords', 'blurReveal', 'scaleWords', 'luxurySplit', 'none'].map((o) => ({
            value: o,
            label: o,
          }))}
        />
      ...
      <div style={groupStyle}>
        ...
        <Select
          value={content.shaderPreset ?? 'none'}
      ...
      {isThree && (
        <>
          ...
          <div style={groupStyle}>
            ...
            <Select
              value={content.reducedMotionFallback ?? 'poster'}
              onChange={(v) =>
                patch({
      ...
      <div style={groupStyle}>
        <Toggle
          label="Apply palette to whole site while in view"
          value={!!content.applyPaletteGlobally}
          onChange={(v) => patch({ applyPaletteGlobally: v })}
        />
      </div>
      {(['background', 'foreground', 'accent', 'muted', 'glow'] as (keyof ScrollHeroPalette)[]).map(
        (key) => (
          <div
            key={key}
            style={{
          ...
          >
            <span style={{ fontSize: '0.8125rem', color: '#a1a1aa', textTransform: 'capitalize' }}>
              {key}
            </span>
            <input
              type="color"
      ...
      >
        No Spline required. This hero uses Three.js (3D models) or H.264 MP4 / image-sequence
        scroll-scrubbing. Missing assets fall back to a premium gradient automatically.
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ marginBottom: '1rem' }}>
      ...
      {currentUrl && (
        ...
        >
          {currentUrl.slice(0, 70)}
          {currentUrl.length > 70 ? '…' : ''}
        </p>
      )}
    </div>
  )
