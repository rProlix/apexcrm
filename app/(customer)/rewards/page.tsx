    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          >
            <div
              className={`h-9 w-9 rounded-xl ${link.bg} border ${link.border} flex items-center justify-center`}
            >
              ...
              <div key={card.id} className="premium-panel premium-border rounded-xl p-4">
                ...
                <div className="h-1.5 rounded-full bg-white/8 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (card.current_punches / card.punch_goal) * 100)}%`,
                    }}
                  />
                </div>
              </div>
          ...
          <div className="premium-panel premium-border rounded-2xl divide-y divide-white/4">
            {recentTxns.map((tx, i) => (
              <div key={i} className="px-4 py-3 flex items-center justify-between">
                ...
                <span
                  className={`text-sm font-bold tabular-nums ${tx.points_delta > 0 ? 'text-emerald-400' : 'text-orange-400'}`}
                >
                  {tx.points_delta > 0 ? '+' : ''}
