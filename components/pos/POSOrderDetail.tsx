...
export function POSOrderDetail({ order, tenantId, userRole }: Props) {
  ...
  const canSendKitchen =
    !['completed', 'cancelled', 'refunded'].includes(status) && status !== 'sent_to_kitchen'
  ...
  const canRefund =
    ['paid', 'partially_paid', 'completed'].includes(order.payment_status as string) &&
    ...
    <div className="min-h-screen bg-zinc-950 p-4 sm:p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-start gap-4 mb-6">
          <Link
            href="/pos/orders"
            className="mt-1 p-2 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-lg transition-colors"
          >
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span
                className={`px-3 py-1 rounded-full border text-sm font-medium ${STATUS_COLORS[status] ?? 'text-zinc-400 bg-zinc-800 border-zinc-700'}`}
              >
                {status.replace(/_/g, ' ')}
              </span>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {canSendKitchen && (
            <button
              onClick={() => doAction('send-to-kitchen')}
              disabled={loading === 'send-to-kitchen'}
        ...
        <div className="grid lg:grid-cols-3 gap-5">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="mt-1 space-y-0.5 ml-6">
                              {mods.map((m) => (
                                <p key={m.id as string} className="text-xs text-zinc-400">
                                  ...
                                  {m.name as string}
                                  {(m.price_delta_cents as number) !== 0 &&
                                    ` (${formatCents(m.price_delta_cents as number)})`}
                                </p>
                              ))}
                            </div>
                          )}
                          {Boolean(item.notes || item.kitchen_notes) && (
                            <p className="text-xs text-zinc-500 mt-1 ml-6 italic">
                              {[String(item.notes ?? ''), String(item.kitchen_notes ?? '')]
                                .filter(Boolean)
                                .join(' | ')}
                            </p>
                          )}
                        </div>
                        <p className="text-sm font-bold text-violet-400 flex-none">
                          {formatCents(item.total_cents as number)}
                        </p>
                      </div>
                    </div>
                  )
              ...
              <div className="border-t border-zinc-800 px-4 py-3 space-y-1.5">
                <div className="flex justify-between text-sm text-zinc-400">
                  <span>Subtotal</span>
                  <span>{formatCents(order.subtotal_cents as number)}</span>
                </div>
                {(order.discount_cents as number) > 0 && (
                  <div className="flex justify-between text-sm text-green-400">
                    <span>Discount</span>
                    <span>−{formatCents(order.discount_cents as number)}</span>
                  </div>
                )}
                {(order.tax_cents as number) > 0 && (
                  <div className="flex justify-between text-sm text-zinc-400">
                    <span>Tax</span>
                    <span>{formatCents(order.tax_cents as number)}</span>
                  </div>
                )}
                {(order.tip_cents as number) > 0 && (
                  <div className="flex justify-between text-sm text-zinc-400">
                    <span>Tip</span>
                    <span>{formatCents(order.tip_cents as number)}</span>
                  </div>
                )}
                <div className="flex justify-between text-base font-bold text-zinc-100 border-t border-zinc-700 pt-2">
                  <span>Total</span>
                ...
                <div className="flex justify-between text-sm text-zinc-400">
                  ...
                  <span className="text-green-400">
                    {formatCents(order.amount_paid_cents as number)}
                  </span>
                </div>
                {(order.balance_due_cents as number) > 0 && (
                  <div className="flex justify-between text-sm font-bold text-red-400">
                    <span>Balance Due</span>
                    <span>{formatCents(order.balance_due_cents as number)}</span>
                  </div>
                )}
              </div>
            </div>
                    ...
                    >
                      <div>
                        <p className="text-sm text-zinc-200 capitalize">
                          {(p.payment_method as string).replace(/_/g, ' ')}
                        </p>
                        <p className="text-xs text-zinc-500">
                          {p.paid_at ? new Date(p.paid_at as string).toLocaleString() : 'Pending'}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold text-green-400">
                          {formatCents(p.amount_cents as number)}
                        </p>
                        <p
                          className={`text-xs capitalize ${p.status === 'paid' ? 'text-green-400' : 'text-zinc-500'}`}
                        >
                          {p.status as string}
                        </p>
                      </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-2">
              ...
              {Boolean(order.table_name) && (
                <InfoRow label="Table" value={order.table_name as string} />
              )}
              {Boolean(order.guest_count) && (
                <InfoRow label="Guests" value={String(order.guest_count)} />
              )}
