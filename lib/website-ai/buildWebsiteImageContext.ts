...
export async function buildWebsiteImageContext(tenantId: string): Promise<RichImageContext> {
  ...
  const sectionDetails: RichSectionDetail[] = sections.map((s) => {
    const c = (s.content && typeof s.content === 'object' ? s.content : {}) as Record<
      string,
      unknown
    >
    return {
  ...
  for (const suggestion of autofillSuggestions) {
    if (suggestion.suggestion_type !== 'reviews' && suggestion.suggestion_type !== 'testimonials')
      continue
    const data = (suggestion.extracted_data as Record<string, unknown>) ?? {}
