...
function buildValidationPrompt(lockedScene: Product360LockedScene): string {
  ...
  const toppingList = lockedScene.foodDetails?.toppings.length
    ? lockedScene.foodDetails.toppings.map((t) => `${t.name} (${t.count})`).join(', ')
    : null
...
${
  isFood
    ? `2. Are ALL required toppings/ingredients present?
3. Are there any EXTRA toppings/ingredients that should NOT be there?
4. ${foodType === 'pizza' ? 'Is this a ' + (lockedScene.foodDetails?.subType?.toLowerCase().includes('cheese') && !lockedScene.foodDetails?.subType?.toLowerCase().includes('combo') ? 'CHEESE-ONLY pizza (no meat, no vegetables)?  ★ This is critical ★' : 'COMBO pizza with visible toppings?  ★ This is critical ★') : 'Are the food details correct?'}`
    : '2. Is the product correct?'
}
...
export async function validateFrameAgainstLockedScene(
  ...
  const { text, error } = await callGeminiMultimodal({
    ...
    temperature: 0.1,
    parts: [{ inlineData: { mimeType, data: frameBase64 } }, { text: prompt }],
  })
  ...
  if (!data || typeof data.score !== 'number') {
    console.warn(
      '[frameConsistencyValidator] Could not parse validation result — raw:',
      text.slice(0, 300)
    )
    return null
  }
  ...
  const shouldRegenerate =
    data.shouldRegenerate || data.detectedVariantDrift || data.score < DRIFT_SCORE_CUTOFF
