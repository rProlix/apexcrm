...
function asProfileImage(value: unknown): VanProfileImage {
  ...
  const mode = record.mode === 'manual' || record.mode === 'automatic_first_upload' ? record.mode : 'fallback'
...
export default async function VehicleProfilePage({
  ...
  const [
    ...
    db.from('van_damage_inspections').select('id, status, review_status, damage_count, ai_summary, ai_confidence, created_at, completed_at')
      .eq('tenant_id', scope.tenantId).eq('business_id', scope.businessId).eq('van_id', vehicleId).order('created_at', { ascending: false }).limit(50),
    newTables.from('van_damage_images').select('id, inspection_id, upload_session_id, upload_order, original_file_index, status, image_role, created_at, s3_key')
      .eq('tenant_id', scope.tenantId).eq('business_id', scope.businessId).order('created_at', { ascending: true }).limit(500),
    newTables.from('van_damage_cases')
      .select('*').eq('tenant_id', scope.tenantId).eq('business_id', scope.businessId).eq('van_id', vehicleId).order('last_observed_at', { ascending: false }).limit(100),
    newTables.from('van_damage_observations')
  ...
  const images = ((imagesResult.data ?? []) as VanProfileImageRow[]).filter((image) => image.upload_session_id && sessionIds.has(image.upload_session_id))
  const observations = (observationsResult.data ?? []) as Array<VanProfileCase['observations'][number] & { damage_case_id: string | null }>
  const channels = new Map((channelsResult.data ?? []).map((channel: { slack_channel_id: string; slack_channel_name: string | null }) => [channel.slack_channel_id, channel.slack_channel_name]))
  const profileImage = asProfileImage(asRecord(asRecord(vehicle.metadata).vanDamage).profileImage)
  const fallbackProfileImageId = images.find((image) => image.s3_key && ['uploaded', 'analyzed'].includes(image.status))?.id ?? null
  const hydratedSessions: VanProfileSession[] = sessions.map((session) => ({
    ...session,
    driver_snapshot: asRecord(session.driver_snapshot),
    channelName: typeof channels.get(session.slack_channel_id) === 'string' ? channels.get(session.slack_channel_id)! : null,
    ...
    images: images
      ...
      .sort((a, b) => (a.upload_order ?? a.original_file_index ?? 2147483647) - (b.upload_order ?? b.original_file_index ?? 2147483647))
      .map((image) => ({ id: image.id, upload_order: image.upload_order, status: image.status, image_role: image.image_role })),
    observations: observations
      .filter((observation) => observation.upload_session_id === session.id)
      .map((observation) => ({ observation_type: observation.observation_type, severity: observation.severity })),
  }))
  const hydratedCases: VanProfileCase[] = ((casesResult.data ?? []) as VanProfileCase[]).map((damageCase) => ({
    ...damageCase,
    observations: observations
      .filter((observation) => observation.damage_case_id === damageCase.id)
      .map((observation) => ({ ...observation, driver_snapshot: asRecord(observation.driver_snapshot) })),
  }))
