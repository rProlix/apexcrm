...
export function CustomerInvitePanel({
    ...
    try {
      ...
      if (data.ok && data.invites?.length > 0) {
        ...
        const sorted = (data.invites as CustomerInvite[]).sort(
          (a, b) =>
            new Date(b.last_sent_at ?? b.expires_at).getTime() -
            new Date(a.last_sent_at ?? a.expires_at).getTime()
        )
  ...
  const handleRevoke = useCallback(async () => {
    if (!invite) return
    if (!confirm('Revoke this invite? The customer will no longer be able to use the invite link.'))
      return
  ...
  return (
    <>
      <div className="premium-panel premium-border rounded-2xl p-5 space-y-4">
        ...
        {invite && cfg ? (
          <div
            className={`flex items-center gap-2.5 rounded-xl ${cfg.bg} border ${cfg.border} px-4 py-3`}
          >
          ...
          <div className="flex items-center gap-2.5 rounded-xl bg-white/4 border border-white/8 px-4 py-3">
            ...
            <p className="text-xs text-white/40">
              {hasAccount
                ? 'Account linked without invite.'
                : 'No portal access. Send an invite to get started.'}
            </p>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {invite && (invite.status === 'pending' || invite.status === 'expired') && (
            ...
            >
              <RefreshCw
                className={`w-3.5 h-3.5 ${actionLoading === 'resend' ? 'animate-spin' : ''}`}
              />
              Resend
            </button>
          )}
