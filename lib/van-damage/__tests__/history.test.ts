...
test('driver attribution fallback uses display, real name, username, safe Slack ID, then unknown', () => {
  assert.equal(
    formatDriverName({ slackWorkspaceId: 'T1', slackUserId: 'U123456', displayName: 'Jordan S.' }),
    'Jordan S.'
  )
  assert.equal(
    formatDriverName({ slackWorkspaceId: 'T1', slackUserId: 'U123456', realName: 'Jordan Stone' }),
    'Jordan Stone'
  )
  assert.equal(
    formatDriverName({ slackWorkspaceId: 'T1', slackUserId: 'U123456', username: 'jstone' }),
    'jstone'
  )
  assert.equal(
    formatDriverName({ slackWorkspaceId: 'T1', slackUserId: 'U123456' }),
    'Slack user U12345'
  )
...
test('Slack file ordering is stable and preserves original order when timestamps are absent', () => {
  assert.deepEqual(
    orderSlackFiles([{ id: 'F2' }, { id: 'F1' }]).map((file) => file.id),
    ['F2', 'F1']
  )
  assert.deepEqual(
    orderSlackFiles([
