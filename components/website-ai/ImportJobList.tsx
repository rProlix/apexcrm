...
export function ImportJobList({ jobs, selected, onSelect }: Props) {
  if (!jobs.length) {
    return <p className="text-xs text-white/30 text-center py-6">No import jobs yet.</p>
  }
      ...
      {jobs.map((job) => (
        ...
        >
          <div className="flex items-center justify-between gap-2">
            ...
            <span
              className={cn(
                'shrink-0 text-2xs font-semibold px-2 py-0.5 rounded border uppercase tracking-wide',
                STATUS_STYLE[job.status ?? 'draft']
              )}
            >
              {job.status}
            </span>
