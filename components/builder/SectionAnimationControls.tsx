...
function ComponentPresetRow({
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.375rem' }}>
      <span style={{ fontSize: '0.6875rem', color: '#71717a', width: 52, flexShrink: 0 }}>
        {label}
      </span>
      <div style={{ flex: 1 }}>
        <Select
...
export function SectionAnimationControls({ sectionId }: { sectionId: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/website/sections/${sectionId}/animation`)
      ...
      .catch(() => {
        /* silently ignore — will just show defaults */
      })
  ...
  if (loading) {
    return (
      <div style={{ color: '#52525b', fontSize: '0.75rem', padding: '0.5rem 0' }}>Loading…</div>
    )
  }
  ...
  return (
    <div style={{ fontSize: '0.8125rem', color: '#f4f4f5' }}>
        ...
        <div>
          ...
          <div style={{ marginTop: '0.25rem' }}>
            <NumInput
              ...
              max={3000}
              step={100}
              onChange={(durationMs) => setState((s) => ({ ...s, durationMs }))}
              suffix="ms"
            />
          </div>
        </div>
        <div>
          ...
          <div style={{ marginTop: '0.25rem' }}>
            <NumInput
              ...
              max={2000}
              step={50}
              onChange={(delayMs) => setState((s) => ({ ...s, delayMs }))}
              suffix="ms"
            />
          </div>
        </div>
        <div>
          ...
          <div style={{ marginTop: '0.25rem' }}>
            <NumInput
              ...
              max={800}
              step={20}
              onChange={(staggerMs) => setState((s) => ({ ...s, staggerMs }))}
              suffix="ms"
            />
          </div>
        </div>
      ...
      <div
        style={{ borderTop: '1px solid #1c1c1e', paddingTop: '0.625rem', marginBottom: '0.625rem' }}
      >
        <FieldLabel>Component animations</FieldLabel>
        <div style={{ marginTop: '0.5rem' }}>
          <ComponentPresetRow
            label="Heading"
            config={state.heading}
