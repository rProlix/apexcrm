...
const DESIGN_LEVELS = new Set<DesignLevel>([
  'clean',
  'premium',
  'luxury',
  'bold',
...
const FLOW_STYLES = new Set<SectionFlowStyle>([
  'soft_blend',
  'curved',
  'angled',
  'layered',
...
const BG_STRATEGIES = new Set<BackgroundStrategy>([
  'alternating_soft',
  'continuous_gradient',
  'layered_surfaces',
  'image_blend',
...
const OVERLAY_STRATS = new Set<OverlayStrategy>([
  'auto_gradient_overlay',
  'auto_blur_overlay',
  'auto_shadow_overlay',
  'solid_scrim',
])
const BG_TYPES = new Set<SectionBackgroundType>([
  'solid',
  'gradient',
  'image',
  'layered',
...
const CARD_STYLES = new Set<SectionCardStyle>([
  'none',
  'soft',
  'glass',
  'floating',
...
const IMAGE_TREATMENTS = new Set<SectionImageTreatment>([
  'none',
  'rounded',
  'floating',
  'overlay',
...
function normalizePalette(input: Record<string, unknown>, preset: DesignPalette): DesignPalette {
  ...
  let surface = hex(input.surface, preset.surface)
  let surfaceAlt = hex(input.surfaceAlt, preset.surfaceAlt)
  let border = hex(input.border, preset.border)
  ...
  if (colorContrast(textPrimary, surface) < 4.5) {
    surface = mixHex(background, textPrimary, isDark(background) ? 0.06 : 0.035)
  }
  if (colorContrast(textPrimary, surfaceAlt) < 4.5) {
    surfaceAlt = mixHex(background, textPrimary, isDark(background) ? 0.1 : 0.065)
  }
  if (colorContrast(border, background) < 1.5) {
    border = mixHex(background, textPrimary, 0.2)
  }

  const roles = deriveBrandColorRoles(primary, background, preset.primary)
...
function normalizeTypography(
  ...
  return {
    headingFontCategory: HEADING_CATS.has(hCat)
      ? (hCat as DesignTypography['headingFontCategory'])
      : preset.headingFontCategory,
    bodyFontCategory: BODY_CATS.has(bCat)
      ? (bCat as DesignTypography['bodyFontCategory'])
      : preset.bodyFontCategory,
...
export function normalizeDesignSystem(
  ...
  const paletteRaw =
    raw.palette && typeof raw.palette === 'object' ? (raw.palette as Record<string, unknown>) : {}
  const gradientsRaw =
    raw.gradients && typeof raw.gradients === 'object'
      ? (raw.gradients as Record<string, unknown>)
      : {}
  const typographyRaw =
    raw.typography && typeof raw.typography === 'object'
      ...
      : {}
  const radiusRaw =
    raw.radius && typeof raw.radius === 'object' ? (raw.radius as Record<string, unknown>) : {}
  const shadowsRaw =
    raw.shadows && typeof raw.shadows === 'object' ? (raw.shadows as Record<string, unknown>) : {}
  const layoutRaw =
    raw.layout && typeof raw.layout === 'object' ? (raw.layout as Record<string, unknown>) : {}
  const flowRaw =
    raw.sectionFlow && typeof raw.sectionFlow === 'object'
      ? (raw.sectionFlow as Record<string, unknown>)
      : {}
  const a11yRaw =
    raw.accessibility && typeof raw.accessibility === 'object'
  ...
  const layout: DesignLayout = {
    ...
    sectionPaddingDesktop: s(
      layoutRaw.sectionPaddingDesktop as string,
      preset.layout.sectionPaddingDesktop
    ),
    sectionPaddingMobile: s(
      layoutRaw.sectionPaddingMobile as string,
      preset.layout.sectionPaddingMobile
    ),
    verticalRhythm: enumVal(
      VERTICAL_RHYTHMS,
      layoutRaw.verticalRhythm,
      preset.layout.verticalRhythm
    ),
  ...
  const sectionFlow = {
    ...
    backgroundStrategy: enumVal(
      BG_STRATEGIES,
      flowRaw.backgroundStrategy,
      preset.sectionFlow.backgroundStrategy
    ),
  }
  ...
  const accessibility = {
    ...
    overlayStrategy: enumVal(
      OVERLAY_STRATS,
      a11yRaw.overlayStrategy,
      'auto_gradient_overlay' as OverlayStrategy
    ),
...
export function normalizeSectionDesign(
  ...
  return {
    ...
    overlay: {
      ...
      type: ['gradient', 'blur', 'scrim', 'shadow'].includes(
        s(overlayRaw.type as string, 'gradient')
      )
        ? (s(overlayRaw.type as string, 'gradient') as SectionDesign['overlay']['type'])
        : 'gradient',
      value: s(overlayRaw.value as string, ''),
      opacity:
        typeof overlayRaw.opacity === 'number' ? Math.min(1, Math.max(0, overlayRaw.opacity)) : 0,
    },
    ...
    imageTreatment: enumVal(
      IMAGE_TREATMENTS,
      raw.imageTreatment,
      DEFAULT_SECTION_DESIGN.imageTreatment
    ),
 ...
 */
export function buildDefaultSectionDesign(designSystem: WebsiteDesignSystem): SectionDesign {
  return {
...
export function ensureReadableSectionColors(
  ...
  const buttonForeground = chooseReadableTextColor(designSystem.palette.primary)
  const buttonContrast = passesWcag(buttonForeground, designSystem.palette.primary, 'AA')
    ? 'pass'
    : 'fail'
...
export function buildCssVars(ds: WebsiteDesignSystem): Record<string, string> {
  ...
  const r = ds.radius
  const roles = deriveBrandColorRoles(p.primary, p.background, DEFAULT_PRESET.palette.primary)
  const link = ensureColorContrast(p.primary, p.background, 4.5)

  return {
    // Palette
    '--color-primary': p.primary,
    '--color-primary-foreground': roles.primaryForeground,
    '--color-primary-hover': roles.primaryHover,
    '--color-primary-active': roles.primaryActive,
    '--color-primary-soft': roles.primarySoft,
    '--color-secondary': p.secondary,
    '--color-accent': p.accent,
    '--color-link': link,
    '--color-focus': roles.focus,
    ...
    '--ds-primary-hover': roles.primaryHover,
    '--ds-primary-active': roles.primaryActive,
    '--ds-primary-soft': roles.primarySoft,
    '--ds-secondary': p.secondary,
    '--ds-accent': p.accent,
    '--ds-border': p.border,
    '--ds-primary-light': tintColor(p.primary, 0.15),
    '--ds-primary-text': roles.primaryForeground,
    // Typography
...
function checkContrast(fg: string, bg: string, _ds: WebsiteDesignSystem): 'pass' | 'fail' | 'warn' {
  ...
  return passesWcag(fg, bg, 'AA') ? 'pass' : passesWcag(fg, bg, 'AA', 'large') ? 'warn' : 'fail'
}
