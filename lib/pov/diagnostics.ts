...
export async function buildPovDiagnostics(
  event: PovEventRow,
  publicBase: string
): Promise<PovDiagnostics> {
  const db = getSupabaseServerClient()

  const [websiteTypeRes, bucketRes, guests, media, photos, videos, audio, sessions] =
    await Promise.all([
      ...
      count('pov_media', (q) =>
        q.eq('event_id', event.id).eq('media_type', 'photo').neq('status', 'deleted')
      ),
      count('pov_media', (q) =>
        q.eq('event_id', event.id).eq('media_type', 'video').neq('status', 'deleted')
      ),
      count('pov_media', (q) =>
        q.eq('event_id', event.id).eq('media_type', 'audio').neq('status', 'deleted')
      ),
