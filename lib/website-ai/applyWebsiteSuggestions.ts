...
async function applyProductsSuggestion(
  ...
  return applySectionSuggestion({ ...suggestion, suggestion_type: 'services' }, ctx, homePageId)
}
...
function mergeContent(
  ...
  if (sectionType === 'testimonials') {
    ...
    const filtered = newItems.filter((ni) => !isDuplicateReview(existItems, ni.name, ni.text))
  ...
  if (sectionType === 'feature_grid') {
    ...
    const filtered = newItems.filter((ni) => !isDuplicateService(existItems, ni.title ?? ''))
    return { ...existing, items: [...existItems, ...filtered] }
  }
  ...
  if (sectionType === 'faq') {
    ...
    const filtered = newItems.filter((ni) => !isDuplicateFaq(existItems, ni.question))
    return { ...existing, items: [...existItems, ...filtered] }
  }
