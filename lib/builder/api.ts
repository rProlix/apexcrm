...
export async function uploadSectionImage(file: File, tenantId: string): Promise<string | null> {
...
export async function getWebsite3DAssets(
  ...
  const f: Get3DAssetsFilters =
    typeof filters === 'string' ? { assetType: filters } : (filters ?? {})
...
export async function getWebsite3DAssetGroups(
  ...
  const empty: Website3DAssetGroups = {
    videos: [],
    imageSequences: [],
    posters: [],
    fallbacks: [],
...
export async function getSectionImages(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      (data as Record<string, string>).error ?? `Failed to load images (${res.status})`
    )
  }
  return res.json()
}
...
export async function activateSectionImage(
  ...
  const res = await fetch(`/api/website/sections/${sectionId}/images/${imageId}/activate`, {
    method: 'POST',
  })
  if (!res.ok) {
    const data = await res.json().catch(() => ({}))
    throw new Error(
      (data as Record<string, string>).error ?? `Failed to activate image (${res.status})`
    )
  }
  return res.json()
}
...
export async function archiveSectionImage(
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}))
    throw new Error(
      (data as Record<string, string>).error ?? `Failed to archive image (${res.status})`
    )
  }
  return res.json()
}
...
export async function restoreSectionImage(
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}))
    throw new Error(
      (data as Record<string, string>).error ?? `Failed to restore image (${res.status})`
    )
  }
  return res.json()
}
