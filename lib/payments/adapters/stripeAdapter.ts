...
import { getStripePlatformClient, stripeRequestOptions } from '@/lib/payments/stripe/server'
import type {
  PaymentAdapter,
  AdapterConfig,
  PaymentLinkParams,
...
function getStripe(config: AdapterConfig): Stripe {
  if (config.accountId) return getStripePlatformClient()
...
function requestOptions(config: AdapterConfig): Stripe.RequestOptions | undefined {
  return config.accountId ? stripeRequestOptions(config.accountId) : undefined
}
...
export const stripeAdapter: PaymentAdapter = {
  ...
  async createInvoice(params: InvoiceParams, config: AdapterConfig): Promise<InvoiceResult> {
    ...
    const finalized = await stripe.invoices.finalizeInvoice(invoice.id, {}, requestOptions(config))
  ...
  async cancelPayment(params: CancelParams, config: AdapterConfig): Promise<void> {
    const stripe = getStripe(config)
    await stripe.paymentIntents.cancel(params.providerTransactionId, {}, requestOptions(config))
  },
  ...
  async handleWebhook(
    ...
    try {
      ...
      throw new Error(
        `[StripeAdapter] Webhook signature verification failed: ${(err as Error).message}`
      )
    }
    ...
    return {
      ...
      currency:
        typeof obj.currency === 'string' ? (obj.currency as string).toUpperCase() : undefined,
