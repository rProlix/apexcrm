...
function variantsFor(preset: NexoraAnimationPreset, duration: number, delay: number): Variants {
  ...
  switch (preset) {
    ...
    case 'fadeUp':
      return { hidden: { opacity: 0, y: 40 }, show: { opacity: 1, y: 0, transition: t } }
    case 'slideInLeft':
      return { hidden: { opacity: 0, x: -60 }, show: { opacity: 1, x: 0, transition: t } }
    case 'slideInRight':
      return { hidden: { opacity: 0, x: 60 }, show: { opacity: 1, x: 0, transition: t } }
    case 'zoomIn':
      return { hidden: { opacity: 0, scale: 0.9 }, show: { opacity: 1, scale: 1, transition: t } }
    case 'softParallax':
      return {
        hidden: { opacity: 0, y: 60 },
        show: { opacity: 1, y: 0, transition: { ...t, duration: duration * 1.3 } },
      }
    case 'imageReveal':
      return {
        hidden: { opacity: 0, clipPath: 'inset(12% 12% 12% 12%)' },
        show: {
          opacity: 1,
          clipPath: 'inset(0% 0% 0% 0%)',
          transition: { ...t, duration: duration * 1.2 },
        },
      }
    case 'maskReveal':
      return {
    ...
    case 'premiumBlurReveal':
      return {
        ...
        show: {
          ...
          filter: 'blur(0px)',
          scale: 1,
          transition: { ...t, duration: duration * 1.3 },
        },
      }
    case 'floating':
    ...
    case 'subtleRotate':
      return {
        ...
        show: { opacity: 1, rotate: 0, scale: 1, transition: t },
      }
    case 'staggerText':
