// app/api/website/canva/imports/[importId]/undo/route.ts
// Undo the latest completed Canva import for this website — restores the
// pre-import draft and reverts canva_* settings. Does not touch the published
// site unless the user separately restores the published version.
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ importId: string }> }) {
  ...
  return NextResponse.json({ ok: true, restored: result.restored, runId: result.runId, publishRequired: true })
}
