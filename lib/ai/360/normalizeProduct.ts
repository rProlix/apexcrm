...
export function normalizeProductSubject(
  ...
  if (categoryPreset) {
    if (categoryPreset.includes('food') || categoryPreset.includes('bowl'))
      productCategory = 'food_bowl'
    else if (categoryPreset.includes('beverage') || categoryPreset.includes('cup'))
  ...
  // ── Subject phrase ────────────────────────────────────────────────────────
  const subjectPhrase = rawDescription.length > 0 ? `${vessel} of ${name.toLowerCase()}` : name
