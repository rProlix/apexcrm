...
export async function POST(req: NextRequest) {
  ...
  const { data: dbSections } = (await sectionsBaseQuery) as {
  ...
  if (!dbSections || dbSections.length === 0)
    return NextResponse.json(
      { error: 'No sections found. Build your website first before using AI Restyle.' },
      { status: 400 }
    )
  ...
  const sections: RestyleSectionContext[] = dbSections.map((s) => {
    ...
    >
    const sc = (
      typeof s.style_config === 'object' && s.style_config !== null ? s.style_config : {}
  ...
  if (normalizeError || !plan) {
    ...
    return NextResponse.json(
      { error: normalizeError ?? 'Failed to process AI response.' },
      { status: 500 }
    )
  }
