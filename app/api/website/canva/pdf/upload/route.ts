...
export async function POST(req: NextRequest) {
  ...
  if (!contentType.includes('multipart/form-data')) {
    return NextResponse.json(
      { ok: false, error: 'Upload must be multipart/form-data with a PDF file.' },
      { status: 400 }
    )
  }
  ...
  if (!isPdf)
    return NextResponse.json(
      { ok: false, error: 'Only PDF files are accepted. Export your Canva design as a PDF.' },
      { status: 400 }
    )
  if (f.size === 0)
    return NextResponse.json({ ok: false, error: 'The PDF file is empty.' }, { status: 400 })
  if (f.size > MAX_PDF_BYTES)
    return NextResponse.json({ ok: false, error: 'PDF is too large (max 25 MB).' }, { status: 413 })
  ...
  if (!ensured.ref)
    return NextResponse.json(
      { ok: false, error: ensured.error ?? 'Could not create the event website record.' },
      { status: 400 }
    )
  ...
  try {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: `Failed to upload PDF: ${e instanceof Error ? e.message : 'storage error'}`,
      },
      { status: 500 }
    )
  }
  ...
  try {
    const { data: imp, error: impErr } = await db
      ...
      .single()
    if (impErr) {
      const msg = isMissingColumnError(impErr.message)
        ? SCHEMA_MISSING_MESSAGE
        : `Failed to create Canva import row: ${impErr.message}`
      return NextResponse.json(
        { ok: false, error: msg, hasRequiredSchema: !isMissingColumnError(impErr.message) },
        { status: isMissingColumnError(impErr.message) ? 503 : 500 }
      )
    }
    importId = imp?.id ?? null
  } catch (e) {
    const message = e instanceof Error ? e.message : 'database error'
    const msg = isMissingColumnError(message)
      ? SCHEMA_MISSING_MESSAGE
      : `Failed to create Canva import row: ${message}`
    return NextResponse.json(
      { ok: false, error: msg, hasRequiredSchema: !isMissingColumnError(message) },
      { status: isMissingColumnError(message) ? 503 : 500 }
    )
  }
