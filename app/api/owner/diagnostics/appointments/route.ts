...
export async function GET() {
  ...
  try {
    const { error } = await supabase.from('appointment_availability_blocks').select('id').limit(1)
    checks.push({
  ...
  try {
    ...
    const { data: row } = await supabase
      ...
      .select(
        'id,tenant_id,staff_id,title,description,block_type,day_of_week,start_time,end_time,start_at,end_at,timezone,is_recurring,max_bookings,appointment_duration_minutes,buffer_before_minutes,buffer_after_minutes,is_active,created_at,updated_at'
      )
      .limit(1)
      .maybeSingle()
  ...
  if (tenantId) {
    try {
      ...
      checks.push({
        ...
        ok: !error,
        detail: error ? `${error.code}: ${error.message}` : `${count ?? 0} block(s) found`,
      })
    } catch (e: unknown) {
    ...
    try {
      ...
      checks.push({
        ...
        ok: !error,
        detail: error ? `${error.code}: ${error.message}` : `${count ?? 0} professional(s) found`,
      })
    } catch (e: unknown) {
    ...
    checks
      ...
      .forEach((c) => {
        if (c.label.includes('table') || c.label.includes('column') || c.label.includes('RLS')) {
          fixes.push(
            `Run migration 061_appointment_availability_visible.sql in Supabase SQL Editor`
          )
        }
        if (c.label.includes('Tenant')) {
          fixes.push(
            'Ensure your user account has a matching row in the public.users table with a valid tenant_id'
          )
        }
