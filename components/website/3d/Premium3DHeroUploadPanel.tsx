...
export function Premium3DHeroUploadPanel({ sectionId }: Props) {
  ...
  const patch = useCallback(
    (changes: Record<string, unknown>) => {
      ...
      if (!sec) return
      st.updateSectionContent(sectionId, {
  ...
  const refresh = useCallback(() => {
    if (!tenantId) return
    void getWebsite3DAssetGroups(tenantId, { sectionId })
      .then(setGroups)
      .catch(() => {})
  }, [tenantId, sectionId])
  ...
  const trackTmp = (u: string) => {
    tmpUrls.current.push(u)
    return u
  }
  ...
  const onVideoFile = useCallback(
    async (file: File) => {
      ...
      if (!tenantId) {
        setNote({ ok: false, msg: 'No tenant context — preview only, cannot upload.' })
        return
      }
      setBusy('video')
      setNote(null)
      try {
        ...
        if (res?.url) {
          patch({
            videoUrl: res.url,
            activeVideoAssetId: res.asset?.id ?? null,
            activeAssetId: res.asset?.id ?? null,
          })
          setNote({
            ok: true,
            msg: `Uploaded ${file.name}. Publish the website to show it publicly.`,
          })
          refresh()
        } else {
  ...
  // ── 2. Image sequence ─────────────────────────────────────────────────────
  const onSequenceFiles = useCallback(
    async (files: FileList) => {
      ...
      try {
        ...
        if (urls.length > 0) {
          ...
          setNote({
            ok: true,
            msg: `Uploaded ${urls.length} frames. Publish the website to show it publicly.`,
          })
          refresh()
        } else {
  ...
  // ── 3. Poster / Fallback ──────────────────────────────────────────────────
  const onPosterFile = useCallback(
    async (file: File) => {
      const tmp = trackTmp(URL.createObjectURL(file))
      patch({ posterUrl: tmp })
      if (!tenantId) return
      ...
      try {
        ...
        if (res?.url) {
          patch({ posterUrl: res.url, posterAssetId: res.asset?.id ?? null })
          refresh()
        }
      } finally {
        setBusy(null)
  ...
  const onFallbackFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        if (res?.url) {
          patch({ fallbackImageUrl: res.url, fallbackAssetId: res.asset?.id ?? null })
          refresh()
        }
      } finally {
        setBusy(null)
  ...
  const activateVideoAsHero = useCallback(
  ...
  const activateSequenceAsHero = useCallback(
    (asset: Website3DAsset) => {
      const frameUrls = Array.isArray(asset.metadata?.frameUrls)
        ...
        : []
      const seqId =
        asset.sequence_id ?? (asset.metadata?.sequenceId as string | undefined) ?? asset.id
      patch({
        renderMode: 'video_scrub',
        useImageSequence: true,
        imageSequenceUrls: frameUrls.length > 1 ? frameUrls : content.imageSequenceUrls,
        activeImageSequenceAssetId: seqId,
        activeAssetId: seqId,
      })
  ...
  return (
    <div style={wrap}>
      ...
      <p style={sub}>
        Upload media here. Files preview instantly in the builder, upload to storage, then go live
        when you Publish. No Spline — MP4 video or image-sequence scrub only.
      </p>
      ...
      <div style={card}>
        ...
        <button
          onClick={() => {
            patch({ renderMode: 'video_scrub', useImageSequence: false })
            patchScrub({ enabled: true, mode: 'video' })
            setNote({ ok: true, msg: 'Using uploaded video as hero.' })
          }}
      ...
      <div style={card}>
        ...
        <input
          ...
          onChange={(e) => {
            if (e.target.files && e.target.files.length) void onSequenceFiles(e.target.files)
            e.currentTarget.value = ''
          }}
        />
        ...
        {frameCount > 0 && content.imageSequenceUrls && (
          ...
          >
            ...
            <img
              src={content.imageSequenceUrls[0]}
              alt="First frame"
              style={{ width: '100%', maxHeight: 150, objectFit: scrub.objectFit }}
            />
          </div>
        ...
        <button
          onClick={() => {
            patch({ renderMode: 'video_scrub', useImageSequence: true })
            patchScrub({ enabled: true, mode: 'image_sequence' })
            setNote({ ok: true, msg: 'Using image sequence as hero.' })
          }}
        ...
        <>
          ...
          {groups.videos.map((a) => (
            <MediaRow
              ...
              actionLabel="Use this video as hero"
              onUse={() => activateVideoAsHero(a)}
            />
          ))}
          {groups.imageSequences.map((a) => {
            ...
            return (
              <MediaRow
                ...
                actionLabel="Use this image sequence as hero"
                onUse={() => activateSequenceAsHero(a)}
              />
            )
          })}
        </>
      ...
      <div style={card}>
        <CheckRow
          ...
          checked={!!scrub.enabled}
          onChange={(v) => {
            patchScrub({ enabled: v })
          }}
        />
        <CheckRow
          label="Pin while scrolling"
          checked={!!scrub.pinOnScroll}
          onChange={(v) => {
        ...
        <div style={{ ...group, marginTop: '0.75rem' }}>
          ...
          <input
            ...
            onChange={(e) => {
              const px = Number(e.target.value) || 1800
              const vh = +(px / 720).toFixed(2)
              patch({ scrollLength: vh })
              patchScrub({ scrollLength: vh })
            }}
          />
        </div>
        <div style={group}>
        ...
        <div style={group}>
          ...
          <Select
            ...
            onChange={(v) => {
              patchScrub({
                reducedMotionFallback: v as VideoScrubSettings['reducedMotionFallback'],
              })
              patch({
      ...
      >
        ...
        <Row
          k="Status"
          v={saveStatus === 'saving' ? 'Saving draft…' : 'Draft saved'}
          good={saveStatus !== 'saving'}
        />
        <Row
          k="Publish"
          v={isPublished ? 'Published' : 'Publish required to go live'}
          good={isPublished}
        />
      </div>
      ...
      >
        ...
        <Diag
          k="active video url"
          v={content.videoUrl ? content.videoUrl.slice(0, 48) + '…' : '—'}
        />
...
function primaryBtn(disabled: boolean): React.CSSProperties {
  return {
    ...
    background: disabled ? '#1c1c1e' : '#c9a84c22',
    color: disabled ? '#52525b' : '#e9d8a6',
    fontSize: '0.74rem',
    fontWeight: 700,
