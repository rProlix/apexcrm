...
const EditorSidebar = dynamic(() => import('./EditorSidebar').then((m) => m.EditorSidebar), {
  ssr: false,
})
...
const AiRestyleDrawer = dynamic(() => import('./AiRestyleDrawer').then((m) => m.AiRestyleDrawer), {
  ssr: false,
})
...
export function EditorShell({ editorCtx }: Props) {
  ...
  const pendingSaves = useRef<
    Map<string, { content: Record<string, unknown>; is_visible?: boolean }>
  >(new Map())

  const executeSave = useCallback(
    async (changedId: string, content: Record<string, unknown>, isVisible?: boolean) => {
    ...
    <>
      ...
      {editMode ? (
        <div style={{ display: 'flex', minHeight: '100vh' }}>
          ...
          {selectedSectionId && <EditorSidebar />}
        </div>
      ) : (
        ...
        <div>
          {sections
            ...
            .map((section) => (
              <ClientSectionRenderer key={section.id} section={section} />
            ))}
        </div>
      )}
    </>
