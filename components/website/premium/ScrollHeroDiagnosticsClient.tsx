...
function detectCaps(): BrowserCaps {
  ...
  const videoScrub =
    !!video.canPlayType && video.canPlayType('video/mp4; codecs="avc1.42E01E"') !== ''
  ...
  return (
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif', color: '#e4e4e7', maxWidth: 900 }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 800, margin: '0 0 0.25rem' }}>
        Premium 3D Scroll Hero — Diagnostics
      </h1>
      <p style={{ color: '#71717a', fontSize: '0.875rem', margin: '0 0 1.5rem' }}>
        No Spline is used. Render modes: Three.js 3D models and H.264 MP4 / image-sequence
        scroll-scrubbing.
      </p>
      ...
      <div style={card}>
        ...
        {!caps ? (
          <p style={{ color: '#71717a', fontSize: '0.8125rem' }}>Detecting…</p>
        ) : (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            ...
            <span style={pill(!caps.reducedMotion)}>
              {caps.reducedMotion ? '⚠' : '✓'} Reduced motion{' '}
              {caps.reducedMotion ? 'ON (static fallback)' : 'off'}
            </span>
            <span style={pill(!caps.coarsePointer)}>
              {caps.coarsePointer ? '📱' : '🖥'} {caps.coarsePointer ? 'Mobile/touch' : 'Desktop'}
            </span>
          </div>
        )}
      </div>
      ...
      <div style={card}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.75rem' }}>
          ...
          <span style={pill(d.summary.sectionsWithMissingAssets === 0)}>
            {d.summary.sectionsWithMissingAssets} missing assets
          </span>
          <span style={pill(d.summary.invalidRenderModes === 0)}>
            {d.summary.invalidRenderModes} invalid render modes
          </span>
          <span style={pill(d.summary.sectionsMissingPoster === 0)}>
            {d.summary.sectionsMissingPoster} missing posters
          </span>
          <span style={pill(d.summary.sectionsMissingFallback === 0)}>
            {d.summary.sectionsMissingFallback} missing fallbacks
          </span>
        </div>
      </div>
      ...
      <div style={card}>
        ...
        {d.sections.length === 0 ? (
          <p style={{ color: '#71717a', fontSize: '0.8125rem' }}>
            No Premium 3D Scroll Hero sections yet.
          </p>
        ) : (
          d.sections.map((s) => (
            <div key={s.sectionId} style={{ padding: '0.75rem 0', borderTop: '1px solid #1c1c1e' }}>
              ...
              {s.renderMode === 'video_scrub' && (
                <p style={{ margin: '0.35rem 0 0', fontSize: '0.7rem', color: '#a1a1aa' }}>
                  ...
                  {' · '}
                  {s.imageSequenceFrameCount > 0
                    ? `${s.imageSequenceFrameCount} frames`
                    : 'no sequence'}
                  {' · '}
              ...
              {s.issues.map((i, idx) => (
                <p
                  key={`i${idx}`}
                  style={{ margin: '0.35rem 0 0', fontSize: '0.75rem', color: '#f87171' }}
                >
                  ✕ {i}
                </p>
              ))}
              {s.warnings.map((w, idx) => (
                <p
                  key={`w${idx}`}
                  style={{ margin: '0.35rem 0 0', fontSize: '0.75rem', color: '#fbbf24' }}
                >
                  ⚠ {w}
                </p>
              ))}
              {s.issues.length === 0 && s.warnings.length === 0 && (
                <p style={{ margin: '0.35rem 0 0', fontSize: '0.75rem', color: '#4ade80' }}>
                  ✓ No issues
                </p>
              )}
            </div>
      ...
      <div style={card}>
        ...
        {d.assets.brokenUrls.map((b) => (
          <p key={b.id} style={{ margin: '0.25rem 0 0', fontSize: '0.75rem', color: '#f87171' }}>
            ✕ {b.name}: {b.reason}
          </p>
        ))}
        {d.assets.largeWarnings.map((b) => (
          <p key={b.id} style={{ margin: '0.25rem 0 0', fontSize: '0.75rem', color: '#fbbf24' }}>
            ⚠ {b.name}: large file ({b.sizeMb} MB)
          </p>
        ))}
      </div>
    </div>
