...
export function AvailabilityEditor({ onSaved }: Props) {
  ...
  function updateRule(idx: number, patch: Partial<DraftRule>) {
    setRules((prev) => prev.map((r, i) => (i === idx ? { ...r, ...patch, _dirty: true } : r)))
  }
      ...
      setRules((prev) =>
        prev.map((r, i) =>
          i === idx
            ? {
                ...saved,
                repeat_days: Array.isArray(saved.repeat_days) ? saved.repeat_days : [],
                _dirty: false,
              }
            : r
        )
            ...
            >
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                  <span className="text-2xs text-white/40 uppercase tracking-wider shrink-0">
                    Hours
                  </span>
                  <input
                    type="time"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    disabled={!rule.is_active}
                    onChange={(e) =>
                      updateRule(idx, { slot_interval_minutes: parseInt(e.target.value) })
                    }
                ...
                {repeatType === 'custom' && (
                  ...
                  >
                    ...
                    {(rule.repeat_days?.length ?? 0) === 0 && (
                      <span className="text-2xs text-amber-400/70 ml-1">
      ...
      {rules.some((r) => r._dirty) && (
        ...
        >
          <p className="text-xs text-gold-400/80">
            {rules.filter((r) => r._dirty).length} unsaved change
            {rules.filter((r) => r._dirty).length !== 1 ? 's' : ''}
          </p>
          <button
            onClick={async () => {
