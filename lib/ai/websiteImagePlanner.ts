  ...
  if (!ctx.sections.length && !ctx.pages.length) {
    return {
      result: {
        ...
        plans: [],
        warnings: [
          'No pages or sections found. Run AI Autofill first to generate website content.',
        ],
      },
    }
  }
  ...
  const plans: ImagePlanItem[] = rawPlans
    ...
    .map((p) => {
      ...
      if (rawRatio && rawRatio !== safeRatio) {
        console.warn(
          `[websiteImagePlanner] Gemini returned unsupported aspect_ratio "${rawRatio}" for section_type "${sectionType}" — normalized to "${safeRatio}"`
        )
      }
      return {
