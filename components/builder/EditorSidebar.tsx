...
import { ScrollExperienceEditor } from './editors/ScrollExperienceEditor'
...
function btn(bg: string, hover: string, extra?: React.CSSProperties): React.CSSProperties {
  return {
    display: 'flex',
    alignItems: 'center',
...
function renderEditor(sectionType: string, sectionId: string) {
  ...
  if (t === 'scroll_experience') return <ScrollExperienceEditor sectionId={sectionId} />
...
function ImageCard({
  ...
  return (
    <div
      style={{
        borderRadius: '0.5rem',
        border: img.is_active && !img.is_archived ? '2px solid #7c3aed' : '1px solid #27272a',
        overflow: 'hidden',
        background: '#0d0d0f',
        ...
        {img.is_archived ? (
          ...
          <>
            ...
            <button
              ...
              disabled={!!loading}
              title="Archive"
              style={btn('#1c1c1e', '#27272a', {
                fontSize: '0.6875rem',
                padding: '0.375rem',
                width: 'auto',
                flex: img.is_active ? 1 : undefined,
              })}
            >
...
function ImageGallery({
  ...
  if (visible.length === 0) {
    return (
      <div
        style={{ padding: '0.75rem 0', textAlign: 'center', color: '#52525b', fontSize: '0.75rem' }}
      >
    ...
    <div>
      {images.some((i) => i.is_archived) && (
        ...
        >
          {showArchived
            ? '▲ Hide archived'
            : `▼ Show archived (${images.filter((i) => i.is_archived).length})`}
        </button>
      )}
    ...
    async (overwrite = false) => {
      ...
      try {
        ...
        if (result.error) {
          ...
          const friendlyErr =
            result.error.includes('aspectRatio') || result.error.includes('aspect_ratio')
        ...
        if (result.updatedSection && result.applied) {
          updateSectionContent(
            result.updatedSection.id,
            result.updatedSection.content as Record<string, unknown>
          )
        }
            ...
            <div style={{ padding: '0 1.25rem 1rem' }}>
              ...
              {aiImage.error && (
                ...
                >
                  ...
                  <button
                    onClick={() => setAiImage((p) => ({ ...p, error: null }))}
                    style={{
                      background: 'none',
                      border: 'none',
                      color: '#f87171',
        ...
        <div style={{ borderBottom: '1px solid #27272a', background: '#0b0b0d' }}>
          ...
          {showPremiumDesign && (
            <div style={{ padding: '1rem 1.25rem 1.25rem' }}>
              <PremiumDesignPanel tenantId={tenantId} initialSectionId={section.id} compact />
            </div>
          )}
        </div>
      )}
      ...
      >
        <p
          style={{
            ...
            fontSize: '0.75rem',
            color:
              saveStatus === 'saving' ? '#f59e0b' : saveStatus === 'saved' ? '#22c55e' : '#52525b',
          }}
        >
