...
export function DomainStatusBadge({ type, status, label, size = 'sm' }: DomainStatusBadgeProps) {
  ...
  if (type === 'verification') {
    const key =
      status === 'true' || status === 'verified' ? 'verified' : (status as VerificationStatus)
  ...
  if (type === 'primary') {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 rounded-full border font-medium ${padClass} bg-[#c9a84c]/15 text-[#c9a84c] border-[#c9a84c]/30`}
      >
  ...
  if (type === 'domain_type') {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 rounded-full border font-medium ${padClass} ${
          isCustom
