...
export default async function TenantPage({ params }: Props) {
  ...
  try {
    ...
    console.error(
      '[TenantPage] getSiteBy* failed for',
      tenantKey,
  ...
  try {
    config = isEditor ? await getDraftSiteConfig(tenantId) : await getPublishedSiteConfig(tenantId)
  } catch (err) {
    console.error(
      '[TenantPage] getConfig failed for tenant',
      tenantId,
      err instanceof Error ? err.message : err
    )
  }
  ...
  if (sections.length === 0) {
    ...
    if (canvaEmbedSrc) {
      return (
        <div>
          <CanvaPreserveEmbed src={canvaEmbedSrc} title={page.title ?? siteData.tenant.name} />
        </div>
      )
    }
    return (
  ...
  const templateConfig =
    (settingsRaw.template_config as Record<string, unknown> | null | undefined) ?? {}
