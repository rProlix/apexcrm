...
export async function ensureCanvaEventWebsiteRecord(params: {
  ...
  if (error || !row)
    return { error: error?.message ?? 'Could not create the event website record.' }

  const urls = eventUrls(slug)
  return {
    ref: { websiteId: row.id, publicSlug: slug, status: 'draft', ...urls, liveUrl: urls.publicUrl },
  }
}
 ...
 */
export async function saveCanvaEventDraft(
  input: SaveCanvaDraftInput
): Promise<SaveCanvaDraftResult> {
  const db = getSupabaseServerClient() as DB
  ...
  if (input.importMode === 'preserve') {
    const validation = validateCanvaEmbedInput(input.canvaUrl ?? input.embedCode, {
      allowCustomDomains: Boolean(input.isCustomDomain),
    })
    if (!validation.ok) {
      return {
        ok: false,
        error:
          validation.reason ?? 'Enter a valid Canva published URL, canva.site link, or embed code.',
      }
    }
  ...
  const ensured = await ensureCanvaEventWebsiteRecord({
    ...
    name: input.name,
    slug: input.slug,
    povEnabled: input.povEnabled,
    povEventId: input.povEventId,
    createdBy: input.createdBy,
  })
  if (!ensured.ref)
    return { ok: false, error: ensured.error ?? 'Could not create the event website record.' }
  const websiteId = ensured.ref.websiteId

  // 3. Load the current draft so undo can restore it later.
  const { data: current } = await db
    .from('websites')
    .select('draft_config, status')
    .eq('id', websiteId)
    .maybeSingle()
  ...
  try {
    ...
    return {
      ok: false,
      error: `Failed to create Canva import row: ${e instanceof Error ? e.message : 'database error'}`,
    }
  }
  ...
  const embedStatus = embedUrl
    ? embedSource.sourceType === 'custom_domain'
      ? 'can_attempt_iframe'
      : 'can_attempt_iframe'
    : 'fallback_required'
  const preservation: 'exact' | 'partial' | 'unknown' =
    input.importMode === 'preserve'
      ? allowCustom
        ? 'partial'
        : embedUrl
          ? 'exact'
  ...
  const draftConfig: Record<string, unknown> = {
    ...
    canvaEmbedCode:
      input.importMode === 'preserve'
        ? buildSafeCanvaIframe(embedUrl ?? input.canvaUrl ?? input.embedCode, {
  ...
  const { data: wasPublished } = await db
    .from('websites')
  ...
  return {
    ...
    status: newStatus,
    draftPreviewUrl: urls.draftPreviewUrl,
    liveUrl: urls.publicUrl,
...
export async function rollbackCanvaEventWebsite(params: {
  ...
  if (params.action === 'restore-last-published') {
    if (!site.published_config)
      return { ok: false, error: 'No published version exists to restore yet.' }
    const { error } = await db
      .from('websites')
      .update({ draft_config: site.published_config })
      .eq('id', websiteId)
      .eq('tenant_id', tenantId)
  ...
  const { data: run } = await db
    .from('website_canva_import_runs')
    .select('*')
    .eq('tenant_id', tenantId)
