...
export async function GET(req: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          type: 'forbidden',
          title: 'Forbidden',
          message: 'Only owners and admins can view the queue.',
        },
      },
      { status: 403 }
    )
  ...
  if (pkgErr) {
    return NextResponse.json(
      {
        ok: false,
        error: { type: 'internal', title: 'Database error', message: pkgErr.message },
      },
      { status: 500 }
    )
  }
  ...
  if (productIds.length > 0) {
    const { data: prodRows } = await db.from('products').select('id, name').in('id', productIds)
  ...
  if (pkgIds.length > 0) {
    ...
    for (const f of (frameRows ?? []) as Record<string, unknown>[]) {
      ...
      if (!frameCountMap[pid])
        frameCountMap[pid] = { total: 0, completed: 0, failed: 0, pending: 0 }
