...
function isFood(subject: NormalizedProductSubject, config: P360GenerationConfig): boolean {
  ...
  return FOOD_CATEGORIES.has(name) || [...FOOD_CATEGORIES].some((kw) => combined.includes(kw))
}
...
const FOOD_TYPE_KEYWORDS: Array<{ type: string; keywords: string[] }> = [
  ...
  {
    type: 'noodle',
    keywords: ['noodle', 'pasta', 'spaghetti', 'fettuccine', 'pad thai', 'mie', 'laksa'],
  },
  { type: 'soup', keywords: ['soup', 'stew', 'bisque', 'chowder', 'broth', 'pottage'] },
  {
    type: 'burger',
    keywords: ['burger', 'hamburger', 'cheeseburger', 'sandwich', 'sub', 'hoagie'],
  },
  ...
  return {
    ...
    lighting: {
      lightingPreset: String(
        lightRaw.lightingPreset ?? config.lightingPreset ?? 'warm_food_commercial'
      ),
    ...
    composition: {
      ...
      backgroundVisibleAmount: String(
        compRaw.backgroundVisibleAmount ?? 'background fills upper half'
      ),
...
function normalizeToppings(
  raw: unknown
): Array<{ name: string; count: string; placement: string; color: string; size?: string }> {
...
export async function buildSceneContract(
  ...
  console.info(
    `[sceneContractBuilder] Planning scene contract for "${subject.name}" (food=${food}, type=${foodType})`
  )
  ...
  if (!data || typeof data !== 'object' || !data.productVariant) {
    console.warn(
      `[sceneContractBuilder] Invalid JSON from Gemini (raw: ${text?.slice(0, 200)}). Using fallback.`
    )
    return buildFallbackContract(subject, config, frameCount, orbitDirection, food, foodType)
  }
...
function buildFallbackContract(
  ...
  return {
    ...
    camera: {
      ...
      orbitDirection,
      angleDegrees: angles,
      forbiddenCameraChanges: ['Do not change camera distance', 'Do not change crop or zoom'],
    },
    lighting: {
