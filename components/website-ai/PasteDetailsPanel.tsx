...
export function PasteDetailsPanel({ tenantId, isOwner, onAnalyzed }: Props) {
  ...
  return (
    <div className="rounded-2xl bg-graphite-800/60 border border-surface-border p-5 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-white/40">
            AI analysis will organize them into structured website content.
          </p>
        </div>
      </div>
      ...
      {/* Source type */}
      <div>
        <label className="text-2xs text-white/40 uppercase tracking-wide block mb-1.5">
          Content type
        </label>
        <select
          value={sourceType}
      ...
      <div>
        ...
        <div className="flex items-center justify-between mt-1.5">
          ...
          <span
            className={cn(
              'text-2xs',
              charCount > charLimit * 0.9 ? 'text-gold-400' : 'text-white/25'
            )}
          >
