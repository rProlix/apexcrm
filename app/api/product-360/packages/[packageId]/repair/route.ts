...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  if (user.role !== 'owner' && user.role !== 'admin') {
    return NextResponse.json(
      { ok: false, errorMessage: 'Only owners and admins may repair packages' },
      { status: 403 }
    )
  }
  ...
  if (!tenantId) {
    return NextResponse.json(
      { ok: false, errorMessage: 'Could not resolve tenant' },
      { status: 400 }
    )
  }
  ...
  if (currentStatus === 'archived') {
    return NextResponse.json(
      {
        ok: false,
        errorMessage: 'Package is archived — unarchive it first',
      },
      { status: 409 }
    )
  }
  ...
  if (currentStatus === 'cancelled') {
    return NextResponse.json(
      {
        ok: false,
        errorMessage: 'Package is cancelled — cannot repair a cancelled package',
      },
      { status: 409 }
    )
  }
  ...
  const totalFrames = Math.max(1, (pkg.target_frame_count as number) || 12)
  ...
  const frameMap = new Map<
    number,
    { image_url: string | null; status: string; generation_started_at: string | null }
  >()
  ...
  const completedFrames = (
    (framesAfter ?? []) as Array<{ frame_index: number; image_url: string | null }>
  ...
  const isBlueprintFailure =
    blueprintBroken ||
    (typeof pkg.generation_error === 'string' &&
      (pkg.generation_error as string).toLowerCase().includes('vessel')) ||
    (typeof pkg.last_error_message === 'string' &&
      (pkg.last_error_message as string).toLowerCase().includes('vessel'))

  const shouldResume =
    REPAIRABLE.has(currentStatus) &&
