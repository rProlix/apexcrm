...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!settings) {
      suggestedFixes.push(
        'No site_settings row found for this tenant. Run a publish to create one.'
      )
    }
    if (!isPublished) {
      suggestedFixes.push(
        'site_settings.is_published = false. The site is unpublished — click Publish in the builder.'
      )
    }
    if (!publishedAt) {
      suggestedFixes.push(
        'site_settings.published_at is null. Run migration 072 and publish once to populate it.'
      )
    }
    if (hasUnpublished) {
      suggestedFixes.push(
        'has_unpublished_changes = true. Publish the site to push draft changes live.'
      )
    }
    if (!designSystem || Object.keys(designSystem).length === 0) {
      suggestedFixes.push(
        'design_system column is empty. Apply a template or run AI Restyle, then publish.'
      )
    }
    ...
    if (draftPages.length > 0 && publishedPages.length === 0) {
      suggestedFixes.push(
        `All ${draftPages.length} page(s) are still in draft. Publish to promote them.`
      )
    }
    ...
    const sectionsWithDesign = allSections.filter((s) => {
      ...
      return (
        sc && typeof sc === 'object' && sc.design && Object.keys(sc.design as object).length > 0
      )
    })

    if (visibleSections.length === 0 && allSections.length > 0) {
      suggestedFixes.push(
        `All ${allSections.length} sections have is_visible=false. Check applySnapshotToWebsiteTables — sections may have been hidden on last publish.`
      )
    }
    ...
    if (isDirty) {
      suggestedFixes.push(
        'website_builder_drafts.dirty = true. A stale draft snapshot exists and may override live edits on next publish. Publish once to clear it (now uses forPublish=true to avoid this).'
      )
    }
    ...
    const { data: versions } = (await db
      .from('site_versions')
      .select(
        'id, status, source, page_count, section_count, published_at, created_at, version_number'
      )
      .eq('tenant_id', tenantId)
    ...
    const { data: cols } = (await db
      ...
      .in('column_name', [
        ...
        'design_system',
      ])) as { data: { column_name: string }[] | null; error: unknown }
    ...
    if (missingCols.length > 0) {
      suggestedFixes.push(
        `Missing columns on site_settings: ${missingCols.join(', ')}. Run migration 072_fix_website_publish_state.sql.`
      )
    }
    ...
    if (!authIdValid) {
      suggestedFixes.push(
        'ctx.auth_id is not a valid UUID. Cannot safely write site_versions.created_by.'
      )
    }
    ...
    if (tenantId && authIdValid) {
      try {
        const { data: nextNum } = (await db.rpc('get_next_site_version_number', {
        ...
        if (testInsertErr) {
          ...
          suggestedFixes.push(
            `Checkpoint insert test FAILED: ${testInsertErr.message ?? 'unknown error'}. ` +
    ...
    return NextResponse.json({
      ...
      authCheck: {
        ...
        noteIfFalse:
          'If profileIdDifferentFromAuthId=false, ctx.auth_id and ctx.id are the same UUID (unusual)',
        requirementNote:
          'site_versions.created_by REFERENCES auth.users(id). Must use ctx.auth_id, not ctx.id.',
      },
