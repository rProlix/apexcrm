...
export function LockedModuleCard({ moduleKey, lockedReason, className }: LockedModuleCardProps) {
  ...
  return (
    ...
    >
      ...
      <h2 className="text-lg font-bold text-white mb-2">{catalog?.label ?? moduleKey} is locked</h2>
...
export function ModuleGuard({
  ...
  if (isLocked) {
    return (
      <LockedModuleCard moduleKey={moduleKey} lockedReason={lockedReason} className={className} />
    )
  }
  return <>{children}</>
}
