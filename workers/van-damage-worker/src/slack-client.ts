...
export async function getSlackFileInfo(token: string, fileId: string): Promise<SlackFileInfo> {
  ...
  if (!payload.ok || !file?.id || !file.mimetype)
    throw new Error(`Slack files.info failed: ${payload.error ?? 'invalid_file'}`)
  if (
...
export async function downloadSlackImage(
  ...
  if (file.size != null && file.size > maxBytes)
    throw new PermanentSlackFileError(`Image ${file.id} exceeds the configured size limit`)
