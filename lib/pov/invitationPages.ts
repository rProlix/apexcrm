 ...
 */
export async function ensureInvitationPages(
  tenantId: string,
  opts: InvitationOpts = {}
): Promise<boolean> {
  ...
  const dateLine = opts.eventDate
    ? new Date(`${opts.eventDate}T00:00:00`).toLocaleDateString(undefined, {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric',
    ...
    : ''
  const cameraHref =
    opts.povEnabled && opts.eventSlug ? `${appBase()}/pov/${opts.eventSlug}/camera` : ''
  const galleryHref =
    opts.povEnabled && opts.eventSlug ? `${appBase()}/pov/${opts.eventSlug}/gallery` : ''
  ...
  try {
    ...
    if (homeId) {
      await addSection(
        ...
        'hero',
        {
          ...
          ...(galleryHref
            ? { ctaSecondaryLabel: 'View Gallery', ctaSecondaryHref: galleryHref }
            : {}),
          align: 'center',
        },
        0
    ...
    console.warn(
      '[pov:invitationPages] could not scaffold pages:',
