...
export const MODULE_CATALOG: Record<CRMModuleKey, ModuleCatalogEntry> = {
  ...
  leads: {
    key: 'leads',
    label: 'Leads',
    description: 'Lead pipeline and conversion tracking',
    icon: '🎯',
    isPremium: false,
    minPlan: 'starter',
  },
  ...
  website: {
    ...
    description: 'Build and publish your business website',
    icon: '🌐',
    isPremium: false,
    minPlan: 'starter',
  },
  ...
  payments: {
    key: 'payments',
    label: 'Payments',
    description: 'Accept payments via Stripe or Square',
    icon: '💳',
    isPremium: false,
    minPlan: 'growth',
  },
  rewards: {
    ...
    description: 'Customer loyalty programs and points',
    icon: '⭐',
    isPremium: false,
    minPlan: 'growth',
  },
  ...
  messages: {
    key: 'messages',
    label: 'Messages',
    description: 'Messaging center for customer comms',
    icon: '💬',
    isPremium: false,
    minPlan: 'growth',
  },
  staff: {
    ...
    description: 'Staff accounts, roles, and scheduling',
    icon: '👔',
    isPremium: false,
    minPlan: 'growth',
  },
  ...
  product_360: {
    ...
    description: 'Interactive 360° product spin packages',
    icon: '🔄',
    isPremium: true,
    minPlan: 'pro',
  },
  damage_ai: {
    key: 'damage_ai',
    label: 'AI Analysis',
    description: 'AI-powered damage and image analysis',
    icon: '🤖',
    isPremium: true,
    minPlan: 'pro',
  },
  ...
  ai_website_builder: {
    ...
    description: 'Generate website pages with AI',
    icon: '✨',
    isPremium: true,
    minPlan: 'pro',
  },
  ai_images: {
    ...
    description: 'Generate and enhance images with AI',
    icon: '🎨',
    isPremium: true,
    minPlan: 'pro',
  },
...
export function explainPlanRecommendation(planKey: CRMPlanKey, answers: OnboardingAnswers): string {
  ...
  if (planKey === 'starter') {
    reasons.push(
      'Your business is just getting started and needs core customer and appointment management.'
    )
    ...
    if (reasons.length === 0)
      reasons.push('Growth unlocks payments, rewards, and an online store for your business.')
  } else if (planKey === 'pro') {
    ...
    if (reasons.length === 0)
      reasons.push('Pro includes all AI tools, 360 product studio, and advanced analytics.')
  } else {
    reasons.push(
      'Your business needs custom limits, multi-location support, or custom integrations.'
    )
  }
...
export function getRecommendedModulesFromAnswers(answers: OnboardingAnswers): CRMModuleKey[] {
  ...
  if (
    answers.needsStore ||
    matchesCategory(bt, ECOMMERCE_BUSINESS_TYPES) ||
    answers.sellsProducts
  ) {
    modules.add('store')
    modules.add('payments')
  }
  ...
  if (
    ...
    matchesCategory(bt, [...SALON_BUSINESS_TYPES, ...RESTAURANT_BUSINESS_TYPES])
  ) {
    modules.add('rewards')
  }
  ...
  if (
    ...
    matchesCategory(bt, [...HEALTH_BUSINESS_TYPES, ...FITNESS_BUSINESS_TYPES])
  ) {
...
export function getRecommendedPlanFromAnswers(answers: OnboardingAnswers): CRMPlanKey {
  // Enterprise signals
  const isEnterprise =
    (answers.employeeCount ?? 0) > 50 || (answers.expectedMonthlyCustomers ?? 0) > 5000
