...
export function RewardsShopItemForm({ tenantId, products, editItem, onClose }: Props) {
  ...
  const [maxPerCustomer, setMaxPerCustomer] = useState<number | ''>(
    editItem?.max_redemptions_per_customer ?? ''
  )
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (pointsCost <= 0) {
      setError('Points cost must be positive')
      return
    }
    setError('')
        ...
        >
          <div className="flex items-center gap-3 mb-1">
            ...
            <h3 className="text-sm font-semibold text-white">
              {editItem ? 'Edit Shop Item' : 'New Shop Item'}
            </h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setMaxPerCustomer(e.target.value === '' ? '' : Number(e.target.value))
                }
