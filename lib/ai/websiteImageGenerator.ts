...
import { mergeNegativePromptIntoPrompt, stripUnsupportedImagenFields } from '@/lib/ai/promptSafety'
...
export async function generateWebsiteImage(
  ...
  if (jobErr || !job) {
    ...
    if (isFkCreatedByError(jobErr)) {
      const fkMsg =
        'Image job creation failed: created_by foreign key violation. Re-run migration 054 to make created_by nullable, or ensure auth_id is a valid auth.users UUID.'
  ...
  const basePrompt = enhancePromptForImagen(
    opts.plan.prompt,
  ...
  const safeAspectRatio = normalizeImagenAspectRatio(opts.plan.aspect_ratio, opts.plan.section_type)
  ...
  if (!response.ok) {
    ...
    logError('4_imagen_response_error', new Error(errMsg), {
      jobId,
      planId: opts.plan.id,
      status: response.status,
    })
  ...
  try {
    ...
    await failJob(
      ...
      jobId,
      opts.plan.id,
      'AI image generation returned unreadable data.',
      null
    )
    return {
      jobId,
      ...
      storagePath: '',
      altText: '',
      error: 'AI image generation returned unreadable data.',
    }
  }
  ...
  if (extractError || !b64) {
    ...
    logError('5_no_image_data', new Error(errMsg), {
      jobId,
      planId: opts.plan.id,
  ...
  if (opts.plan.section_id) {
    const { error: galleryErr } = await db.from('website_section_images').insert({
      tenant_id: opts.tenantId,
    ...
    if (galleryErr) {
      ...
      console.warn(
        '[websiteImageGenerator] Failed to insert website_section_images row:',
        galleryErr.message
      )
    } else {
...
function extractImageFromImagenResponse(json: Record<string, unknown>): {
  ...
  const b64 = (first.bytesBase64Encoded ??
    ...
    first.imageBytes ??
    first.image_bytes) as string | undefined

  const mimeType =
    ((first.mimeType ?? first.mime_type ?? first.imageType ?? first.image_type) as
      | string
      | undefined) ?? 'image/png'
