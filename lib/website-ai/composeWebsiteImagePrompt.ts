...
export function composeWebsiteImagePrompt(
  ...
  const environmentStatement = brief.environment ? `Set in: ${brief.environment}.` : ''
  ...
  const avoidanceStatement =
    brief.shouldAvoid.length > 0 ? `Do not show: ${brief.shouldAvoid.join(', ')}.` : ''
...
function getSectionIntro(sectionType: string, businessType: string, businessName: string): string {
  ...
  return (
    introMap[sectionType] ??
    `Professional website image for ${businessName}, a ${businessType} business`
  )
}
...
function buildQualityModifiers(brief: SectionImageBrief): string {
  const baseQuality =
    'professional commercial photography, high resolution, 8K quality, sharp focus'
...
function buildIndustryStyle(businessType: string): string {
  const styleMap: Record<string, string> = {
    restaurant:
      'Food photography with natural warm lighting, appetizing plating, fresh ingredients visible, bokeh background',
    auto_shop:
      'Automotive photography with dramatic studio or outdoor lighting, mirror-like surface reflections, premium presentation',
    salon:
      'Beauty photography with bright clean lighting, elegant atmosphere, premium salon environment, soft diffused light',
    contractor:
      'Construction/trade photography showing quality craftsmanship, professional tools, and skilled workmanship',
    plumber:
      'Professional service photography with clean uniform, modern equipment, trustworthy skilled-tradesperson presentation',
    medical:
      'Clean clinical photography with bright natural light, professional medical environment, calming trustworthy atmosphere',
    fitness:
      'Action-oriented fitness photography, dynamic energy, motivational gym environment, athletic subjects in motion',
    car_rental:
      'Automotive fleet photography, clean outdoor backdrop, premium vehicle presentation, aspirational lifestyle',
    ecommerce:
      'Clean commercial product photography with neutral backgrounds, precise focus on product details',
    salon_spa:
      'Spa and beauty photography with serene calming atmosphere, soft lighting, luxurious textures',
    general:
      'Clean professional business photography, modern environment, trustworthy presentation',
  }
...
export function buildSectionSpecificPrompt(
  ...
  const guardrail =
    'Do not show: text overlays, watermarks, logos, unrelated industries, generic stock photos.'

  const subject =
    subjectMap[sectionType] ??
    `Professional image for ${businessName}, ${bt} section: ${sectionType}.`

  return [
    subject,
    industryStyle,
    'professional commercial photography, high resolution, website-ready.',
    guardrail,
  ].join(' ')
}
