        ...
        {items.slice(0, 3).map((item, i) => (
          <div key={i} className="rounded-lg bg-graphite-700/40 border border-white/6 p-3">
            ...
            <p className="text-xs text-white/50 italic">
              &ldquo;{String(item.text ?? item.quote ?? '')}&rdquo;
            </p>
          </div>
        ))}
      </div>
  ...
  if (suggestionType === 'hero' || suggestionType === 'about') {
    return (
      <div className="mt-2 rounded-lg bg-graphite-700/40 border border-white/6 p-3">
        ...
        {!!ps.subheadline && <p className="text-xs text-white/50">{String(ps.subheadline)}</p>}
        {!!ps.body && <p className="text-xs text-white/50">{String(ps.body)}</p>}
      </div>
    )
  }
  ...
  if (suggestionType === 'contact' || suggestionType === 'hours') {
    ...
    return (
      <div className="mt-2 rounded-lg bg-graphite-700/40 border border-white/6 p-3 space-y-1">
        {fields.map((f) =>
          ps[f] ? (
            <div key={f} className="flex items-start gap-2">
              <span className="text-2xs text-white/30 uppercase tracking-wide min-w-[48px]">
                {f}
              </span>
        ...
        {Array.isArray(ps.hours) &&
          (ps.hours as Array<Record<string, unknown>>).slice(0, 4).map((h, i) => (
