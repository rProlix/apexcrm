...
export async function GET(_req: NextRequest, { params }: RouteCtx) {
  ...
  const [guests, photos, videos, audio, total, pending, reported, hidden] = await Promise.all([
    ...
    count('pov_media', (q) =>
      q.eq('event_id', eventId).eq('media_type', 'photo').neq('status', 'deleted')
    ),
    count('pov_media', (q) =>
      q.eq('event_id', eventId).eq('media_type', 'video').neq('status', 'deleted')
    ),
    count('pov_media', (q) =>
      q.eq('event_id', eventId).eq('media_type', 'audio').neq('status', 'deleted')
    ),
