...
interface Props {
  ...
  onStatusChange?: (
    status: CanvaEmbedStatus,
    info: { iframeSrc: string | null; reason?: string }
  ) => void
}
...
export function CanvaPreserveEmbed({
  ...
  const [status, setStatus] = useState<CanvaEmbedStatus>(
    resolved.iframeSrc ? 'loading' : 'no_source'
  )
  const frameRef = useRef<HTMLIFrameElement>(null)

  useEffect(() => {
    if (!resolved.iframeSrc) {
      setStatus('no_source')
      return
    }
    setStatus('loading')
  ...
  return (
    <section style={{ background: 'var(--color-bg)', position: 'relative' }}>
      ...
      {showFullFallback && (
        <div
          style={{
            ...
            padding: '3rem 1rem',
            textAlign: 'center',
            minHeight: ratioMode ? undefined : '40vh',
        ...
        >
          ...
          <p style={{ color: 'var(--color-muted)', fontSize: '0.8125rem', maxWidth: 520 }}>
            The Canva site or its custom domain blocks embedding. You can open it in a new tab —
            Event Camera and Gallery still work below.
          </p>
      ...
      {showNativeActions && actions.length > 0 && (
        ...
        >
          {actions.map((a, i) => (
            <a key={a.label} href={a.href} style={i === 0 ? primaryCta : secondaryCta}>
              {a.label}
            </a>
          ))}
        </div>
