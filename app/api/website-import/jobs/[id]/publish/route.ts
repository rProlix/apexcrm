...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!results?.length) {
    return NextResponse.json(
      { error: 'No approved results to apply. Approve fields first.' },
      { status: 400 }
    )
  }
  ...
  for (const result of results) {
    switch (result.result_key) {
      case 'businessName':
        settingsUpdates.site_name = result.result_value
        break
      case 'logoUrl':
        settingsUpdates.logo_url = result.result_value
        break
      case 'faviconUrl':
        settingsUpdates.favicon_url = result.result_value
        break
      case 'brandColors':
        settingsUpdates.brand_colors = result.result_value
        break
      case 'seoTitle':
  ...
  if (auto_publish && !apply_only) {
    await db.from('site_settings').update({ is_published: true }).eq('tenant_id', tenantId)
