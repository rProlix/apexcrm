...
export function SiteFooter({ config }: Props) {
  ...
  const copyright =
    footerCfg?.copyright ??
    `© ${new Date().getFullYear()} ${settings.site_name ?? 'Store'}. All rights reserved.`
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            {footerCfg?.showSocials && (
              <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.25rem' }}>
                {socials.twitter && (
                  <SocialLink href={`https://twitter.com/${socials.twitter}`} label="Twitter" />
                )}
                {socials.instagram && (
                  <SocialLink href={`https://instagram.com/${socials.instagram}`} label="IG" />
                )}
                {socials.facebook && (
                  <SocialLink href={`https://facebook.com/${socials.facebook}`} label="FB" />
                )}
                {socials.linkedin && (
                  <SocialLink href={`https://linkedin.com/in/${socials.linkedin}`} label="LI" />
                )}
              </div>
            )}
          </div>
        ...
        >
          <p style={{ fontSize: '0.75rem', color: 'var(--color-muted)', margin: 0 }}>{copyright}</p>
