...
export function ConfidenceBadge({ confidence, size = 'sm' }: Props) {
  ...
  const { label, classes } =
    pct >= 85
      ? {
          label: `${pct}%`,
          classes: 'bg-emerald-500/12 text-emerald-400 border-emerald-500/20 shadow-emerald-500/10',
        }
      : pct >= 60
        ? {
            label: `${pct}%`,
            classes: 'bg-gold-500/12 text-gold-400 border-gold-500/20 shadow-gold-500/10',
          }
