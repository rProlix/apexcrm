...
export async function POST(req: NextRequest) {
  ...
  try {
    const pkg = await createPackage({
      ...
      name,
      label:
        (body.label as string | null | undefined) ??
        ((body.preset as string | undefined)?.trim() || null),
      ...
      startsAt:
        ((body.starts_at ?? body.startsAt ?? body.promo_starts_at) as string | null) ?? null,
      ...
      generationProvider: (body.generationProvider ?? body.generation_provider) as
        | 'gemini'
        | 'leonardo'
        | undefined,
      generationMode: (body.generationMode ?? body.generation_mode) as
        | 'text_to_image'
        | 'reference_image'
        | undefined,
      referenceImageRequired: !!(body.referenceImageRequired ?? body.reference_image_required),
      consistencyMode: (body.consistencyMode ?? body.consistency_mode) as
        | 'standard'
        | 'strict'
        | 'ultra_strict'
        | undefined,
    ...
    if (err instanceof P360ApiError) {
      ...
      const status =
        ae.type === 'not_found'
          ...
          : ae.type === 'forbidden'
            ? 403
            : ae.type === 'validation_error' || ae.type === 'constraint_error'
              ? 422
              : 500
