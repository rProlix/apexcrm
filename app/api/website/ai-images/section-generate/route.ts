...
export async function POST(req: NextRequest) {
  ...
  const sectionDetail: RichSectionDetail =
    richCtx.sectionDetails.find((s) => s.id === sectionId) ??
  ...
  const aspectRatioNote = getAspectRatioNormalizationNote(
  ...
  if (planInsertErr || !plan) {
    if (isSchemaCacheError(planInsertErr)) {
      ...
      console.error(
        '[AI-IMAGE][section-generate] Schema/table error on plan insert:',
        planInsertErr?.message
      )
      return NextResponse.json(
        {
    ...
    if (isFkCreatedByError(planInsertErr)) {
      ...
      return NextResponse.json(
        {
          error:
            'Plan insert failed: created_by foreign key violation. Re-run migration 054 to make created_by nullable.',
          code: 'FK_CREATED_BY',
          detail: planInsertErr?.message,
        },
        { status: 500 }
    ...
    return NextResponse.json(
      { error: planInsertErr?.message ?? 'Failed to create plan.' },
      { status: 500 }
    )
  }
...
function buildFallbackSectionDetail(section: {
  ...
  return {
    ...
    imageUrl:
      firstString(c.backgroundImage, c.background_image, c.imageUrl, c.image_url, c.image) ?? null,
    content: c,
  }
}
...
function extractItemStrings(c: Record<string, unknown>): string[] {
  ...
  return c.items
    .map((i: unknown) => {
      const o =
        i && typeof i === 'object' && !Array.isArray(i) ? (i as Record<string, unknown>) : {}
