...
const LIGHTING_DESCRIPTIONS: Record<string, string> = {
  // ── New presets ──
  luxury_softbox:
    'large premium softboxes providing silky wrap-around light, ultra-soft shadows, luxury feel',
  gold_rim_light: 'warm gold rim backlighting, editorial luxury look, golden glow on product edges',
  clean_ecommerce_white:
    'bright flat white studio lighting, minimal shadows, clean Amazon-style product presentation',
  dramatic_black_studio:
    'dark studio background with high-contrast selective rim light, dramatic shadows',
  natural_window_light:
    'soft natural daylight streaming from one side, gentle realistic shadows, airy atmosphere',
  neon_showcase: 'vibrant neon accent lighting, colorful rim lights, futuristic product showcase',
  warm_restaurant_tabletop:
    'warm golden ambient restaurant lighting, cozy editorial food photography style',
  automotive_showroom:
    'crisp dealership-quality studio lighting, reflective industrial surfaces, sharp highlights',
  jewelry_macro_shine:
    'intense macro sparkle lighting, brilliant facet highlights, gem fire and crystal clarity',
  matte_product_soft_glow:
    'soft diffused warm glow, matte surface textures, subtle depth and dimension',
  ...
  luxury_dramatic:
    'dramatic high-contrast luxury lighting, deep shadows, rich highlights, editorial feel',
  retail_bright:
    'bright retail-optimized lighting, clear product visibility, commercial presentation',
  ...
  glossy_reflective:
    'glossy reflective studio lighting, sharp specular highlights, reflective surface sheen',
...
const BACKGROUND_DESCRIPTIONS: Record<string, string> = {
  ...
  transparent_isolated:
    'clean neutral background optimized for background removal and composite use',
  ...
  restaurant_tabletop:
    'restaurant tabletop setting, marble or slate surface with soft ambient light',
...
const CATEGORY_DESCRIPTORS: Record<string, string> = {
  ...
  beverage_cup:
    'beverage in a cup or container, condensation details if applicable, drink photography standards',
  apparel:
    'apparel item laid flat or on invisible form, wrinkle-free, fashion photography standards',
  cosmetics: 'cosmetic or beauty product, luxury beauty product photography standards',
  electronics:
    'consumer electronics product, tech product photography standards, clean industrial feel',
...
const CAMERA_DESCRIPTIONS: Record<string, string> = {
  eye_level_product:
    'camera at product mid-height, straight-on eye-level angle, classic e-commerce framing',
  slight_top_down: 'camera tilted gently downward ~15°, shows top surface of the product',
  hero_low_angle:
    'low heroic camera angle looking slightly upward, dramatic powerful product presence',
  macro_detail: 'close macro framing, rich material texture detail, fills the frame',
  floating_catalog_view:
    'slight elevated floating catalog angle, isolated product on clean background',
  // Legacy
...
function buildProductIdentityBlock(
  ...
  lines.push(
    `  CRITICAL: same shape, same proportions, same materials, same colors, same labels/branding, same scale in every single frame.`
  )
  return lines.join('\n')
}
...
function buildStyleBlock(config: P360GenerationConfig): string {
  ...
  if (config.shadowStrength !== null) {
    const shadowDesc =
      config.shadowStrength < 0.3 ? 'minimal' : config.shadowStrength > 0.7 ? 'strong' : 'moderate'
    parts.push(`  Shadows: ${shadowDesc} intensity product shadow`)
  }
  if (config.reflectionIntensity !== null) {
    const refDesc =
      config.reflectionIntensity < 0.3
        ? 'no reflection'
        : config.reflectionIntensity > 0.7
