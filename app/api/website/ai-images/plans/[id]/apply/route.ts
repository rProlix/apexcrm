...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!typedPlan.generated_asset_url)
    return NextResponse.json(
      { error: 'No generated image to apply. Generate first.' },
      { status: 422 }
    )
  ...
  updateDraftAfterImageApply(
    tenantId,
    ctx.id ?? '',
    'ai_images',
    `AI image applied to section ${typedPlan.section_id} (plan ${planId})`
  )
...
function updateDraftAfterImageApply(
  ...
  Promise.resolve().then(async () => {
    try {
      ...
      console.warn(
        '[ai-images] Background draft update failed (non-fatal):',
