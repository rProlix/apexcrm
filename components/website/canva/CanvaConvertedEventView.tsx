...
import {
  CanvaPdfPageVisualSection,
  type CanvaPdfPageVisualConfig,
} from './CanvaPdfPageVisualSection'
...
function CtaLink({ href, label, eventSlug }: { href: string; label: string; eventSlug?: string }) {
  ...
  if (resolved.startsWith('/') && !/^https?:\/\//i.test(resolved)) {
    return (
      <Link href={resolved} style={cta}>
        {label}
      </Link>
    )
  }
  return (
    <a href={resolved} style={cta} target="_blank" rel="noopener noreferrer">
      {label}
    </a>
  )
}
...
function SectionView({
  ...
  const reveal = (node: React.ReactNode) => (
    <AnimatedReveal
      preset={anim.preset as NexoraAnimationPreset}
      delay={anim.delay}
  ...
  if (section.section_type === 'canva_pdf_page_visual') {
    ...
    if (cfg.renderedImageUrl)
      return <CanvaPdfPageVisualSection config={{ ...cfg, type: 'canva_pdf_page_visual' }} />
    return null
  }
  ...
  switch (section.section_type) {
      ...
      return (
        <section
          style={{
            ...
            textAlign: 'center',
            overflow: 'hidden',
            background: bg ? `url(${bg}) center/cover no-repeat` : 'var(--color-bg)',
          }}
        >
          ...
          <div style={{ ...wrap, position: 'relative', zIndex: 2 }}>
            {reveal(
              <>
                ...
                {str(c.ctaLabel) && ctaHref && ctaHref !== '#' && (
                  <div style={{ marginTop: '1.75rem' }}>
                    <CtaLink href={ctaHref} label={str(c.ctaLabel)} eventSlug={eventSlug} />
                  </div>
                )}
              </>
            )}
        ...
        <section style={{ padding: '4rem 0', background: 'var(--color-bg)' }}>
          <div style={wrap}>
            ...
            <div
              style={{
                display: 'grid',
                gridTemplateColumns: `repeat(auto-fit,minmax(${cols >= 3 ? 220 : 280}px,1fr))`,
                gap: '1.25rem',
              }}
            >
              {items.map((it, i) => (
                <AnimatedReveal
                  key={i}
                  preset={anim.preset as NexoraAnimationPreset}
                  delay={(anim.delay ?? 0) + i * (anim.stagger ?? 0.08)}
                  duration={anim.duration}
                >
                  <div
                    style={{
                  ...
                  >
                    <h3 style={{ color: 'var(--color-text)', fontWeight: 600, fontSize: '1.1rem' }}>
                      {str(it.title)}
                    </h3>
                    <p
                      style={{
    ...
    case 'cta': {
      ...
      if ((!ctaHref || ctaHref === '#') && str(c.ctaLabel)) {
        const mapped = linkMapping?.find(
          (l) => l.label.toLowerCase() === str(c.ctaLabel).toLowerCase() && !l.dead
        )
        if (mapped) ctaHref = mapped.href
      }
      return (
        ...
        <section style={{ padding: '3rem 0', background: 'var(--color-bg)' }}>
          <div style={wrap}>
            {reveal(
              str(c.html) ? (
                <div
                  style={{ color: 'var(--color-text)', opacity: 0.85, lineHeight: 1.7 }}
...
export function CanvaConvertedEventView({
  title,
  sections,
  pages,
  theme,
  ...
  const homeSections =
    sections ??
    pages?.find((p) => p.slug === 'home' || !p.slug)?.sections ??
    pages?.[0]?.sections ??
    []

  return (
    <main
      style={{ minHeight: '100vh', background: 'var(--color-bg,#0b0b0b)', ...themeVars(theme) }}
    >
      {isDraftPreview && (
        <div
          style={{
