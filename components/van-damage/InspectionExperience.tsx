...
type Vehicle = {
  id: string; name: string; van_number: string | null; make: string | null; model: string | null
  year: number | null; color: string | null; plate_number: string | null; vin: string | null
  status: string; metadata: RecordValue
} | null
type Inspection = {
  id: string; title: string | null; status: string; review_status: string; source: string
  image_count: number; damage_count: number; ai_summary: string | null; ai_confidence: number | null
  ai_model: string | null; slack_team_id: string | null; slack_channel_id: string | null
  slack_message_ts: string | null; slack_user_id: string | null; van_id: string | null
  metadata: RecordValue; created_at: string; updated_at: string; completed_at: string | null
  reviewed_at: string | null
}
type AiRun = {
  id: string; status: string; provider: string; model: string | null; prompt_version: string | null
  input_summary: RecordValue; parsed_response: RecordValue; created_at: string; completed_at: string | null
} | null
type Job = {
  status: string; attempt_count: number; created_at: string; started_at: string | null
  completed_at: string | null; payload: RecordValue
} | null
...
const severityPresentation = {
  minor: { label: 'Minor', color: 'emerald', classes: 'border-emerald-400/20 bg-emerald-400/10 text-emerald-200' },
  moderate: { label: 'Moderate', color: 'amber', classes: 'border-amber-400/20 bg-amber-400/10 text-amber-200' },
  major: { label: 'Major', color: 'orange', classes: 'border-orange-400/20 bg-orange-400/10 text-orange-200' },
  critical: { label: 'Critical', color: 'red', classes: 'border-red-400/20 bg-red-400/10 text-red-200' },
} as const
...
export function InspectionExperience(props: InspectionExperienceProps) {
  ...
  const maxSeverity = items.reduce((best, item) => severityRank[item.severity ?? 'unknown'] > severityRank[best] ? item.severity ?? 'unknown' : best, 'unknown')
  const severityKey: keyof typeof severityPresentation =
    maxSeverity === 'critical' ? 'critical' : rating === 3 || maxSeverity === 'high' ? 'major' : rating === 2 || maxSeverity === 'medium' ? 'moderate' : 'minor'
  const severity = severityPresentation[severityKey]
  const needsReview = inspection.status === 'needs_review' || parsed.needsHumanReview === true || inspection.review_status === 'in_review'
  const safetyConcern = maxSeverity === 'critical' || items.some((item) => ['broken_light', 'broken_mirror', 'glass_damage', 'tire_wheel_damage'].includes(item.damage_type ?? ''))
  const safeToRent = rating == null ? 'Pending' : rating <= 1 && !safetyConcern ? 'Likely safe' : rating === 2 && !safetyConcern ? 'Review first' : 'Hold for review'
  const outOfService = maxSeverity === 'critical' ? 'Recommended' : rating === 3 ? 'Assess before use' : 'Not indicated'
  ...
  const panels = [...new Set(items.map((item) => humanize(item.vehicle_area)).filter((value) => value !== 'Unknown'))]
    ...
    <section className="overflow-hidden rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_80%_0%,rgba(201,168,76,.12),transparent_32%),linear-gradient(135deg,#18181b,#101012)] shadow-panel-lg">
      <div className="border-b border-white/8 px-5 py-4 md:px-7">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            <Link href={`/dashboard/damage-ai?businessId=${encodeURIComponent(props.businessId)}`} className="focus-ring rounded-xl border border-white/10 p-2 text-white/55 hover:bg-white/5 hover:text-white" aria-label="Back to inspections"><ArrowLeft className="h-4 w-4" /></Link>
            <div className="min-w-0">
              <p className="truncate text-xs font-medium uppercase tracking-[.18em] text-gold-300/75">AI fleet inspection</p>
              <h1 className="mt-1 truncate text-xl font-semibold text-white md:text-2xl">{inspection.title || 'Van damage inspection'}</h1>
            </div>
          </div>
      ...
      <div className="grid lg:grid-cols-[.85fr_1.3fr_1fr]">
        <div className="border-b border-white/8 p-5 md:p-7 lg:border-b-0 lg:border-r">
          <p className="text-xs font-medium uppercase tracking-[.16em] text-white/35">Inspection overview</p>
          <div className="mt-5 flex items-end gap-3">
            <ConfidenceRing value={confidence} />
            <div className="pb-1">
              <p className="text-xs text-white/35">AI confidence</p>
              <p className="text-lg font-semibold text-white">{confidence == null ? 'Pending' : confidence >= .8 ? 'High confidence' : confidence >= .55 ? 'Medium confidence' : 'Low confidence'}</p>
            </div>
          </div>
        ...
        <div className="border-b border-white/8 p-5 md:p-7 lg:border-b-0 lg:border-r">
          <div className="flex items-center justify-between">
            <p className="text-xs font-medium uppercase tracking-[.16em] text-white/35">Vehicle profile</p>
            <span className="inline-flex items-center rounded-full bg-white/5 px-2.5 py-1 text-[10px] capitalize text-white/55"><span className={`mr-1.5 h-1.5 w-1.5 rounded-full ${vehicle?.status === 'active' ? 'bg-emerald-400' : 'bg-amber-400'}`} />{vehicle?.status || 'unmatched'}</span>
          </div>
          <div className="mt-4 grid grid-cols-[110px_1fr] gap-5">
            <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[.07] to-white/[.02]">
              <VanIllustration />
              <span className="absolute bottom-2 rounded-full bg-black/35 px-2 py-1 text-[9px] text-white/40 backdrop-blur">Fleet vehicle</span>
            </div>
            <div className="min-w-0">
              <h2 className="truncate text-xl font-semibold text-white">{vehicle?.name || 'Vehicle pending match'}</h2>
              <p className="mt-1 text-sm text-white/45">{[vehicle?.year, vehicle?.make, vehicle?.model].filter(Boolean).join(' ') || 'Vehicle details unavailable'}</p>
        ...
        <div className="p-5 md:p-7">
          <div className="flex items-center justify-between">
            <p className="text-xs font-medium uppercase tracking-[.16em] text-white/35">AI verdict</p>
            <Sparkles className="h-4 w-4 text-gold-300" />
          </div>
          <p className="mt-4 text-2xl font-semibold capitalize text-white">{humanize(asText(parsed.vehicleCondition, 'Analysis pending'))}</p>
          <p className="mt-1 text-sm capitalize text-white/45">{rating == null ? 'Damage rating pending' : `${rating}/3 · ${humanize(asText(parsed.damageRatingLabel))}`}</p>
          <div className="mt-5 grid grid-cols-3 gap-2">
            <VerdictState label="Safe to rent" value={safeToRent} tone={safeToRent === 'Likely safe' ? 'good' : 'warn'} />
            <VerdictState label="Needs review" value={needsReview ? 'Yes' : 'No'} tone={needsReview ? 'warn' : 'good'} />
      ...
      <main className="min-w-0 space-y-6">
        <section className="rounded-2xl border border-white/10 bg-graphite-800 p-5 md:p-6">
          <div className="flex items-start gap-4">
            <div className="rounded-xl border border-gold-400/20 bg-gold-400/10 p-2.5"><Bot className="h-5 w-5 text-gold-300" /></div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-medium uppercase tracking-[.16em] text-gold-300/70">Gemini damage summary</p>
              <p className="mt-3 text-base leading-7 text-white/75">{inspection.ai_summary || asText(parsed.summary, 'Analysis has not completed yet.')}</p>
            </div>
          </div>
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <SummaryMetric icon={PanelTop} label="Detected damage" value={`${inspection.damage_count} finding${inspection.damage_count === 1 ? '' : 's'}`} />
            <SummaryMetric icon={MapPin} label="Panels affected" value={panels.length ? panels.join(', ') : 'None identified'} />
            <SummaryMetric icon={CircleGauge} label="Estimated severity" value={severity.label} />
            <SummaryMetric icon={Wrench} label="Repair priority" value={repairPriority(severityKey)} />
            <SummaryMetric icon={ShieldAlert} label="Safety concerns" value={safetyConcern ? 'Potential concern' : 'None indicated'} />
...
function WorkflowActions(props: InspectionExperienceProps) {
  ...
  async function runAction() {
    ...
    setMessage(null)
    const response = await fetch(`/api/van-damage/inspections/${props.inspection.id}?businessId=${encodeURIComponent(props.businessId)}`, {
      method: 'PATCH',
  ...
  return <section aria-label="Inspection workflow" className="rounded-2xl border border-white/10 bg-graphite-800 px-4 py-3">
    <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
      <div><p className="text-xs font-medium text-white/70">Review workflow</p><p className="mt-0.5 text-[11px] text-white/35">{message || 'Actions are recorded in the inspection audit trail.'}</p></div>
      <div className="flex flex-wrap gap-2">
        {actions.map(({ icon: Icon, ...action }) => <button key={action.action} disabled={pending} onClick={() => setConfirmation(action)} className={`focus-ring inline-flex items-center rounded-xl border px-3 py-2 text-xs transition disabled:opacity-50 ${action.danger ? 'border-red-400/15 text-red-200/70 hover:bg-red-400/10' : action.action === 'approve' ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-200 hover:bg-emerald-400/15' : 'border-white/10 text-white/60 hover:bg-white/5 hover:text-white'}`}><Icon className="mr-1.5 h-3.5 w-3.5" />{action.label}</button>)}
      </div>
    </div>
    {confirmation && <div role="dialog" aria-modal="true" aria-labelledby="workflow-dialog-title" className="fixed inset-0 z-[90] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-graphite-800 p-6 shadow-panel-lg">
        <div className="flex items-start justify-between"><div><h2 id="workflow-dialog-title" className="font-semibold text-white">{confirmation.title}</h2><p className="mt-2 text-sm leading-6 text-white/50">{confirmation.body}</p></div><button autoFocus onClick={() => setConfirmation(null)} aria-label="Cancel" className="focus-ring rounded-lg p-1 text-white/40 hover:text-white"><X className="h-4 w-4" /></button></div>
        <div className="mt-6 flex justify-end gap-2"><button onClick={() => setConfirmation(null)} className="focus-ring rounded-xl border border-white/10 px-4 py-2 text-sm text-white/60 hover:bg-white/5">Cancel</button><button disabled={pending} onClick={runAction} className={`focus-ring rounded-xl px-4 py-2 text-sm font-medium disabled:opacity-50 ${confirmation.danger ? 'bg-red-500 text-white' : 'bg-white text-graphite-950'}`}>{pending ? 'Saving…' : 'Confirm'}</button></div>
      </div>
    </div>}
  </section>
}
...
function ConfidenceRing({ value }: { value: number | null }) {
  ...
  return <div className="relative h-16 w-16 shrink-0" title={`${percent}% AI confidence`}>
    <svg viewBox="0 0 64 64" className="-rotate-90" aria-hidden="true"><circle cx="32" cy="32" r="27" fill="none" stroke="rgba(255,255,255,.08)" strokeWidth="5" /><circle cx="32" cy="32" r="27" fill="none" stroke={color} strokeWidth="5" strokeLinecap="round" pathLength="100" strokeDasharray="100" strokeDashoffset={100 - percent} className="transition-all duration-1000" /></svg>
    <span className="absolute inset-0 flex items-center justify-center text-sm font-semibold text-white">{value == null ? '—' : `${percent}%`}</span>
  </div>
}
function MetaRow({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return <div className="flex items-center justify-between gap-4"><dt className="text-white/35">{label}</dt><dd className={`truncate text-right capitalize text-white/65 ${mono ? 'font-mono' : ''}`}>{value}</dd></div>
}
function MiniStat({ label, value }: { label: string; value: string }) {
  return <div><p className="text-[10px] uppercase tracking-wider text-white/30">{label}</p><p className="mt-1 truncate text-xs font-medium text-white/70">{value}</p></div>
}
function VerdictState({ label, value, tone }: { label: string; value: string; tone: 'good' | 'warn' | 'bad' }) {
  const styles = tone === 'good' ? 'text-emerald-300 bg-emerald-400/8' : tone === 'warn' ? 'text-amber-300 bg-amber-400/8' : 'text-red-300 bg-red-400/8'
  return <div className={`rounded-xl p-2.5 ${styles}`}><p className="text-[9px] uppercase tracking-wide opacity-60">{label}</p><p className="mt-1 text-[11px] font-medium leading-4">{value}</p></div>
}
function SummaryMetric({ icon: Icon, label, value }: { icon: typeof Activity; label: string; value: string }) {
  return <div className="rounded-xl border border-white/8 bg-white/[.025] p-4"><Icon className="h-4 w-4 text-gold-300/75" /><p className="mt-3 text-[10px] uppercase tracking-wider text-white/30">{label}</p><p className="mt-1 line-clamp-2 text-sm font-medium capitalize text-white/70">{value}</p></div>
}
function ConfidenceGuide({ label, range, color }: { label: string; range: string; color: string }) {
  return <div className="flex items-center justify-between text-xs"><span className="flex items-center text-white/55"><span className={`mr-2 h-2 w-2 rounded-full ${color}`} />{label}</span><span className="text-white/30">{range}</span></div>
}
function repairPriority(severity: keyof typeof severityPresentation) {
  return severity === 'critical' ? 'Immediate' : severity === 'major' ? 'High · before next rental' : severity === 'moderate' ? 'Schedule promptly' : 'Monitor / routine'
}
...
function SeverityPanel({ items, active }: { items: DamageItem[]; active: keyof typeof severityPresentation }) {
  const cards = [
    ...
    { key: 'moderate', label: 'Moderate', description: 'Surface repair advised', levels: ['medium'] },
    { key: 'major', label: 'Major', description: 'Repair before service', levels: ['high'] },
    { key: 'critical', label: 'Critical', description: 'Potential safety impact', levels: ['critical'] },
  ]
  return <section className="rounded-2xl border border-white/10 bg-graphite-800 p-5 md:p-6">
    <h2 className="font-semibold text-white">Severity profile</h2><p className="mt-1 text-xs text-white/35">Highest observed class: <span className="capitalize text-white/60">{active}</span></p>
    <div className="mt-5 grid grid-cols-2 gap-3">
      {cards.map((card, index) => {
        const count = items.filter((item) => card.levels.includes(item.severity ?? '')).length
        const selected = card.key === active
        return <div key={card.key} className={`relative overflow-hidden rounded-xl border p-3 transition ${selected ? severityPresentation[card.key as keyof typeof severityPresentation].classes : 'border-white/8 bg-white/[.02] text-white/40'}`}>
          <div className="flex items-center justify-between"><span className="text-xs font-medium">{card.label}</span><span className="text-xs">{count}</span></div><p className="mt-2 text-[10px] opacity-55">{card.description}</p><div className="mt-3 h-1 overflow-hidden rounded-full bg-black/20"><div className="h-full rounded-full bg-current transition-all duration-1000" style={{ width: selected ? `${Math.max(30, (index + 1) * 25)}%` : count ? '25%' : '0%' }} /></div>
        </div>
      })}
    </div>
  </section>
...
function RepairEstimate({ severity, recommendation, safetyConcern }: { severity: keyof typeof severityPresentation; recommendation: string; safetyConcern: boolean }) {
  const values = severity === 'critical'
    ? ['Specialist / safety repair', 'Immediate', 'Several days', 'Remove from service and inspect']
    : severity === 'major' ? ['Body repair', 'High', '1–3 days', 'Inspect before next rental']
      : severity === 'moderate' ? ['Light body / surface repair', 'Medium', 'Several hours–1 day', 'Schedule repair promptly']
  ...
  return <section className="rounded-2xl border border-gold-400/15 bg-[linear-gradient(135deg,rgba(201,168,76,.08),rgba(20,20,22,1)_45%)] p-5 md:p-6">
    <div className="flex items-start justify-between"><div><div className="flex items-center gap-2"><Wrench className="h-4 w-4 text-gold-300" /><h2 className="font-semibold text-white">Estimated repair plan</h2></div><p className="mt-1 text-xs text-white/35">AI-generated operational estimate · not a repair quote</p></div><span className="rounded-full border border-gold-400/20 bg-gold-400/10 px-2.5 py-1 text-[10px] text-gold-200">AI estimate</span></div>
    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {['Complexity', 'Priority', 'Downtime', 'Next action'].map((label, index) => <div key={label} className="rounded-xl border border-white/8 bg-black/10 p-4"><p className="text-[10px] uppercase tracking-wider text-white/30">{label}</p><p className="mt-2 text-sm font-medium text-white/70">{values[index]}</p></div>)}
    </div>
    <p className="mt-4 text-xs leading-5 text-white/45">{recommendation}{safetyConcern ? ' A qualified technician should confirm any possible safety concern.' : ''}</p>
  </section>
}
...
function ProcessingTimeline({ inspection, job, aiRun }: { inspection: Inspection; job: Job; aiRun: AiRun }) {
  const steps = [
    ...
    { label: 'Analysis completed', time: aiRun?.completed_at || inspection.completed_at, icon: CheckCircle2 },
    { label: inspection.review_status === 'reviewed' ? 'Inspection approved' : 'Review pending', time: inspection.reviewed_at as string | undefined, icon: UserRound },
    { label: inspection.van_id ? 'Vehicle updated' : 'Vehicle match pending', time: inspection.completed_at, icon: CarFront },
  ]
  return <section className="rounded-2xl border border-white/10 bg-graphite-800 p-5 md:p-6">
    <div className="flex items-center gap-2"><History className="h-4 w-4 text-violet-300" /><h2 className="font-semibold text-white">Inspection timeline</h2></div>
...
function CommentsPanel(props: InspectionExperienceProps) {
  ...
  const comments = [
    {
      id: `system-${props.inspection.id}`, body: 'Inspection created from Slack image intake.',
      kind: 'system', authorName: 'NexoraNow', createdAt: props.inspection.created_at, parentId: null, attachments: [] as RecordValue[],
    },
    ...(props.inspection.ai_summary ? [{
      id: `ai-${props.inspection.id}`, body: props.inspection.ai_summary,
      kind: 'ai', authorName: 'Gemini AI', createdAt: props.aiRun?.completed_at || props.inspection.completed_at || props.inspection.created_at, parentId: null, attachments: [] as RecordValue[],
    }] : []),
    ...userComments.map((comment) => ({
      id: String(comment.id), body: asText(comment.body), kind: asText(comment.kind, 'internal'),
      authorName: asText(comment.authorName, 'Team member'), createdAt: asText(comment.createdAt, props.inspection.updated_at),
  ...
  async function submit() {
    ...
    const response = await fetch(`/api/van-damage/inspections/${props.inspection.id}?businessId=${encodeURIComponent(props.businessId)}`, { method: 'POST', body: form })
    const result = await response.json() as { error?: string }
    setBusy(false)
  ...
  return <section className="rounded-2xl border border-white/10 bg-graphite-800 p-5 md:p-6">
    ...
    <div className="mt-5 rounded-xl border border-white/10 bg-black/10 p-3">
      ...
      <div className="mt-2 flex items-center justify-between gap-3">
        ...
        <button disabled={busy || !body.trim()} onClick={submit} className="focus-ring inline-flex shrink-0 items-center rounded-lg bg-white px-3 py-2 text-xs font-medium text-graphite-950 disabled:opacity-40"><Send className="mr-1.5 h-3.5 w-3.5" />{busy ? 'Posting…' : 'Post note'}</button>
      </div>
    </div>
  </section>
}
...
function CommentAttachments({ attachments, inspectionId, businessId }: { attachments: RecordValue[]; inspectionId: string; businessId: string }) {
  ...
  return <div className="mt-3 flex flex-wrap gap-2">{attachments.map((attachment) => <a key={String(attachment.id)} href={`/api/van-damage/inspections/${inspectionId}/attachments/${String(attachment.id)}?businessId=${encodeURIComponent(businessId)}`} className="focus-ring inline-flex items-center rounded-lg border border-white/10 bg-white/[.03] px-2.5 py-1.5 text-[10px] text-white/55 hover:bg-white/5 hover:text-white"><Paperclip className="mr-1.5 h-3 w-3" /><span className="max-w-48 truncate">{asText(attachment.name, 'Attachment')}</span>{typeof attachment.size === 'number' && <span className="ml-1.5 text-white/25">{(attachment.size / 1024).toFixed(0)} KB</span>}</a>)}</div>
}

function VehicleHealth({ vehicle, related, currentDamage }: { vehicle: Vehicle; related: RelatedInspection[]; currentDamage: number }) {
  const points = [...related].reverse().map((item) => item.damage_count).concat(currentDamage)
  const max = Math.max(1, ...points)
  const path = points.length > 1 ? points.map((value, index) => `${index ? 'L' : 'M'} ${10 + index * (280 / (points.length - 1))} ${85 - value / max * 65}`).join(' ') : 'M 10 85 L 290 85'
  const repairs = asRecordArray(asRecord(vehicle?.metadata.vanDamage).repairs).length
  return <section className="rounded-2xl border border-white/10 bg-graphite-800 p-5 md:p-6">
    <div className="flex items-center justify-between"><div><h2 className="font-semibold text-white">Vehicle health</h2><p className="mt-1 text-xs text-white/35">Condition trend across inspection history</p></div><Gauge className="h-5 w-5 text-emerald-300" /></div>
    <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_220px]">
      <div className="rounded-xl border border-white/8 bg-black/10 p-3"><svg viewBox="0 0 300 100" className="h-32 w-full" aria-label="Damage finding trend"><defs><linearGradient id="damageTrend" x1="0" y1="0" x2="1" y2="0"><stop stopColor="#c9a84c" stopOpacity=".4" /><stop offset="1" stopColor="#e8c34a" /></linearGradient></defs><path d="M10 85 H290" stroke="rgba(255,255,255,.08)" /><path d={path} fill="none" stroke="url(#damageTrend)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /></svg><p className="text-center text-[10px] uppercase tracking-wider text-white/25">Damage findings over time</p></div>
      <div className="grid grid-cols-2 gap-3">
        <MiniHealth label="Inspections" value={String(related.length + 1)} />
        <MiniHealth label="Open" value={String(related.filter((item) => item.status === 'needs_review').length + (currentDamage > 0 ? 1 : 0))} />
        <MiniHealth label="Repairs" value={String(repairs)} />
        <MiniHealth label="Availability" value={vehicle?.status === 'active' ? 'Available' : humanize(vehicle?.status)} />
      </div>
    </div>
  </section>
}
...
function ActivityFeed({ inspection, job, aiRun, images }: { inspection: Inspection; job: Job; aiRun: AiRun; images: DamageImage[] }) {
  ...
  const events = [
    ...custom,
    ...(inspection.reviewed_at ? [{ label: `Review status changed to ${humanize(inspection.review_status)}`, time: inspection.reviewed_at, icon: FileCheck2 }] : []),
    ...(inspection.van_id ? [{ label: 'Vehicle record updated', time: inspection.completed_at || inspection.updated_at, icon: CarFront }] : []),
    ...(aiRun?.completed_at ? [{ label: 'AI analysis completed', time: aiRun.completed_at, icon: Bot }] : []),
    ...images.map((image) => ({ label: 'Image uploaded', time: image.updated_at || image.created_at, icon: ImageIcon })),
    ...(job?.started_at ? [{ label: 'Processing started', time: job.started_at, icon: Clock3 }] : []),
    { label: 'Inspection created', time: inspection.created_at, icon: FileCheck2 },
  ].sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime())
  return <section className="rounded-2xl border border-white/10 bg-graphite-800 p-5">
    <div className="flex items-center justify-between"><h2 className="text-sm font-semibold text-white">Activity</h2><Activity className="h-4 w-4 text-violet-300" /></div>
...
function InspectionMetadata(props: InspectionExperienceProps) {
  ...
  const rows = [
    ...
    ['S3 image count', String(props.images.filter((image) => image.status === 'uploaded' || image.status === 'analyzed').length)],
    ['Processing duration', processingDuration],
  ]
  return <section className="overflow-hidden rounded-2xl border border-white/10 bg-graphite-800">
    <button onClick={() => setOpen((value) => !value)} aria-expanded={open} className="focus-ring flex w-full items-center justify-between px-5 py-4 text-left md:px-6"><span><span className="font-semibold text-white">Inspection metadata</span><span className="ml-2 text-xs text-white/30">Technical details</span></span><ChevronDown className={`h-4 w-4 text-white/40 transition ${open ? 'rotate-180' : ''}`} /></button>
    {open && <dl className="grid gap-px border-t border-white/8 bg-white/5 sm:grid-cols-2">{rows.map(([label, value]) => <div key={label} className="flex items-center justify-between gap-4 bg-graphite-800 px-5 py-3 text-xs"><dt className="text-white/35">{label}</dt><dd className="truncate text-right text-white/60">{value}</dd></div>)}</dl>}
  </section>
}
