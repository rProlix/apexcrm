...
export function SectionDivider({ style, position, fillColor, height = 80 }: Props) {
  ...
  const svgStyle: React.CSSProperties = {
    ...
    zIndex: 2,
    pointerEvents: 'none',
    ...(position === 'top' ? { top: `-${h - 1}px` } : { bottom: `-${h - 1}px` }),
  }
    ...
    case 'curve':
      return (
        <svg
          ...
          style={svgStyle}
        >
          <path d="M0,80 Q720,-10 1440,80 L1440,80 L0,80 Z" fill={fill} />
        </svg>
      )
    ...
    case 'fade':
      return (
        ...
        >
          ...
          <rect x="0" y="0" width="1440" height="80" fill={`url(#fade-grad-${position})`} />
        </svg>
      )
    ...
    case 'overlap':
      return (
        <svg
          ...
          preserveAspectRatio="none"
          aria-hidden="true"
          style={{ ...svgStyle, height: `${h * 1.5}px` }}
        >
          <path d="M0,40 C360,80 1080,0 1440,40 L1440,120 L0,120 Z" fill={fill} />
        </svg>
      )
