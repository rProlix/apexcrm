...
export async function applyCanvaImport(params: {
  ...
  const allowCustomDomains =
    params.allowCustomDomains ??
    (importRow as { validation_mode?: string }).validation_mode === 'custom_domain'
  ...
  try {
    const { data: s } = await db
      .from('site_settings')
      .select('pov_enabled, pov_event_id')
  ...
  if (!homeId) {
    return {
      ok: false,
      mode: importRow.import_mode,
      sectionsWritten: 0,
      warnings,
      animationPreservation: 'unknown',
    }
  }
  ...
  const embedSrc =
    importRow.import_mode === 'preserve'
  ...
  if (importRow.import_mode === 'preserve') {
    if (embedSrc) {
      ...
      if (mode === 'custom_domain') {
        warnings.push(
          'Custom domain accepted. Embedding may fail if the domain blocks iframes — a fallback "Open Canva Website" button is shown.'
        )
      }
    } else {
  ...
  try {
    await db
      ...
      .update({
        ...
        canva_embed_code:
          importRow.import_mode === 'preserve'
            ? buildSafeCanvaIframe(importRow.source_url ?? importRow.embed_code, {
                allowCustomDomains,
              })
            : null,
  ...
  return {
    ...
    warnings,
    animationPreservation: preservation,
  }
}
