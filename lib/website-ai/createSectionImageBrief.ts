...
function inferBusinessTypeFromContent(ctx: RichImageContext): string | null {
  ...
  if (
    /pho|noodle|ramen|sushi|pizza|burger|restaurant|cafe|coffee|bakery|food|cuisine|dining|menu|dish/.test(
      allText
    )
  )
    return 'restaurant'
  if (/ceramic coat|detail|polish|paint correction|car wash|auto detailing/.test(allText))
    return 'auto_shop'
  if (/hair|lash|brow|facial|wax|nail|beauty|salon|spa|skincare|estheti/.test(allText))
    return 'salon'
...
function buildBaseBrief(
  ...
  switch (section.section_type) {
    case 'hero':
      return buildHeroBrief(businessType, ctx, section)
    case 'about':
      return buildAboutBrief(businessType, ctx, section)
    case 'feature_grid':
      return buildFeatureGridBrief(businessType, ctx, section)
    case 'testimonials':
      return buildTestimonialsBrief(businessType, ctx, section)
    case 'faq':
      return buildFaqBrief(businessType, ctx, section)
    case 'contact':
      return buildContactBrief(businessType, ctx, section)
    case 'product_grid':
      return buildProductGridBrief(businessType, ctx, section)
    case 'image_gallery':
      return buildGalleryBrief(businessType, ctx, section)
    case 'cta':
      return buildCtaBrief(businessType, ctx, section)
    default:
      return buildGenericBrief(businessType, ctx, section)
  }
}
...
function buildHeroBrief(
  ...
  const servicesPreview = ctx.services
    .slice(0, 3)
    .map((s) => s.name)
    .join(', ')
  ...
  return {
    ...
    composition:
      'Wide hero composition (16:9), subject prominent in center or rule-of-thirds, clear negative space for text overlay',
    environment: environmentMap[bt] ?? environmentMap.general,
    styling: 'Premium commercial photography, clean and professional',
    lighting:
      bt === 'restaurant'
        ? 'Warm natural or ambient restaurant lighting'
        : bt === 'salon'
          ? 'Bright, clean, flattering salon lighting'
          : 'Professional even lighting, no harsh shadows',
    mood:
      bt === 'restaurant'
...
function buildFeatureGridBrief(
  bt: string,
  ctx: RichImageContext,
  s: RichSectionDetail
): SectionImageBrief {
...
function buildTestimonialsBrief(
  bt: string,
  ctx: RichImageContext,
  s: RichSectionDetail
): SectionImageBrief {
  const reviewTone = ctx.reviews.length > 0 ? ctx.reviews[0].text.slice(0, 80) : 'happy customers'
  ...
  return {
    ...
    mood:
      bt === 'restaurant'
        ...
        : bt === 'salon'
          ? 'Soft, elegant, relaxing'
          : 'Warm, trustworthy, positive',
...
function buildContactBrief(
  ...
  return {
    ...
    mustShow: [
      'welcoming, accessible feeling',
      'professional environment',
      'physical or visual presence of the business',
    ],
...
function buildProductGridBrief(
  bt: string,
  ctx: RichImageContext,
  s: RichSectionDetail
): SectionImageBrief {
  const topProductNames = ctx.topProducts
    .slice(0, 3)
    .map((p) => p.name)
    .join(', ')
  ...
  return {
    ...
    businessType: bt,
    subject: topProductNames
      ? `Products: ${topProductNames}`
      : `Product collection for ${ctx.tenantName}`,
    mustShow: [
    ...
    environment:
      bt === 'restaurant'
        ? 'Restaurant table or food styling environment'
        : 'Clean studio with neutral background',
