...
export function ItemCustomizerModal({ product, editingItem, settings, onConfirm, onClose }: Props) {
  ...
  const [selectedMods, setSelectedMods] = useState<CartModifierSelection[]>(
  ...
  const toggleModifier = (
    ...
    setSelectedMods((prev) => {
      ...
      if (
        group.max_allowed &&
        prev.filter((m) => m.modifier_group_id === group.id).length >= group.max_allowed
      ) {
        return prev
      }
      return [...prev, sel]
      ...
      <div className="w-full sm:max-w-lg bg-zinc-900 sm:rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          {modifierGroups.map((group) => {
            ...
            const mods = ((group.modifiers ?? []) as unknown as Record<string, unknown>[]).filter(
              (m) => m.status === 'active'
            )
            ...
            return (
              <div key={group.id}>
                ...
                {group.description && (
                  <p className="text-xs text-zinc-500 mb-2">{group.description}</p>
                )}
                  ...
                  {mods.map((mod: Record<string, unknown>) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          toggleModifier(group, mod as Parameters<typeof toggleModifier>[1])
                        }
                      ...
                      >
                        ...
                        {priceDelta !== 0 && (
                          <span
                            className={`text-xs ml-2 flex-none ${sel ? 'text-violet-200' : priceDelta > 0 ? 'text-zinc-400' : 'text-green-400'}`}
                          >
                            {priceDelta > 0 ? '+' : ''}
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <button
                ...
                className="w-9 h-9 flex items-center justify-center bg-zinc-800 hover:bg-zinc-700 rounded-lg text-zinc-300 transition-colors"
              >
