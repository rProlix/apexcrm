...
function matchesFilter(w: WebsiteWithUrl, f: FilterKey): boolean {
  switch (f) {
    ...
    case 'published':
      return w.status === 'published'
    case 'business':
      return w.website_type === 'business' || w.website_type === 'creative'
    case 'events':
      return w.website_type === 'invitational'
    case 'pov':
      return w.website_type === 'pov_event' || w.pov_badge
    case 'canva':
      return w.canva_badge
  }
}
...
export function WebsitesListClient({
  ...
  tenantId: string
  initialWebsites: WebsiteWithUrl[]
  rootDomain: string
}) {
  ...
  const visible = useMemo(
    () => websites.filter((w) => matchesFilter(w, filter)),
    [websites, filter]
  )
  ...
  async function archive(w: WebsiteWithUrl) {
    if (
      !confirm(`Archive “${w.name}”? Its public URL will stop serving. You can restore it later.`)
    )
      return
    setBusyId(w.id)
    setError(null)
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {visible.map((w) => {
            ...
            const connectedDomain = w.custom_domain
              ? w.custom_domain
              : w.subdomain
                ? `${w.subdomain}.${rootDomain}`
                : null
            const published = w.status === 'published'
            return (
              <div
                key={w.id}
                className="rounded-2xl border border-surface-border bg-graphite-800/60 p-5 space-y-4"
              >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {w.is_primary_business_site && (
                        <span className="text-2xs px-1.5 py-0.5 rounded bg-white/10 text-white/50">
                          Primary
                        </span>
                      )}
                      {w.canva_badge && (
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <p className="text-2xs text-white/30 pl-6">
                    ...
                    {w.published_at
                      ? ` · Published ${new Date(w.published_at).toLocaleDateString()}`
                      : ''}
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  <a href={absoluteUrl(w.preview_url)} target="_blank" rel="noreferrer">
                    <Button variant="ghost" size="sm">
                      <ExternalLink className="h-3.5 w-3.5" /> Preview Draft
                    </Button>
                  </a>
                  <Button
                    variant="primary"
                  ...
                  {published && (
                    <a
                      href={absoluteUrl(w.live_url ?? w.public_url)}
                      target="_blank"
                      rel="noreferrer"
                    >
                      <Button variant="ghost" size="sm">
...
function DomainModal({
  ...
  const inputCls =
    'w-full h-10 px-3 rounded-xl bg-graphite-900 border border-surface-border text-sm text-white placeholder-white/30 focus:border-gold-500/50 focus:outline-none'
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-2xl border border-surface-border bg-graphite-800 p-6 space-y-4"
