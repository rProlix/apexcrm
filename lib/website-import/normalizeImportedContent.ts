...
import type { ExtractedBusinessFields, NormalizedImportContent, StructuredAddress } from './types'
...
function normalizeSocialLinks(raw: Record<string, string>): NormalizedImportContent['socialLinks'] {
...
function normalizeServices(raw: string[]): Array<{ title: string; description: string }> {
  return raw
    .map((s) => ({
...
export function normalizeImportedContent(fields: ExtractedBusinessFields): NormalizedImportContent {
  const businessName = fields.businessName
    ? sanitizeShortText(fields.businessName.value, 100)
    : null

  const description = fields.description ? sanitizeText(fields.description.value) : null

  const phone = fields.phone ? normalizePhone(fields.phone.value) : null

  const email = fields.email ? normalizeEmail(fields.email.value) : null

  const logoUrl = fields.logoUrl ? normalizeUrl(fields.logoUrl.value) : null

  const faviconUrl = fields.faviconUrl ? normalizeUrl(fields.faviconUrl.value) : null
  ...
  const socialLinks = fields.socialLinks ? normalizeSocialLinks(fields.socialLinks.value) : {}

  const services = fields.services ? normalizeServices(fields.services.value as string[]) : []
  ...
  const faqItems = fields.faqItems ? normalizeFaq(fields.faqItems.value) : []

  const images = fields.images
    ? normalizeImages(fields.images.value.map((img) => ({ src: img.src, alt: img.alt })))
    : []
  ...
  const normalized: Omit<NormalizedImportContent, 'confidenceMap'> = {
    ...
    seoDescription: fields.seoDescription?.value
      ? sanitizeShortText(fields.seoDescription.value, 300)
      : null,
